...
Gemma 4 currently builds as the standalone `gemma4` executable. It is not yet
selectable through `coli` and is not exposed by Colibri's OpenAI-compatible
gateway. Those entry points remain follow-up integration work.

CI generates a one-layer Gemma 4 checkpoint with the production tensor names,
Q6_K embeddings, Q4_0 projections and packed routed experts. An independent
NumPy reference records its logits and greedy tokens; the native engine must
match those tokens and logits with expert-cache capacities 1, 2, and 8.
