...
| `COLI_CUDA_TRANSIENT_PREFILL` | `0` (off) | Experimental multi-GPU RAM spill tier for prefill (`S>1`): asynchronously upload non-resident INT4 experts to one reusable staging set per selected GPU, execute them there, then replace the staging weights. Requires at least two CUDA devices. Decode remains on the normal resident-GPU/CPU path. |
