...
| `COLI_CUDA_TRANSIENT_PREFILL` | `0` (off) | Experimental multi-GPU RAM spill tier for prefill (`S>1`): stream non-resident INT4 experts through two reusable pinned staging slots per selected GPU, overlapping the next H2D copy with current expert compute. Requires at least two CUDA devices. Decode remains on the normal resident-GPU/CPU path. |
| `COLI_CUDA_TRANSIENT_MIN_ROWS` | `1` | Minimum routed rows per cold expert for the transient prefill tier. Values above `1` leave smaller expert batches on CPU. |
| `COLI_CUDA_TRANSIENT_NVME` | `0` (off) | With transient prefill and `PIPE=1`, send each ready disk-loaded expert to a GPU while the remaining NVMe reads continue. Supports both loader threads and `URING=1`; value-preserving and opt-in. |
