...
| `COLI_NO_OMP_TUNE` | off | **Kill-switch** for the OpenMP hot-thread tuning (`OMP_WAIT_POLICY=passive` + `GOMP_SPINCOUNT` spin window (libgomp) / `KMP_BLOCKTIME=200` (LLVM libomp) + proc-bind). Keeps the worker team hot across back-to-back matmul regions but parks idle cores between turns; set `=1` when the CPU is mostly waiting on the GPU (Metal) so the brief spin doesn't steal the shared power budget. |
