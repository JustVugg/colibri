...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "usage: %s file [blkMB] [n] [threads] [direct 0/1]\n", argv[0]);
        return 1;
    }
    ...
    if (fd < 0 && direct) {
        ...
        direct = 0;
        fd = open(argv[1], COMPAT_O_RDONLY);
    }
#else
    ...
    off_t *offs = malloc(n * sizeof(off_t));
    srand(1234);
    for (int i = 0; i < n; i++) {
    ...
    double t0 = now();
    int64_t tot = 0; /* long e' 32-bit su Windows (LLP64): >2GB andava in overflow */
