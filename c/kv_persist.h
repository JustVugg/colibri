...
static int64_t kv_rec_bytes(Model *m) {
    ...
    if (m->has_dsa)
        for (int i = 0; i < c->n_layers; i++)
            if (m->Ic[i]) rec += (int64_t)c->index_hd * 4;
    return rec;
}
...
static int kv_disk_open(Model *m) {
    ...
    if (!k->disk_fp) {
        ...
        int32_t h[8];
        kv_hdr(m, h, 0);
        fwrite(KV_MAGIC, 1, 8, k->disk_fp);
        fwrite(h, 4, 8, k->disk_fp);
        fflush(k->disk_fp);
        fclose(k->disk_fp);
        k->disk_fp = fopen(k->disk_path, "r+b");
        if (!k->disk_fp) return 0;
...
static void kv_disk_append(Model *m, const int *hist, int len) {
    ...
    for (int p = k->disk_nrec; p < len; p++) {
        ...
        *(int32_t *)b = hist[p];
        b += 4;
        for (int i = 0; i < c->n_layers; i++) {
            memcpy(b, m->Lc[i] + (int64_t)p * c->kv_lora, (size_t)c->kv_lora * 4);
            b += c->kv_lora * 4;
            memcpy(b, m->Rc[i] + (int64_t)p * c->qk_rope, (size_t)c->qk_rope * 4);
            b += c->qk_rope * 4;
        }
        if (m->has_dsa)
            for (int i = 0; i < c->n_layers; i++)
                if (m->Ic[i]) {
                    memcpy(b, m->Ic[i] + (int64_t)p * c->index_hd, (size_t)c->index_hd * 4);
                    b += c->index_hd * 4;
                }
...
static int kv_disk_load(Model *m, int *hist, int maxctx) {
    ...
    KVState *k = m->kv;
    Cfg *c = &m->c;
    FILE *f = fopen(k->disk_path, "rb");
    if (!f) return 0;
    char mg[8];
    int32_t h[8], w[8];
    kv_hdr(m, w, 0);
    if (fread(mg, 1, 8, f) != 8 || memcmp(mg, KV_MAGIC, 8) || fread(h, 4, 8, f) != 8 || h[0] != w[0] || h[1] != w[1] ||
        h[2] != w[2] || h[3] != w[3] || h[4] != w[4] || h[5] != w[5]) {
    ...
    for (int p = 0; p < nrec; p++) {
        ...
        for (int i = 0; i < c->n_layers; i++) {
            if (fread(m->Lc[i] + (int64_t)p * c->kv_lora, 4, c->kv_lora, f) != (size_t)c->kv_lora ||
                fread(m->Rc[i] + (int64_t)p * c->qk_rope, 4, c->qk_rope, f) != (size_t)c->qk_rope) {
                nrec = p;
                goto out;
            }
        }
        if (m->has_dsa)
            for (int i = 0; i < c->n_layers; i++)
                if (m->Ic[i])
                    if (fread(m->Ic[i] + (int64_t)p * c->index_hd, 4, c->index_hd, f) != (size_t)c->index_hd) {
                        nrec = p;
                        goto out;
                    }
    }
