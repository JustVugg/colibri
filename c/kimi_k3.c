...
typedef struct {
    void *w1, *w2, *w3;
} VkExp; /* ColiVkTensor* triple */
static VkExp *g_vkexp;
static int64_t g_vkexp_n;
...
static int vk_budget_full(void) {
    ...
    double cap = budget - 0.5;
    if (g_vk_gb > 0 && g_vk_gb < cap) cap = g_vk_gb;
    return used >= cap;
}
static int w_vk_upload(W *w) {
    ...
    if (fmt < 0) return 0;
    return coli_vk_tensor_ensure((ColiVkTensor **)&w->vk, fmt == 1 ? (const void *)w->q8 : (const void *)w->q4, w->s,
                                 fmt, w->I, w->O, w->gs);
}
#endif
static void w_matmul(float *y, const float *x, const W *w, int S) {
    ...
    if (w->fmt == 0) matmul(y, x, w->f, S, w->I, w->O);
        ...
        fprintf(stderr, "w_matmul: bad fmt %d\n", w->fmt);
        exit(1);
    }
}
...
static void w_addrow(const W *w, int r, float coef, float *acc) {
    ...
    if (w->fmt == 0) {
        ...
        const int8_t *p = w->q8 + (int64_t)r * I;
        float s = w->s[r] * coef;
        for (int i = 0; i < I; i++) acc[i] += s * p[i];
    } else {
        int rb = (I + 1) / 2, ng = (I + w->gs - 1) / w->gs;
        const uint8_t *p = w->q4 + (int64_t)r * rb;
        const float *scl = w->s + (int64_t)r * ng;
        for (int g = 0; g * w->gs < I; g++) {
            float s = scl[g] * coef;
            int e = (g + 1) * w->gs;
            if (e > I) e = I;
            for (int i = g * w->gs; i < e; i += 2) {
                uint8_t b = p[i >> 1];
...
static void w_load(Model *m, W *w, const char *name, int O, int I, int bits) {
    char nm[512];
    snprintf(nm, sizeof(nm), "%s%s", m->pfx, name);
    ...
    if (t->dtype == 3) {
        ...
        if (!ts) {
            fprintf(stderr, "%s: quantized (U8) but no %s scale sidecar\n", nm, qn);
            exit(1);
        }
        if (t->nbytes == (int64_t)O * I && ts->numel == O) { /* int8 per-row */
            if (g_bits_env && bits == 4 && I % 64 == 0) {
                ...
                float *s8 = falloc(O);
                if (!q8) {
                    fprintf(stderr, "OOM int8 tmp %s\n", nm);
                    exit(1);
                }
                st_read_raw(&m->S, nm, q8, 1);
                st_read_f32(&m->S, qn, s8, 0);
                w->fmt = 4;
                w->gs = gs;
                w->q4 = malloc((int64_t)O * rb);
                w->s = falloc((int64_t)O * ng);
                if (!w->q4) {
                    fprintf(stderr, "OOM int4 %s\n", nm);
                    exit(1);
                }
                for (int r = 0; r < O; r++) {
                    const int8_t *src = q8 + (int64_t)r * I;
                    float sc8 = s8[r];
                    uint8_t *dst = w->q4 + (int64_t)r * rb;
                    float *scl = w->s + (int64_t)r * ng;
                    for (int g = 0; g < ng; g++) {
                        const int8_t *gp = src + g * gs;
                        int am = 0;
                        for (int i = 0; i < gs; i++) {
                            int a = gp[i] < 0 ? -gp[i] : gp[i];
                            if (a > am) am = a;
                        }
                        float s = am * sc8 / 7.f;
                        ...
                        for (int i = 0; i < gs; i += 2) {
                            int v0 = (int)lrintf(gp[i] * inv);
                            if (v0 > 7) v0 = 7;
                            if (v0 < -8) v0 = -8;
                            int v1 = (int)lrintf(gp[i + 1] * inv);
                            if (v1 > 7) v1 = 7;
                            if (v1 < -8) v1 = -8;
            ...
            if (!w->q8) {
                fprintf(stderr, "OOM int8 %s\n", nm);
                exit(1);
            }
            st_read_raw(&m->S, nm, w->q8, 1);
            w->s = falloc(O);
            st_read_f32(&m->S, qn, w->s, 0);
            ...
            w->s = falloc((int64_t)O * (I / 64));
            st_read_f32(&m->S, qn, w->s, 0);
        } else {
            fprintf(stderr,
                    "%s: U8 tensor is %lld bytes / %lld scales — matches neither int8 [%d,%d] nor int4-g64, refusing "
                    "(untrusted container)\n",
                    nm, (long long)t->nbytes, (long long)ts->numel, O, I);
            exit(1);
        }
        return;
    }
    if (t->numel != (int64_t)O * I) {
        fprintf(stderr, "%s: numel %lld != %dx%d\n", nm, (long long)t->numel, O, I);
        exit(1);
    }
    if (bits >= 32) {
        w->fmt = 0;
        w->f = falloc((int64_t)O * I);
        st_read_f32(&m->S, nm, w->f, 0);
        return;
    }
    int gs = 64;
    if (bits <= 4 && I % gs) {
        bits = 8;
    } /* int4-g64 wants I%64==0; fall back */
    float *scr = falloc((int64_t)QCHUNK * I);
    if (bits > 4) {
        w->fmt = 1;
        w->q8 = malloc((int64_t)O * I);
        w->s = falloc(O);
        if (!w->q8) {
            fprintf(stderr, "OOM int8 %s\n", nm);
            exit(1);
        }
        for (int r0 = 0; r0 < O; r0 += QCHUNK) {
            int n = O - r0 < QCHUNK ? O - r0 : QCHUNK;
            st_read_slice_f32(&m->S, nm, (int64_t)r0 * I, (int64_t)n * I, scr, 1);
            for (int r = 0; r < n; r++) {
                const float *src = scr + (int64_t)r * I;
                float am = 0;
                for (int i = 0; i < I; i++) {
                    float a = fabsf(src[i]);
                    if (a > am) am = a;
        ...
        for (int r0 = 0; r0 < O; r0 += QCHUNK) {
            ...
            for (int r = 0; r < n; r++) {
                const float *src = scr + (int64_t)r * I;
                uint8_t *dst = w->q4 + (int64_t)(r0 + r) * rb;
                float *scl = w->s + (int64_t)(r0 + r) * ng;
                for (int g = 0; g < ng; g++) {
                    const float *gp = src + g * gs;
                    float am = 0;
                    for (int i = 0; i < gs; i++) {
                        float a = fabsf(gp[i]);
                        if (a > am) am = a;
...
static float *f32_load(Model *m, const char *name, int64_t want) {
    ...
    if (want > 0 && t->numel != want) {
        fprintf(stderr, "%s: numel %lld != %lld\n", nm, (long long)t->numel, (long long)want);
        exit(1);
    }
...
static void load_cfg(Cfg *c, const char *snap) {
    ...
    jval *ep = json_get(tc, "rms_norm_eps");
    ...
    c->gate_lb = lb ? (float)lb->num : -5.f;
    c->kda_proj = c->kda_heads * c->kda_hd;
    if (c->hidden < 1 || c->hidden > 65536 || c->n_layers < 1 || c->n_layers > 128 || c->n_experts < 1 ||
        c->n_experts > 4096 || c->topk < 1 || c->topk > 64 || c->topk > c->n_experts || c->vocab < 1 ||
        c->vocab > (1 << 22) || c->kda_proj < 1 || c->kda_proj > (1 << 20) || c->conv_k < 1 || c->conv_k > 8 ||
        c->latent < 32 || c->latent % 32 || c->moe_inter % 32 || c->res_bs < 1 || c->kda_hd > 512 ||
        c->kv_lora > 4096) {
    ...
    if (!kl || kl->t != J_ARR) {
        fprintf(stderr, "config.json: missing kda_layers\n");
        exit(1);
    }
    for (int i = 0; i < kl->len; i++) {
        int v = (int)kl->kids[i]->num; /* 1-indexed */
        if (v >= 1 && v <= c->n_layers) c->is_kda[v - 1] = 1;
    }
    jval *b = json_get(root, "bos_token_id");
    if (!b) b = json_get(tc, "bos_token_id");
    c->bos = b && b->t == J_NUM ? (int)b->num : -1;
    jval *e = json_get(root, "eos_token_id");
    if (!e) e = json_get(tc, "eos_token_id");
    if (e && e->t == J_NUM) c->eos[c->n_eos++] = (int)e->num;
    else if (e && e->t == J_ARR)
        for (int i = 0; i < e->len && c->n_eos < 8; i++) c->eos[c->n_eos++] = (int)e->kids[i]->num;
    free(buf);
    (void)arena;
}
...
static void expert_table_init(Model *m) {
    Cfg *c = &m->c;
    m->e_w1p = (int64_t)c->moe_inter * (c->latent / 2);
    m->e_w1s = (int64_t)c->moe_inter * (c->latent / 32);
    m->e_w2p = (int64_t)c->latent * (c->moe_inter / 2);
    m->e_w2s = (int64_t)c->latent * (c->moe_inter / 32);
    m->e_slot = 2 * (m->e_w1p + m->e_w1s) + m->e_w2p + m->e_w2s;
    m->eref = calloc((size_t)c->n_layers * c->n_experts, sizeof(ERef));
    if (!m->eref) {
        fprintf(stderr, "OOM expert table\n");
        exit(1);
    }
    ...
    for (int li = 0; li < c->n_layers; li++) {
        ...
        for (int e2 = 0; e2 < c->n_experts; e2++) {
            ...
            for (int k = 0; k < 6; k++) {
                char nm[512];
                snprintf(nm, sizeof(nm), "%smodel.layers.%d.block_sparse_moe.experts.%d.%s.weight_%s", m->pfx, li, e2,
                         mat[k / 2], (k & 1) ? "scale" : "packed");
                st_tensor *t = st_find(&m->S, nm);
                if (!t) {
                    missing++;
                    er->fd[k] = -1;
                ...
                if (t->nbytes != want[k]) {
                    fprintf(stderr, "%s: %lld bytes, expected %lld — refusing (untrusted container)\n", nm,
                            (long long)t->nbytes, (long long)want[k]);
                    exit(1);
                }
                er->fd[k] = t->fd;
                er->off[k] = t->off;
    ...
    if (missing)
        fprintf(stderr, "[K3] WARNING: %d expert tensors missing (incomplete download?) — touching one aborts\n",
                missing);
}
...
static void model_init(Model *m, const char *snap, int n_layers_env) {
    ...
    if ((c->n_layers + c->res_bs - 1) / c->res_bs + 1 > 16) {
        fprintf(stderr, "attn_res: too many blocks\n");
        exit(1);
    }
    ...
    if (g_k3_topp > 0.f)
        fprintf(stderr,
                "[K3] TOPP=%.2f: routed experts pruned to cumulative weight (quality lever — A/B with K3_LOGITS)\n",
                g_k3_topp);
...
#define NM(...) (snprintf(nm, sizeof(nm), __VA_ARGS__), nm)
    for (int i = 0; i < c->n_layers; i++) {
        ...
        if (l->kda) {
            ...
            w_load(m, &a->qb, NM("model.layers.%d.self_attn.q_b_proj.weight", i), c->n_heads * c->qk_head, c->q_lora,
                   mbits);
            w_load(m, &a->kva, NM("model.layers.%d.self_attn.kv_a_proj_with_mqa.weight", i), c->kv_lora + c->qk_rope,
                   c->hidden, mbits);
            w_load(m, &a->kvb, NM("model.layers.%d.self_attn.kv_b_proj.weight", i),
                   c->n_heads * (c->qk_nope + c->v_head), c->kv_lora, mbits);
            w_load(m, &a->o, NM("model.layers.%d.self_attn.o_proj.weight", i), c->hidden, c->n_heads * c->v_head,
                   mbits);
            w_load(m, &a->g, NM("model.layers.%d.self_attn.g_proj.weight", i), c->n_heads * c->v_head, c->hidden,
                   mbits);
        ...
        if (l->sparse) {
            Moe *o = &l->moe;
            o->router =
                f32_load(m, NM("model.layers.%d.block_sparse_moe.gate.weight", i), (int64_t)c->n_experts * c->hidden);
            o->rbias =
                f32_load(m, NM("model.layers.%d.block_sparse_moe.gate.e_score_correction_bias", i), c->n_experts);
            o->lat_norm = f32_load(m, NM("model.layers.%d.block_sparse_moe.routed_expert_norm.weight", i), c->latent);
            w_load(m, &o->lat_down, NM("model.layers.%d.block_sparse_moe.routed_expert_down_proj.weight", i), c->latent,
                   c->hidden, bits);
            w_load(m, &o->lat_up, NM("model.layers.%d.block_sparse_moe.routed_expert_up_proj.weight", i), c->hidden,
                   c->latent, bits);
            int shi = c->moe_inter * c->n_shared;
            w_load(m, &o->sh_gate, NM("model.layers.%d.block_sparse_moe.shared_experts.gate_proj.weight", i), shi,
                   c->hidden, bits);
            w_load(m, &o->sh_up, NM("model.layers.%d.block_sparse_moe.shared_experts.up_proj.weight", i), shi,
                   c->hidden, bits);
            w_load(m, &o->sh_down, NM("model.layers.%d.block_sparse_moe.shared_experts.down_proj.weight", i), c->hidden,
                   shi, bits);
        } else {
        ...
        if (i % 8 == 0)
            fprintf(stderr, "[K3] loaded layer %d/%d (%.1fs, RSS %.1f GB)\n", i + 1, c->n_layers, now_s() - t0,
                    rss_gb());
    }
    ...
    if (m->has_head) {
        m->final_norm = f32_load(m, "model.norm.weight", c->hidden);
        {
            ...
            m->out_sw = falloc(c->hidden);
            for (int d = 0; d < c->hidden; d++) m->out_sw[d] = rn[d] * rp[d];
            free(rn);
            free(rp);
        }
...
#ifdef COLI_VULKAN
    {
        ...
        if (!ev || atoi(ev)) {
            char sbuf[512];
            const char *spv = coli_vk_resolve_spv(sbuf, sizeof(sbuf));
        ...
        if (g_k3_vk) {
            ...
            double used = 0, budget = 0;
            coli_vk_mem_budget(&used, &budget);
            fprintf(stderr,
                    "[K3-VK] resident: %d shared-expert mats (%.1f/%.1f GB); routed MXFP4 tier fills at decode "
                    "(K3_VK_UP=%d/step, cap %s)\n",
    ...
    for (int i = 0; i < c->n_layers; i++)
        if (m->L[i].sparse) {
            m->ecache[i].cap = cap;
            m->ecache[i].s = calloc(cap, sizeof(Slot));
...
static void res_mix(float *out, const float *prefix, const float *bres, int nb, int D, const float *sw, float eps) {
    const float *v[16];
    float sc[16];
    ...
    for (int d = 0; d < D; d++) {
        float a = 0;
        for (int e = 0; e <= nb; e++) a += sc[e] * v[e][d];
        out[d] = a;
    }
}
...
static void kda_forward(Model *m, Layer *l, int li, const float *x, int C, float *out) {
    ...
    for (int t = 0; t < C; t++) {
        ...
        float *vecs[3] = {qt, kt, tv};
        float *taps[3] = {a->conv_q, a->conv_k, a->conv_v};
        for (int w2 = 0; w2 < 3; w2++) {
            float *win = wins[w2], *vec = vecs[w2];
            const float *cw = taps[w2];
...
#pragma omp parallel for schedule(static)
        for (int h = 0; h < H; h++) {
            ...
            sq = 1.f / sqrtf(sq + 1e-6f);
            sk = 1.f / sqrtf(sk + 1e-6f);
            for (int i = 0; i < hd; i++) {
                qn[i] = qh[i] * sq * qscale;
                kn[i] = kh[i] * sk;
            }
            ...
            double ms = 0;
            for (int vv = 0; vv < hd; vv++) ms += (double)oh[vv] * oh[vv];
...
#pragma omp parallel for schedule(static)
        for (int h = 0; h < H; h++) {
            ...
            for (int t = 0; t < nt; t++) {
                ...
                float s2 = 0;
                for (int i = 0; i < kvl; i++) s2 += qabs[i] * Lt[i];
            ...
            for (int t = 0; t < nt; t++) {
                const float *Lt = m->Lc[li] + (int64_t)t * kvl;
                float s2 = sc[t];
...
static void expert_read(Model *m, int li, int eid, Slot *s) {
    ...
    if (er->fd[0] < 0) {
        fprintf(stderr, "[K3] expert L%d E%d missing on disk\n", li, eid);
        exit(1);
    }
    if (er->contig) {
        ...
        for (int k = 0; k < 6; k++) {
            if (er->fd[k] < 0) {
                fprintf(stderr, "[K3] expert L%d E%d tensor %d missing on disk\n", li, eid, k);
                exit(1);
            }
...
static void expert_apply(Model *m, Slot *s, const float *z, float wk, float *u, float *gate, float *up, float *hz) {
    Cfg *c = &m->c;
    uint8_t *w1p = s->buf, *w1s = w1p + m->e_w1p, *w2p = w1s + m->e_w1s, *w2s = w2p + m->e_w2p, *w3p = w2s + m->e_w2s,
            *w3s = w3p + m->e_w1p;
...
static int vk_expert_apply(Model *m, int li, int eid, const float *z, float wk, float *u, float *gate, float *up,
                           float *hz) {
    Cfg *c = &m->c;
    VkExp *v = &g_vkexp[(int64_t)li * c->n_experts + eid];
    if (!v->w1 || !v->w2 || !v->w3) return 0;
    if (!coli_vk_matmul_pair((ColiVkTensor **)&v->w1, gate, NULL, NULL, c->moe_inter, (ColiVkTensor **)&v->w3, up, NULL,
                             NULL, c->moe_inter, 7, z, 1, c->latent, 32))
        return 0;
    for (int i = 0; i < c->moe_inter; i++) gate[i] = situf_(gate[i], up[i], c->situ_b1, c->situ_b2);
    if (!coli_vk_matmul((ColiVkTensor **)&v->w2, hz, gate, NULL, NULL, 7, 1, c->moe_inter, c->latent, 32)) return 0;
    for (int i = 0; i < c->latent; i++) u[i] += wk * hz[i];
    g_vk_hit++;
    return 1;
}
...
static void vk_expert_try_upload(Model *m, int li, int eid, Slot *s) {
    ...
    if (ok) {
        for (int64_t i = 0; i < n2; i++) sc[i] = mx4_scale(w2s[i]);
        ok = ok && coli_vk_tensor_ensure((ColiVkTensor **)&v->w2, w2p, sc, 7, MI, LT, 32);
    }
    if (ok) {
        for (int64_t i = 0; i < n1; i++) sc[i] = mx4_scale(w3s[i]);
        ok = ok && coli_vk_tensor_ensure((ColiVkTensor **)&v->w3, w3p, sc, 7, LT, MI, 32);
    }
    free(sc);
    if (!ok) {
        if (v->w1) {
            coli_vk_tensor_free(v->w1);
            v->w1 = NULL;
        }
        if (v->w2) {
            coli_vk_tensor_free(v->w2);
            v->w2 = NULL;
        }
        if (v->w3) {
            coli_vk_tensor_free(v->w3);
            v->w3 = NULL;
        }
        g_vk_full = 1;
        fprintf(stderr, "[K3-VK] expert tier full: %ld experts resident\n", g_vk_res);
...
static void experts_apply_union(Model *m, int li, int nu, const int *uids, const int *pfirst, const int *pcnt,
                                const int *poslist, const float *wlist, const float *Z, int stride, float *U,
    ...
    for (int base = 0; base < nu; base += LP_MAX) {
        ...
        int nmiss = 0;
        for (int j = 0; j < nb; j++) {
            use[j] = slot_find(m, li, uids[base + j]);
            qof[j] = -1;
            if (!use[j]) {
                m->miss++;
                use[j] = &m->ws[nmiss];
        ...
        if (nmiss) {
            ...
            if (g_k3_pipe) {
                for (int q = 0; q < nmiss; q++) {
                    g_lp.job[q].li = li;
                    g_lp.job[q].eid = uids[base + missk[q]];
                    g_lp.job[q].s = &m->ws[q];
                }
                lp_submit(m, nmiss);
            } else {
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        for (int j = 0; j < nb; j++) {
            if (g_k3_pipe && qof[j] >= 0 && !atomic_load_explicit(&g_lp.ready[qof[j]], memory_order_acquire)) {
                double t0 = now_s(); /* t_eload = UN-hidden I/O (wait) time */
                while (!atomic_load_explicit(&g_lp.ready[qof[j]], memory_order_acquire)) usleep(50);
                m->t_eload += now_s() - t0;
            }
#ifdef COLI_VULKAN
            ...
            for (int p2 = 0; p2 < pcnt[base + j]; p2++) {
                int t = poslist[f + p2];
                expert_apply(m, use[j], Z + (int64_t)t * stride, wlist[f + p2], U + (int64_t)t * stride, gate, up, hz);
            }
        }
...
static void moe_forward(Model *m, Layer *l, int li, const float *x, int C, float *out) {
    ...
    int *idxs = malloc((size_t)C * K * sizeof(int));
    float *wsels = falloc((int64_t)C * K);
    int *keff = malloc((size_t)C * sizeof(int));
    if (!idxs || !keff) {
        fprintf(stderr, "OOM moe sel\n");
        exit(1);
    }
    for (int t = 0; t < C; t++) {
        float *st = sco + (int64_t)t * E;
        for (int e = 0; e < E; e++) st[e] = sigmoidf_(st[e]);
        int *idx = idxs + (int64_t)t * K;
        float *wsel = wsels + (int64_t)t * K;
        for (int kk = 0; kk < K; kk++) {
            ...
            idx[kk] = best;
            wsel[kk] = st[best]; /* weight = RAW sigmoid score */
        }
        {
            float sm = 0;
            for (int kk = 0; kk < K; kk++) sm += wsel[kk];
        ...
        if (g_k3_topp > 0.f && g_k3_topp < 1.f) {
            for (int a2 = 1; a2 < Kt; a2++) {
                int e = idx[a2];
                float w2 = wsel[a2];
                int b2 = a2 - 1;
                while (b2 >= 0 && wsel[b2] < w2) {
                    idx[b2 + 1] = idx[b2];
                    wsel[b2 + 1] = wsel[b2];
                    b2--;
                }
            ...
            if (keep < Kt) {
                float sm = 0;
                for (int kk = 0; kk < keep; kk++) sm += wsel[kk];
     ...
     * batches perfectly. */
    {
        ...
        int *poslist = malloc((size_t)C * K * sizeof(int));
        float *wlist = falloc((int64_t)C * K);
        int *cur = malloc((size_t)C * K * sizeof(int));
        if (!map || !uid || !pcnt || !pfirst || !poslist || !cur) {
            fprintf(stderr, "OOM moe union\n");
            exit(1);
        }
        for (int e = 0; e < E; e++) map[e] = -1;
        int nu = 0;
        for (int t = 0; t < C; t++)
            for (int kk = 0; kk < keff[t]; kk++) {
        ...
        for (int t = 0; t < C; t++)
            for (int kk = 0; kk < keff[t]; kk++) {
                ...
                poslist[cur[j]] = t;
                wlist[cur[j]] = wsels[(int64_t)t * K + kk];
                cur[j]++;
            }
        ...
        for (int a2 = 0; a2 < nu - 1; a2++)
            for (int b2 = a2 + 1; b2 < nu; b2++) {
        ...
        if (!g_k3_direct)
            for (int j = 0; j < nu; j++) {
                ...
                int64_t sizes[6] = {m->e_w1p, m->e_w1s, m->e_w2p, m->e_w2s, m->e_w1p, m->e_w1s};
                if (er->contig) {
                    if (er->fd[0] >= 0) posix_fadvise(er->fd[0], er->off[0], m->e_slot, POSIX_FADV_WILLNEED);
                } else
                    for (int k2 = 0; k2 < 6; k2++)
                        if (er->fd[k2] >= 0) posix_fadvise(er->fd[k2], er->off[k2], sizes[k2], POSIX_FADV_WILLNEED);
            }
#ifdef COLI_VULKAN
        ...
        if (g_k3_vk && C == 1) {
            ...
            for (int j = 0; j < nu; j++) {
                ...
                if (pcnt[j] == 1) {
                    ...
                    handled = vk_expert_apply(m, li, uid[j], z + (int64_t)t * LT, wlist[pfirst[j]], u + (int64_t)t * LT,
                                              gate, up, hz);
                }
                if (!handled) {
                    uid[keep] = uid[j];
                    pcnt[keep] = pcnt[j];
                    pfirst[keep] = pfirst[j];
                    keep++;
                }
            }
            nu = keep;
    ...
    for (int t = 0; t < C; t++) rmsnorm_(u + (int64_t)t * LT, u + (int64_t)t * LT, o->lat_norm, LT, c->eps);
    ...
    w_matmul(sg, x, &o->sh_gate, C);
    w_matmul(su, x, &o->sh_up, C);
...
static void dense_forward(Model *m, Layer *l, const float *x, int C, float *out) {
    ...
    w_matmul(g, x, &l->d_gate, C);
    w_matmul(u, x, &l->d_up, C);
...
static float *g_x0 = NULL;
static int g_x0_n = 0;     /* K3_X0: injected inputs (validation) */
...
static float *step_chunk(Model *m, const int *ids, int pos0, int C) {
    ...
    for (int i = 0; i < c->n_layers; i++) {
        ...
        for (int t = 0; t < C; t++) {
            ...
            if (have_prefix) {
                for (int d = 0; d < D; d++) p[d] += a[d];
            } else {
                memcpy(p, a, D * sizeof(float));
            }
...
static void cb_special(ChatB *b, int id) {
    if (b->n >= b->cap) {
        fprintf(stderr, "chat prompt too long\n");
        exit(1);
    }
    b->ids[b->n++] = id;
}
...
static int chat_build(Tok *T, const char *sys, const char *user, int thinking, int *ids, int cap, int *sp) {
    ChatB b = {T,
               ids,
               0,
               cap,
               chat_special(T, "<|open|>"),
               chat_special(T, "<|close|>"),
               chat_special(T, "<|sep|>"),
               chat_special(T, "<|end_of_msg|>")};
    ...
    sp[3] = b.sp_eom;
    if (sys && *sys) {
        cb_open(&b, "message", "system");
        cb_text(&b, sys);
        cb_close(&b, "message");
        cb_special(&b, b.sp_eom);
    }
    cb_open(&b, "message", "user");
    cb_text(&b, user);
    cb_close(&b, "message");
    cb_special(&b, b.sp_eom);
...
static void chat_assistant(ChatB *b, const char *reasoning, const char *text) {
    cb_open(b, "message", "assistant");
    cb_open(b, "think", NULL);
    cb_text(b, reasoning);
    cb_close(b, "think");
    cb_open(b, "response", NULL);
    cb_text(b, text);
    cb_close(b, "response");
    cb_close(b, "message");
    cb_special(b, b->sp_eom);
 ...
 *   G <thinking>\n
 */
static int chat_build_wire(Tok *T, const char *wire, int nwire, int *thinking, int *ids, int cap, int *sp) {
    ChatB b = {T,
               ids,
               0,
               cap,
    ...
    *thinking = 0;
    while (p < end) {
        ...
        if (*p == 'A') {
            ...
            if (!reason || !text) {
                fprintf(stderr, "OOM chat assistant\n");
                exit(1);
            }
            memcpy(reason, nl + 1, (size_t)nr);
            reason[nr] = 0;
            memcpy(text, nl + 1 + nr, (size_t)nt);
            text[nt] = 0;
        ...
        if (!text) {
            fprintf(stderr, "OOM chat message\n");
            exit(1);
        }
        memcpy(text, nl + 1, (size_t)nb);
        text[nb] = 0;
...
static int serve_read_req(ServeReq *q, const char *active) {
    ...
    int slot, plen, max_tok;
    float temp, top_p;
    if (sscanf(line, "%*s %*s %d %d %d %f %f", &slot, &plen, &max_tok, &temp, &top_p) != 5 || plen < 0 ||
        plen > (1 << 24) || max_tok < 1) {
    ...
    if (fread(payload, 1, (size_t)plen, stdin) != (size_t)plen) {
        free(payload);
        return -1;
    }
    (void)fgetc(stdin);
...
static void serve_one(Model *m, Tok *T, ServeReq *q) {
    ...
    if (q->plen >= 8 && !memcmp(q->payload, "K3CHAT1\n", 8)) {
        ...
        if (n < 0) {
            printf("ERROR %s invalid K3 chat payload\n", q->id);
            fflush(stdout);
            free(ids);
            return;
        }
    ...
    if (getenv("K3_PREFIX_LOG")) {
        ...
        if (reuse) fprintf(stderr, "[PREFIX] reusing %d of %d prompt tokens (%.0f%%)\n", reuse, np, 100.0 * reuse / np);
        else
    ...
    for (int s = 0; s < q->max_tok && !cancelled; s++) {
        ...
        lo = NULL;
        int eos = 0;
        for (int i = 0; i < m->c.n_eos; i++)
            if (tk == m->c.eos[i]) eos = 1;
        int show = !eos;
        if (chat && sp[0] >= 0) {
            if (tk == sp[0] || tk == sp[1]) {
                xsup = 1;
                ...
                if (xsup) {
                    xsup = 0;
                    xtag[xtl] = 0;
                    if (xopen && !strcmp(xtag, "response") && thinking) serve_data(q->id, "</think>", 8);
                }
                show = 0;
            } else if (xsup) {
                int nb = tok_decode(T, &tk, 1, buf, sizeof(buf) - 1);
                if (xtl + nb < (int)sizeof(xtag)) {
                    memcpy(xtag + xtl, buf, (size_t)nb);
                    xtl += nb;
                }
                show = 0;
...
int main(int argc, char **argv) {
    ...
    if (!serving && (argc < 2 || !strcmp(argv[1], "--help") || !strcmp(argv[1], "-h") || !strcmp(argv[1], "help"))) {
        k3_usage(argv[0]);
        return argc < 2 ? 1 : 0; /* asking for help is not a failure */
    }
    const char *snap = serving ? getenv("SNAP") : argv[1], *prompt = NULL, *idstr = NULL, *sysmsg = NULL,
               *wirepath = NULL;
    if (!snap || !*snap) {
        fprintf(stderr, "set SNAP=<Kimi K3 snapshot directory>\n");
        return 1;
    }
    int ngen = 32, chat = 0;
    ...
    if (wirepath) {
        ...
        fseek(wf, 0, SEEK_SET);
        if (wn < 0 || wn > (1 << 24)) {
            fprintf(stderr, "wire payload too large\n");
            fclose(wf);
            return 1;
        }
        char *wire = malloc((size_t)wn + 1);
        int *wid = malloc(65536 * sizeof(int));
        if (!wire || !wid) {
            fprintf(stderr, "OOM wire test\n");
            return 1;
        }
        if (fread(wire, 1, (size_t)wn, wf) != (size_t)wn) {
            fprintf(stderr, "short wire read\n");
            return 1;
        }
        fclose(wf);
        wire[wn] = 0;
        ...
        if (n < 0) {
            fprintf(stderr, "invalid K3 chat wire payload\n");
            return 1;
        }
    ...
    if (getenv("K3_X0")) { /* injected input rows [T,hidden] f32, bypasses embed */
        ...
        if (fread(g_x0, 4, (size_t)g_x0_n * m.c.hidden, f) != (size_t)g_x0_n * m.c.hidden) {
            fprintf(stderr, "K3_X0 short read\n");
            return 1;
        }
        fclose(f);
    ...
    if (serving) {
        if (!has_tok) {
            fprintf(stderr, "serve mode needs tokenizer.json\n");
            return 1;
        }
        serve_loop(&m, &T);
        return 0;
    ...
    if (idstr) {
        ...
        while (*p && np < 65536) {
            while (*p == ' ' || *p == ',') p++;
            if (!*p) break;
            ids[np++] = (int)strtol(p, (char **)&p, 10);
        }
    } else if (chat) {
        if (!has_tok) {
            fprintf(stderr, "--chat needs tokenizer.json\n");
            return 1;
        }
        if (!prompt) {
            fprintf(stderr, "--chat needs a user message\n");
            return 1;
        }
        if (m.c.bos >= 0) ids[np++] = m.c.bos;
        ...
        if (!has_tok) {
            fprintf(stderr, "no tokenizer.json in the snapshot — pass --ids instead\n\n");
            k3_usage(argv[0]);
            return 1;
        }
        if (m.c.bos >= 0) ids[np++] = m.c.bos;
        ...
        fprintf(stderr, "no prompt and no --ids: nothing to generate from\n\n");
        k3_usage(argv[0]);
        return 1;
    }
    ...
    for (int s = 0; s < ngen; s++) {
        ...
        if (chat && sp[0] >= 0) {
            if (t == sp[0] || t == sp[1]) {
                ...
                if (has_tok) {
                    int n2 = tok_decode(&T, &t, 1, buf, sizeof(buf) - 1);
                    if (xtl + n2 < (int)sizeof(xtag)) {
                        memcpy(xtag + xtl, buf, n2);
                        xtl += n2;
                    }
                }
                show = 0;
        ...
        fprintf(stderr, "  [tok %d: %.1fs/tok, hit %.0f%%, %.1f GB read]\n", ntok, el / ntok,
                100.0 * m.hits / (m.hits + m.miss + 1e-9), m.ebytes / 1e9);
    }
    if (lo) free(lo);
    double dt = now_s() - tg;
    fprintf(stderr,
            "\n[K3] decode %d tokens in %.1fs (%.2f tok/s) | expert hit %.1f%% (%llu/%llu) | %.1f GB streamed\n", ntok,
            dt, ntok / dt, 100.0 * m.hits / (m.hits + m.miss + 1e-9), (unsigned long long)m.hits,
            (unsigned long long)(m.hits + m.miss), m.ebytes / 1e9);
    fprintf(stderr, "[K3] time: attn %.1fs moe %.1fs (eload %.1fs) head %.1fs | RSS %.1f GB\n", m.t_attn, m.t_moe,
            m.t_eload, m.t_head, rss_gb());
    if (m.trace) fclose(m.trace);
    {
        const char *sv = getenv("USAGE_SAVE");
        if (!(sv && atoi(sv) == 0) && g_k3_usage[0]) rt_save(g_k3_usage, 0);
    } /* same bytes as every other engine */
    return 0;
}
