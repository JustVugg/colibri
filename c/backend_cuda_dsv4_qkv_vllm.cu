...
namespace vllm {
template <typename T> struct _typeConvert;
template <> struct _typeConvert<__nv_bfloat16> {
  using packed_hip_type = __nv_bfloat162;
  __device__ static float2 convert(__nv_bfloat162 v) { return __bfloat1622float2(v); }
  __device__ static __nv_bfloat162 convert(float2 v) { return __floats2bfloat162_rn(v.x, v.y); }
  __device__ static __nv_bfloat16 convert(float v) { return __float2bfloat16(v); }
  __device__ static float convert(__nv_bfloat16 v) { return __bfloat162float(v); }
};
}
