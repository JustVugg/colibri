...
class _TerminationSignal(BaseException):
    """A terminal signal captured for cooperative lifecycle cleanup."""
...
def _normalized_runtime_knobs(
    ...
    *,
    ...
    """Validate the managed runtime-knob vocabulary for one target.

    This is pure input validation used by the core ``start`` lifecycle path,
    so it lives in the headless common layer rather than the optional
    benchmark bundle.  ``node_core_count`` is supplied by the caller (the
    facade) so the helper stays free of planning coupling.
    """
    result = {}
