"""Dependency-free data contracts for the RAM-disk control plane.

These immutable projections convert lifecycle dictionaries into small values
that the headless CLI and presentation layer can share without reimplementing
placement, health, or action rules.  The module deliberately performs no I/O
and owns no lifecycle state;
``ramdisk.py`` remains the authority for planning, confirmation tokens, and
all mutations.

Importing this module requires only the Python standard library.
"""
