...
static const int *g_pre_idx;
static const float *g_pre_w;
static const int *g_pre_keff;
#ifdef __APPLE__
...
typedef struct {
    ...
    uint8_t *q4;
    float *s;
    int O, I, gs; /* gs=group size (0=per-row, 128=grouped) */
#ifdef COLI_CUDA
    ColiCudaTensor *cuda;
#endif
#ifdef COLI_VULKAN
    ColiVkTensor *vk;
    int vk_eligible; /* resident on the Vulkan expert tier */
#endif
...
typedef struct {
    ...
    QT q_a, q_b, kv_a, kv_b, o;
    float *q_a_ln, *kv_a_ln;
#ifdef COLI_CUDA
...
typedef struct {
    ...
    float **Lc, **Rc;
    int max_t;     /* alias della KVState attiva */
    int *kv_start; /* prima pos valida nella KV del layer (MTP: parziale) */
    KVState *kv;
    ESlot **ecache;
    int *ecn;
    int ecap; /* LRU expert per-layer */
    float **kv_dev_L, **kv_dev_R;
    int *kv_dev_valid; /* ombra KV su device (decode) */
    ...
    ESlot **pin;
    int *npin;                       /* HOT-STORE: expert pinnati in RAM (mai evicted) */
    ...
    int *dsa_sel, *dsa_nsel;
    int dsa_scap; /* selezione per posizione del batch corrente */
    ...
    int **eroute;
    int *enr; /* metodo C: routing dell'ULTIMO token per layer */
    ...
    uint64_t gpu_expert_calls;
    int gpu_expert_count;
    int64_t gpu_expert_bytes;
    ...
    double route_kl_sum;
    uint64_t route_kl_n; /* mean KL(true||chosen) on gate mass */
    ...
    uint64_t cpu_expert_rows;
    int64_t cpu_expert_bytes;
...
static void matmul_i4_grouped_pair(float *yg, float *yu, const float *x, const uint8_t *qg, const float *sg,
                                   const uint8_t *qu, const float *su, int S, int I, int O, int gs);

static void expert_gate_up(float *g, float *u, const float *x, QT *wg, QT *wu, int S) {
    if (!g_no_fused_pair && !spec_pinned() && S == 1 && wg->fmt == 2 && wu->fmt == 2 && wg->I == wu->I &&
        wg->O == wu->O)
        matmul_i4_pair(g, u, x, wg->q4, wg->s, wu->q4, wu->s, wg->I, wg->O);
    else if (!g_no_fused_pair && S == 1 && wg->fmt == 4 && wu->fmt == 4 && wg->I == wu->I && wg->O == wu->O &&
             wg->gs == wu->gs)
...
#define VK_FMT_OK(t)                                                                                                   \
    ((t)->fmt == 1 || (t)->fmt == 2 || (t)->fmt == 5 || ((t)->fmt == 4 && (t)->gs >= 8 && (t)->gs % 8 == 0))
...
static int vk_matmul_pair_qt(QT *a, float *ya, QT *b, float *yb, const float *x, int S) {
    ...
    return coli_vk_matmul_pair(&a->vk, ya, wa, a->s, a->O, &b->vk, yb, wb, b->s, b->O, a->fmt, x, S, a->I, a->gs);
}
#endif
#ifdef COLI_CUDA
static int qt_cuda_upload(QT *t) {
    ...
    if (t->fmt == 8)
        return 0; /* fp8-e4m3 passthrough: no CUDA kernel yet either (matches the
    ...
    const void *weights = t->fmt == 0 ? (const void *)t->qf : t->fmt == 1 ? (const void *)t->q8 : (const void *)t->q4;
...
static void cuda_stats_print(void) {
    ...
    if (calls)
        fprintf(stderr,
                "[CUDA] expert groups: %llu call, %llu expert, %llu righe "
                "(%.2f expert/call)%s\n",
                (unsigned long long)calls, (unsigned long long)experts, (unsigned long long)rows,
                (double)experts / calls, getenv("COLI_CUDA_PROFILE") ? "; timing sotto" : "");
    if (calls && getenv("COLI_CUDA_PROFILE"))
        fprintf(stderr, "[CUDA] expert groups timing: H2D %.1f ms | kernel %.1f ms | D2H %.1f ms\n", h2d, kernel, d2h);
    if (g_ovl_issue + g_ovl_cpu + g_ovl_take > 0)
        fprintf(stderr, "[CUDA] overlap window: pack+issue %.2fs | cpu-rows %.2fs | take(sync+acc) %.2fs\n",
...
static void dc_wall_read(int64_t out[2], int64_t *all) { /* mutex-consistent snapshot for the report */
    if (!dc_needed()) {
        ...
        *all = 0;
        return;
    } /* off for the whole process => accumulators are
    ...
    pthread_mutex_lock(&g_dc_wall_mx);
    out[0] = g_dc_wall_ns[0];
    out[1] = g_dc_wall_ns[1];
    *all = g_dc_wall_all_ns;
    pthread_mutex_unlock(&g_dc_wall_mx);
}
...
typedef struct {
    ...
    int64_t io, cpu_bytes;
    uint64_t hits, miss, ereq, n_fw, n_emit, nlat, n_p2p, cpu_rows;
    uint64_t hit_pin, hit_ecache;
    uint64_t dc_n[2], dc_direct_n[2];
    int64_t dc_bytes[2], dc_ns[2];         /* DISK-CLASS */
...
static void prof_base(Model *m, ProfBase *b) {
    ...
    b->ereq = m->ereq;
    b->hit_pin = m->hit_pin;
    b->hit_ecache = m->hit_ecache;
    b->n_fw = m->n_fw;
    b->n_emit = m->n_emit;
    b->nlat = g_prof_nlat;
    b->n_p2p = m->n_p2p;
    b->cpu_bytes = m->cpu_expert_bytes;
    b->cpu_rows = m->cpu_expert_rows;
    for (int i = 0; i < 2; i++) {
...
static float *falloc(int64_t n) {
    ...
    if (n < 0 || (uint64_t)n > SIZE_MAX / sizeof(float)) {
        fprintf(stderr, "falloc: n=%lld is out of range\n", (long long)n);
        exit(1);
    }
...
#pragma omp parallel for schedule(static)
    for (int o = 0; o < O; o++) {
        ...
        const uint8_t *wu2 = qu + (int64_t)o * rb;
        const float *sgl = sg + (int64_t)o * ng;
        const float *sul = su + (int64_t)o * ng;
        for (int s = 0; s < S; s++) {
            ...
            for (int g = 0; g * gs < I; g++) {
                ...
                for (; i + 16 <= base + glen; i += 16) {
                    ...
                    __m256 w1g =
                        _mm256_cvtepi32_ps(_mm256_sub_epi32(_mm256_cvtepu8_epi32(_mm_srli_si128(nibg, 8)), b8));
                    ...
                    __m256 w1u =
                        _mm256_cvtepi32_ps(_mm256_sub_epi32(_mm256_cvtepu8_epi32(_mm_srli_si128(nibu, 8)), b8));
...
static void matmul_qt_ex(float *y, const float *x, QT *w, int S, int allow_idot) {
    ...
    if (g_metal_enabled && S >= g_metal_gemm_min && !spec_pinned() && (w->fmt == 1 || w->fmt == 2 || w->fmt == 4) &&
        !omp_in_parallel()) {
...
#endif
    if (w->fmt == 0) {
        matmul(y, x, w->qf, S, w->I, w->O);
        return;
    }
    if (w->fmt == 4) {
        matmul_i4_grouped(y, x, w->q4, w->s, S, w->I, w->O, w->gs);
        return;
    }
    if (w->fmt == 6) {
        matmul_e8(y, x, w->q4, NULL, S, w->I, w->O);
        return;
    } /* scales live in-block */
...
static int la_pred[3][130][16];
static signed char la_val[3][130];
...
static void numa_slab_bind(void *p, size_t n) {
    ...
    syscall(SYS_mbind, a, len, 3 /*MPOL_INTERLEAVE*/, &mask, (unsigned long)(g_numa_nodes + 1), 0);
#else
    (void)p;
    (void)n;
#endif
...
static void numa_init(void) {
    ...
    for (int i = 0; i < 64; i++) {
        char pth[64];
        snprintf(pth, sizeof(pth), "/sys/devices/system/node/node%d", i);
        struct stat st;
        if (stat(pth, &st)) break;
        g_numa_nodes = i + 1;
    }
    if (g_numa_nodes < 2) {
        fprintf(stderr, "[NUMA] single node: COLI_NUMA ignored\n");
        return;
    }
     ...
     * errno==EPERM disables — any other failure keeps NUMA on. */
    {
        ...
        if (!posix_memalign(&pg, 4096, 4096)) {
            unsigned long mask = (1UL << g_numa_nodes) - 1;
            errno = 0;
            long rc = syscall(SYS_mbind, pg, 4096, 3 /*MPOL_INTERLEAVE*/, &mask, (unsigned long)(g_numa_nodes + 1), 0);
...
static void *qalloc(size_t n) {
    ...
    if (g_metal_enabled) {
        void *p;
        size_t r = (n + 16383) & ~(size_t)16383;
        if (posix_memalign(&p, 16384, r)) {
            fprintf(stderr, "OOM qalloc\n");
            exit(1);
        }
        coli_metal_register(p, r);
        return p;
    }
#endif
    void *p = malloc(n);
...
static int g_pilot_nw =
    1; /* PILOT_WORKERS: blocking-path pilot threads (SPMC ring). 1 = today's behaviour (byte-identical). Only the
          non-URING PILOT_REAL path fans out; URING already batches to QD>1. */
static int g_pilot_evict_guard =
    1;                                 /* PILOT_EVICT_GUARD=0 -> old plain-LRU eviction (A/B). Default ON:
...
static void qt_alloc(QT *t, int O, int I, int bits) {
    ...
    } else if (bits >= 5 || g_nopack) {
        t->fmt = 1;
        t->q8 = qalloc((int64_t)O * I);
        t->s = qsalloc(O);
    } else if (bits >= 4) {
        t->fmt = 2;
        t->q4 = qalloc((int64_t)O * ((I + 1) / 2));
        t->s = qsalloc(O);
    } else if (bits == 3) {
        t->fmt = 5;
        t->q4 = qalloc((int64_t)O * i3_rowbytes(I));
        t->s = (float *)qalloc((size_t)O * i3_groups(I) * sizeof(float));
    } else {
        t->fmt = 3;
        t->q4 = qalloc((int64_t)O * ((I + 3) / 4));
        t->s = qsalloc(O);
    }
}
...
static void rope_interleave(float *v, int pos, const Cfg *c) {
    ...
    if (c->qk_rope > 256) {
        fprintf(stderr, "qk_rope=%d exceeds rope_interleave buffer (256)\n", c->qk_rope);
        exit(1);
    }
    typedef struct {
        int pos, qk, valid;
        float theta, cs[128], sn[128];
    } RopeCache; /* (#80) */
...
static char *cfg_slurp(const char *path) {
    ...
    long n = ftell(f);
    fseek(f, 0, SEEK_SET);
    if (n < 0 || (long long)n > CFG_MAX_BYTES) {
        fclose(f);
        return NULL;
    }
...
static jval *cfg_root(const char *snap, char **arena) {
    ...
    if (n < 0 || n > (256L << 20)) {
        fprintf(stderr, "%s: size %ld out of range (0..256 MB)\n", p, n);
        exit(1);
    }
    char *b = malloc((size_t)n + 1);
    if (!b) {
...
static void load_cfg(Cfg *c, const char *snap) {
    ...
    c->dense_inter = gi(r, "intermediate_size");
    c->first_dense = gi(r, "first_k_dense_replace");
    c->q_lora = gi(r, "q_lora_rank");
    c->kv_lora = gi(r, "kv_lora_rank");
    c->qk_nope = gi(r, "qk_nope_head_dim");
    c->qk_rope = gi(r, "qk_rope_head_dim");
    c->v_head = gi(r, "v_head_dim");
    c->n_shared = gi(r, "n_shared_experts");
    c->vocab = gi(r, "vocab_size");
    c->n_group = gi(r, "n_group");
    c->topk_group = gi(r, "topk_group");
    jval *nt = json_get(r, "norm_topk_prob");
    c->norm_topk = (nt && nt->t == J_BOOL) ? nt->boolean : 0;
    jval *ep = json_get(r, "rms_norm_eps");
    c->eps = ep ? (float)ep->num : 1e-5f;
    jval *rs = json_get(r, "routed_scaling_factor");
    c->routed_scale = rs ? (float)rs->num : 1.f;
    ...
    if (eo) {
        if (eo->t == J_NUM) c->stop_ids[c->n_stop++] = (int)eo->num;
            ...
            for (int i = 0; i < eo->len && c->n_stop < 8; i++) c->stop_ids[c->n_stop++] = (int)eo->kids[i]->num;
    }
    ...
    {
        char gp[2100];
        snprintf(gp, sizeof(gp), "%s/generation_config.json", snap);
        ...
        if (gf) {
            ...
            if (gb) {
                size_t gg = fread(gb, 1, (size_t)gn, gf);
                gb[gg] = 0;
                char *ga = NULL;
                jval *gr = json_parse(gb, &ga);
                jval *ge = gr ? json_get(gr, "eos_token_id") : NULL;
                if (ge) {
                    int add[8], na = 0;
                    if (ge->t == J_NUM) add[na++] = (int)ge->num;
                    else if (ge->t == J_ARR)
                        for (int i = 0; i < ge->len && na < 8; i++) add[na++] = (int)ge->kids[i]->num;
                    for (int i = 0; i < na && c->n_stop < 8; i++) {
                        int dup = 0;
                        for (int j = 0; j < c->n_stop; j++)
                            if (c->stop_ids[j] == add[i]) dup = 1;
    ...
    c->index_topk = gi(r, "index_topk");
    c->index_nh = gi(r, "index_n_heads");
    c->index_hd = gi(r, "index_head_dim");
    {
        jval *it = json_get(r, "indexer_types");
        int freq = gi(r, "index_topk_freq");
        if (freq < 1) freq = 1;
        jval *of = json_get(r, "index_skip_topk_offset");
        int off = of ? (int)of->num : 2;
    ...
    if (c->n_group != 1) {
        fprintf(stderr, "this engine requires n_group=1 (GLM-5.2)\n");
        exit(1);
    }
...
#define CKR(name, v, lo, hi)                                                                                           \
    if ((v) < (lo) || (v) > (hi)) {                                                                                    \
        fprintf(stderr, "config: %s=%d is outside [%d,%d]\n", name, (int)(v), (int)(lo), (int)(hi));                   \
        exit(1);                                                                                                       \
    }
    /* CKR expands to a braced `if` with NO trailing semicolon, so clang-format reads each
     * CKR as the body of the previous one and indents this block one level deeper on EVERY
     * run -- it never reaches a fixed point, and `make lint-c` would fail on a tree that was
     * just formatted. Kept as the two-column table it was written as, which is also how it
     * stays readable. */
    /* clang-format off */
...
static const struct {
    ...
    {"f32", 0},          {"int8-row", 1}, {"int4-row", 2},       {"int2-row", 3},
    {"int4-grouped", 4}, {"int3-g64", 5}, {"e8-iq3-lattice", 6}, {"fp8-e4m3-b128", 8},
};
#define N_FMT_NAMES (int)(sizeof(FMT_NAMES) / sizeof(FMT_NAMES[0]))

static int qt_fmt_by_name(const char *name) {
    for (int i = 0; i < N_FMT_NAMES; i++)
        if (!strcmp(FMT_NAMES[i].name, name)) return FMT_NAMES[i].fmt;
    return -1; /* unrecognized name -- never a valid qt_resolve_fmt() return value */
}
static const char *qt_name_by_fmt(int fmt) {
    for (int i = 0; i < N_FMT_NAMES; i++)
        if (FMT_NAMES[i].fmt == fmt) return FMT_NAMES[i].name;
...
static int qt_resolve_fmt(const char *name, int O, int I, int64_t nb, int64_t ns, int *gs, const char *stamped_name) {
    ...
    if (ns == 4 && nb == (int64_t)O * e8_rowbytes(I)) {
        ...
        if (fp8_blk_f32_also || fp8_blk_ue8m0_also || i8_row_also) {
            ...
            fprintf(stderr,
                    "%s: [%d,%d] byte layout (nb=%lld ns=%lld) matches E8/IQ3 "
                    ...
                    fp8_blk_ue8m0_also
                        ? " AND per-128x128-block FP8 ue8m0 scales (fmt=8, 4 blocks, recognized-not-implemented)"
                        : "",
                    i8_row_also ? " AND plain int8 per-row (fmt=1, O=1)" : "",
                    stamped_name
                        ? " -- metadata stamp present but names a format/encoding that doesn't resolve the ambiguity"
                        : "");
            exit(1);
        }
        *gs = 0;
    ...
    if (!fmt) {
        fprintf(stderr,
                "%s: quantized weight is %lld bytes — no int8/int4/int2/int3-g64/fp8 layout for [%d,%d], refusing "
                "(untrusted container)\n",
    ...
    *gs = 0;
    if (fmt == 2) {
        int g = detect_group_size(O, I, ns);
        if (g > 0) {
            fmt = 4;
            *gs = g;
        }
    ...
    if (fmt == 1) {
        ...
        if (is_row && is_blk) {
            if (sf == 1 || sf == 8) {
                ...
                fprintf(stderr,
                        "%s: [%d,%d] scale array is %lld bytes — matches BOTH per-row "
            ...
            if (sf == 1 && is_row) {
                ...
                fprintf(stderr,
                        "%s: [%d,%d] fp8-e4m3-b128 with ue8m0 scales recognized but not "
    ...
    if (ns != exp_scale * 4) {
        fprintf(stderr,
                "%s: scale array is %lld bytes — expected %lld for [%d,%d] fmt=%d, refusing (untrusted container)\n",
    ...
    if (st_has(&m->S, sn)) {
        ...
        if (fmt == 1) {
            ...
            if (t->fmt != 4 || !t->q4) {
                ...
                t->gs = gs;
                t->q4 = qalloc(nb);
                t->s = (float *)qalloc((size_t)O * (size_t)ng * sizeof(float));
            }
            st_read_raw(&m->S, name, t->q4, drop);
        } else if (fmt == 5) {
            int64_t ng = i3_groups(I); /* int3-g64: 24B/group weights + O*ng group scales */
            if (t->fmt != 5 || !t->q4) {
                t->fmt = 5;
                t->O = O;
                t->I = I;
            ...
            int64_t nblk =
                fp8_nblk(O) * fp8_nblk(I); /* fp8-e4m3: raw bytes (q8, like fmt=1)
                                            * + one f32 scale per 128x128 block. BOTH weights and scale via qalloc, not
                                            * falloc, from day one -- the GPU-visibility lesson fmt=4 and fmt=5 each had
                                            * to learn separately (see review round 1 audit history in the report). */
            if (t->fmt != 8 || !t->q8) {
                t->fmt = 8;
                t->O = O;
                t->I = I;
                t->gs = 0;
                t->q8 = qalloc(nb);
                t->s = (float *)qalloc((size_t)nblk * sizeof(float));
            }
            st_read_raw(&m->S, name, t->q8, drop);
        } else {
            if (t->fmt != fmt || !t->q4) {
        ...
        if (t->fmt == 0) st_read_f32_cap(&m->S, name, t->qf, (int64_t)O * I, drop);
            ...
            st_read_f32_cap(&m->S, name, tmp, (int64_t)O * I, drop);
            qt_fill(t, tmp, bits);
            free(tmp);
        }
    }
...
static QT qt_load(Model *m, const char *name, int O, int I, int bits) {
    ...
    if (g_cuda_enabled && g_cuda_dense) {
        t.cuda_eligible = 1;
        int slot = g_cuda_rr++ % g_cuda_ndev;
...
static float *ld(Model *m, const char *name) { /* tensore 1D f32 residente (norme/bias) */
    ...
    if (n < 0) st_die_missing(&m->S, name);
...
static void qt_cuda_colocate(QT *dst, const QT *src) {
    ...
    int old = -1, now = -1;
    for (int i = 0; i < g_cuda_ndev; i++) {
        if (g_cuda_devices[i] == dst->cuda_device) old = i;
        if (g_cuda_devices[i] == src->cuda_device) now = i;
    }
...
static void layer_cuda_shard_kvb(Layer *l, int H, int Q, int V) {
    ...
    int rb = l->kv_b.fmt == 1                         ? l->kv_b.I
             : (l->kv_b.fmt == 2 || l->kv_b.fmt == 4) ? (l->kv_b.I + 1) / 2
                                                      : (l->kv_b.I + 3) / 4;
    ...
    for (int d = 0, h0 = 0; d < g_cuda_ndev; d++) {
        ...
        const float *scale =
            l->kv_b.s + (int64_t)h0 * (Q + V) * (l->kv_b.gs > 0 ? (l->kv_b.I + l->kv_b.gs - 1) / l->kv_b.gs : 1);
        if (!coli_cuda_tensor_upload_g(&l->kv_b_shard[d], part, scale, l->kv_b.fmt, l->kv_b.I, rows, g_cuda_devices[d],
                                       l->kv_b.gs))
            return;
        l->shard_h0[d] = h0;
        l->shard_hn[d] = hn;
    ...
    int old = -1;
    for (int i = 0; i < g_cuda_ndev; i++)
        if (g_cuda_devices[i] == l->kv_b.cuda_device) old = i;
...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits) {
    ...
    m->ecap = cap;
    m->ecache = calloc(NR, sizeof(ESlot *));
    m->ecn = calloc(NR, sizeof(int));
    m->kv_dev_L = calloc(NR, sizeof(float *));
    m->kv_dev_R = calloc(NR, sizeof(float *));
    m->kv_dev_valid = calloc(NR, sizeof(int));
#ifdef COLI_VULKAN
    m->vk_kv_valid = calloc(NR, sizeof(int));
#endif
    m->eroute = calloc(NR, sizeof(int *));
    m->enr = calloc(NR, sizeof(int));
    m->pin = calloc(NR, sizeof(ESlot *));
    m->npin = calloc(NR, sizeof(int));
...
#define PM(s) (snprintf(nm, sizeof(nm), "model.layers.%d." s, i), nm)
            ...
            m->enorm = ld(m, PM("enorm.weight"));
            m->hnorm = ld(m, PM("hnorm.weight"));
...
#undef PM
     ...
     * TUTTI i layer full. Auto-rilevamento come per MTP: niente flag, niente passi extra. */
    {
        ...
        if (m->has_dsa) {
            m->ix_wq = calloc(c->n_layers, sizeof(QT));
            m->ix_wk = calloc(c->n_layers, sizeof(QT));
            m->ix_wp = calloc(c->n_layers, sizeof(QT));
            m->ix_knw = calloc(c->n_layers, sizeof(float *));
            m->ix_knb = calloc(c->n_layers, sizeof(float *));
            for (int i = 0; i < c->n_layers; i++) {
                if (!c->idx_type[i]) continue;
#define PI(s) (snprintf(nm, sizeof(nm), "model.layers.%d.self_attn.indexer." s, i), nm)
                ...
                m->ix_knw[i] = ld(m, PI("k_norm.weight"));
                m->ix_knb[i] = ld(m, PI("k_norm.bias"));
#undef PI
            }
    ...
    if (m->has_dsa)
        for (int i = 0; i < c->n_layers; i++)
            if (c->idx_type[i]) rb += qt_bytes(&m->ix_wq[i]) + qt_bytes(&m->ix_wk[i]) + qt_bytes(&m->ix_wp[i]);
...
static void embed_row(Model *m, int tok, float *x) {
    ...
    if (tok < 0 || tok >= e->O) {
        ...
        return;
    } /* #SEC-5: out-of-range token id -> zero row, never OOB */
    if (e->fmt == 0) {
    ...
    if (e->fmt == 4) { /* grouped int4: per-group scale (embed/lm_head at io_bits, usually fmt 0/1) */
        ...
        for (int g = 0; g * gs < D; g++) {
            int base = g * gs, glen = gs;
            if (base + glen > D) glen = D - base;
            float s = scl[g];
            for (int i = base; i + 1 < base + glen; i += 2) {
                uint8_t byte = q[i >> 1];
                x[i] = (float)((int)(byte & 0xF) - 8) * s;
                x[i + 1] = (float)((int)(byte >> 4) - 8) * s;
            }
            if (glen & 1) {
                uint8_t byte = q[(base + glen - 1) >> 1];
                x[base + glen - 1] = (float)((int)(byte & 0xF) - 8) * s;
            }
        }
        return;
    }
    ...
    if (e->fmt == 5) {
        const uint8_t *q = e->q4 + (int64_t)tok * i3_rowbytes(D); /* int3-g64 */
        const float *sr = e->s + (int64_t)tok * i3_groups(D);
        int64_t ng = i3_groups(D);
        for (int64_t g = 0; g < ng; g++) {
            const uint8_t *lo = q + g * I3_GBYTES, *hi = lo + 16;
    ...
    const uint8_t *q = e->q4 + (int64_t)tok * ((D + 3) / 4);
    float s = e->s[tok]; /* int2 */
    for (int i = 0; i < D; i++) {
        uint8_t byte = q[i >> 2];
        int sh = (i & 3) * 2;
        x[i] = (float)((int)((byte >> sh) & 3) - 2) * s;
    }
}
...
static int64_t g_mmap_wired = 0;
static long g_mmap_wire_failed = 0;
static void *map_of_fd(int fd) {
    pthread_mutex_lock(&g_map_mtx);
    for (int i = 0; i < g_nmaps; i++)
        if (g_maps[i].fd == fd) {
            void *b = g_maps[i].base;
    ...
    if (g_nmaps < 512 && fstat(fd, &st) == 0) {
        ...
        if (p != MAP_FAILED) {
            base = p;
            g_maps[g_nmaps].fd = fd;
            g_maps[g_nmaps].base = p;
...
static int64_t mir_pread_striped(shards *S, int fd, int rep, char *buf, int64_t len, int64_t base) {
    ...
    MirStripe st[MIR_REPS];
    pthread_t th[MIR_REPS];
    int nth = 0, ns = 0;
    for (int i = 0; i < nsf; i++) {
        int64_t o = (int64_t)i * chunk;
        if (o >= len) break;
...
static ssize_t mir_pread(shards *S, int fd, int rep, void *buf, int64_t n, int64_t off, const char *tag) {
    ...
    if (rc && used) {
        ...
        used = 0;
        rc = pread_full(fd, buf, n, off, tag);
    }
    if (!rc) {
    ...
    while (got < n) {
        ...
        if (r < 0) {
            ...
            fprintf(stderr, "%s: %s (off %lld, %lld/%lld bytes, WinErr=%lu)\n", tag, strerror(errno), (long long)off,
                    (long long)got, (long long)n, (unsigned long)compat_pread_lasterr);
#else
        ...
        if (r == 0) {
            fprintf(stderr, "%s: short read at EOF (off %lld, %lld/%lld bytes) — truncated shard?\n", tag,
...
static int expert_load_impl(Model *m, int layer, int eid, ESlot *s, int fatal, int demand) {
    ...
    if (!st_has(&m->S, qn)) { /* fallback: tensori pieni, quantizza a runtime.
        ...
        atomic_fetch_add_explicit(&g_prof_io,
                                  st_nbytes(&m->S, nm[0]) + st_nbytes(&m->S, nm[1]) + st_nbytes(&m->S, nm[2]),
    ...
    for (int k = 0; k < 3; k++) {
        ...
        tq[k] = st_find(&m->S, qn);
        if (!tw[k] || !tq[k]) {
            if (fatal) st_die_missing(&m->S, nm[k]); /* #586: diagnose, don't just name it */
    ...
    if (g_disk_split) { /* split load/byte per tipo layer; atomici: expert_load gira anche su OMP/pipe/pilot */
        int64_t tb = 0;
        for (int k = 0; k < 3; k++) tb += tw[k]->nbytes + tq[k]->nbytes;
        if (layer == c->n_layers) {
            __atomic_add_fetch(&m->ld_mtp, 1, __ATOMIC_RELAXED);
            __atomic_add_fetch(&m->bytes_mtp, (uint64_t)tb, __ATOMIC_RELAXED);
        } else {
            __atomic_add_fetch(&m->ld_main, 1, __ATOMIC_RELAXED);
    ...
    if (g_mmap) {
        ...
        int okm = 1;
        for (int k = 0; k < 3; k++) {
            bw[k] = map_of_fd(rep_bfd(&m->S, tw[k]->fd, rep));
            bq[k] = map_of_fd(rep_bfd(&m->S, tq[k]->fd, rep));
        ...
        if (okm) {
            ...
            for (int k = 0; k < 3; k++) {
                ...
                int fmt = qt_resolve_fmt(tw[k]->name, OO[k], II[k], nb, tq[k]->nbytes, &gs,
                                         NULL); /* routed expert: never stamped */
                qt[k]->fmt = fmt;
                qt[k]->O = OO[k];
                qt[k]->I = II[k];
                qt[k]->gs = gs;
                qt[k]->qf = NULL;
                qt[k]->q8 = (int8_t *)((char *)bw[k] + tw[k]->off);
                qt[k]->q4 = (uint8_t *)((char *)bw[k] + tw[k]->off);
            ...
            for (int k = 0; k < 3; k++) {
                char *p = (char *)bw[k] + tw[k]->off;
                size_t n = (size_t)tw[k]->nbytes;
#if defined(__APPLE__) || defined(__linux__) || defined(__FreeBSD__)
                ...
                acc += p[n - 1];
                (void)acc;
                char *q = (char *)bq[k] + tq[k]->off;
                size_t nq = (size_t)tq[k]->nbytes;
...
#else
        compat_aligned_free(s->slab);
        if (posix_memalign((void **)&s->slab, 4096, wtot + 8192)) {
            fprintf(stderr, "OOM slab\n");
            if (fatal) exit(1);
            s->slab = NULL;
            s->slab_cap = 0;
    ...
    if (!s->fslab || ftot > s->fslab_cap) {
        ...
        if (ftot < 0 || (uint64_t)ftot > SIZE_MAX / sizeof(float) || posix_memalign((void **)&s->fslab, 16384, fb)) {
            ...
            compat_aligned_free(s->slab);
            s->slab = NULL;
            s->slab_cap = 0; /* clean, hidden slot (eid stays -1) */
            s->fslab = NULL;
            s->fslab_cap = 0;
            return -1;
        }
        ...
        if (fatal) {
            s->fslab = falloc(ftot);
        } /* main path: byte-identical exit-on-OOM */
            ...
            if (ftot < 0 || (uint64_t)ftot > SIZE_MAX / sizeof(float) ||
                ...
                compat_aligned_free(s->slab);
                s->slab = NULL;
                s->slab_cap = 0; /* leave a clean, hidden slot (eid stays -1) */
                s->fslab = NULL;
                s->fslab_cap = 0;
                return -1;
            }
    ...
    if (contig) {
        ...
        if (dfd >= 0) { /* O_DIRECT: offset/len allineati a 4K */
            ...
            if (r >= need) {
                pos[ord[0]] = off0 - base;
                pos[ord[1]] = pos[ord[0]] + tw[ord[0]]->nbytes;
                pos[ord[2]] = pos[ord[1]] + tw[ord[1]]->nbytes;
                done = 1;
                dc_direct = 1;
            }
        }
        if (!done) { /* fallback bufferizzato */
            if (mir_pread(&m->S, tw[ord[0]]->fd, rep, s->slab, wtot, off0, "pread expert")) {
                if (fatal) exit(1);
    ...
    if (!done) { /* non contigui: 3 pread bufferizzate */
        ...
        for (int a = 0; a < 3; a++) {
            int k = ord[a];
            if (mir_pread(&m->S, tw[k]->fd, rep, s->slab + o, tw[k]->nbytes, tw[k]->off, "pread expert")) {
                if (fatal) exit(1);
                if (dc_on) dc_wall_exit(dc_cls, now_s()); /* pair the enter on the non-fatal unwind */
                return -1;
            }
    ...
    float *fp[3];
    int64_t fo = 0; /* scale (piccole) */
    for (int k = 0; k < 3; k++) {
        if (mir_pread(&m->S, tq[k]->fd, rep, (char *)(s->fslab + fo), tq[k]->nbytes, tq[k]->off, "pread qs")) {
            if (fatal) exit(1);
            if (dc_on) dc_wall_exit(dc_cls, now_s()); /* pair the enter on the non-fatal unwind */
            return -1;
        }
    ...
    if (g_drop) { /* scarta subito le pagine: evita che la page
        ...
        for (int k = 0; k < 3; k++)
            posix_fadvise(rep_bfd(&m->S, tq[k]->fd, rep), tq[k]->off, tq[k]->nbytes, POSIX_FADV_DONTNEED);
    }
    QT *qt[3] = {&s->g, &s->u, &s->d};
    int OO[3] = {I, I, D}, II[3] = {D, D, I};
    for (int k = 0; k < 3; k++) {
        ...
        int gs = 0;
        int fmt =
            qt_resolve_fmt(tw[k]->name, OO[k], II[k], nb, tq[k]->nbytes, &gs, NULL); /* routed expert: never stamped */
        qt[k]->fmt = fmt;
        qt[k]->O = OO[k];
        qt[k]->I = II[k];
        qt[k]->gs = gs;
...
static int uring_add_read(UringBatch *b, int li, int fd, void *buf, size_t len, int64_t off, size_t expect) {
    if (b->nreq >= URING_REQ_MAX || expect > INT_MAX) {
        errno = E2BIG;
        return -1;
    }
    int ri = b->nreq++;
...
static int uring_load_add(UringBatch *b, Model *m, int layer, int eid, ESlot *s, int fatal) {
    ...
    if (!s->slab || wtot + 8192 > s->slab_cap) {
        ...
        if (posix_memalign((void **)&s->slab, 16384, need)) {
            s->slab = NULL;
            s->slab_cap = 0;
            return uring_load_error(l, ENOMEM, "io_uring expert slab"), li;
        }
        s->slab_cap = need;
        if (g_metal_enabled) coli_metal_register(s->slab, need);
#else
    ...
    if (!s->fslab || ftot > s->fslab_cap) {
        ...
        if (posix_memalign((void **)&s->fslab, 16384, fb)) {
            s->fslab = NULL;
            s->fslab_cap = 0;
            return uring_load_error(l, ENOMEM, "io_uring expert scales"), li;
        }
        s->fslab_cap = ftot;
        if (g_metal_enabled) coli_metal_register(s->fslab, fb);
#else
    ...
    for (int a = 0; a < 3; a++)
        for (int z = a + 1; z < 3; z++)
            if (l->tw[ord[z]]->off < l->tw[ord[a]]->off) {
                int t = ord[a];
                ord[a] = ord[z];
                ord[z] = t;
            }
    ...
    if (contig) {
        ...
        if (dfd >= 0) {
            ...
            l->pos[ord[0]] = off0 - base;
            l->pos[ord[1]] = l->pos[ord[0]] + l->tw[ord[0]]->nbytes;
        ...
        for (int a = 0; a < 3; a++) {
            int k = ord[a];
            l->pos[k] = o;
            if (uring_add_read(b, li, l->tw[k]->fd, s->slab + o, (size_t)l->tw[k]->nbytes, l->tw[k]->off,
                               (size_t)l->tw[k]->nbytes))
                return uring_load_error(l, errno, "io_uring expert read"), li;
            o += l->tw[k]->nbytes;
        }
    }
    int64_t fo = 0;
    for (int k = 0; k < 3; k++) {
        if (uring_add_read(b, li, l->tq[k]->fd, s->fslab + fo, (size_t)l->tq[k]->nbytes, l->tq[k]->off,
                           (size_t)l->tq[k]->nbytes))
...
static void uring_reap(UringBatch *b) {
    ...
    while (coli_uring_peek(&b->ring, &cqe)) {
        ...
        UringRead *r = &b->req[cqe.user_data - 1];
        UringLoad *l = &b->load[r->load];
...
static int uring_finalize_load(UringBatch *b, int li, int publish_eid) {
    ...
    if (uring_wait_load(b, li) < 0) {
        errno = l->error;
        if (l->fatal) {
            perror("io_uring expert completion");
            exit(1);
        }
        return -1;
    }
    if (g_drop) {
        int ord0 = 0;
        for (int k = 1; k < 3; k++)
            if (l->tw[k]->off < l->tw[ord0]->off) ord0 = k;
        int64_t wtot = l->tw[0]->nbytes + l->tw[1]->nbytes + l->tw[2]->nbytes;
    ...
    for (int k = 0; k < 3; k++) {
        ...
        qt[k]->gs = gs;
        qt[k]->qf = NULL;
        qt[k]->q8 = (int8_t *)(s->slab + l->pos[k]);
        qt[k]->q4 = s->slab + l->pos[k];
        qt[k]->s = fp[k];
    }
...
typedef struct {
    ...
    pthread_mutex_t mx;
    pthread_cond_t cv;      /* ONLY for parking/waking idle workers */
...
static void *pipe_worker(void *arg) {
    ...
    for (;;) {
        ...
        while ((atomic_load_explicit(&p->cur, memory_order_relaxed) >> 8) == seen) pthread_cond_wait(&p->cv, &p->mx);
        ...
        for (;;) {
            ...
            if (atomic_compare_exchange_weak_explicit(&p->cur, &c, c + 1, memory_order_acq_rel, memory_order_relaxed)) {
                ...
                expert_load(p->m, L, eid, &p->m->ws[i], 1, 1); /* needed-now load: fatal on I/O error (matches serial
                                                                  path); demand=1: this IS moe()'s own miss path */
...
static void pipe_init(Model *m) {
...
#ifdef __linux__
    if (g_uring) {
        if (uring_batch_init(&g_ub_pipe)) {
            perror("URING=1 io_uring_setup");
            exit(1);
        }
        g_pp.m = m;
        g_pp.started = 1;
    ...
    g_pp.m = m;
    g_pp.nw = g_pipe_nw;
    if (g_pp.nw > 16) g_pp.nw = 16;
    if (g_pp.nw < 1) g_pp.nw = 1;
    atomic_store(&g_pp.cur, 0);
    atomic_store(&g_pp.njobs, 0);
    pthread_mutex_init(&g_pp.mx, NULL);
...
static void pipe_dispatch(Model *m, int layer, const int *eids, int njobs) {
    ...
    if (g_uring) {
        ...
        for (int q = 0; q < njobs; q++) {
            ...
            if (li != q) {
                fprintf(stderr, "URING: expert batch overflow\n");
                exit(1);
            }
        }
        if (uring_submit_batch(&g_ub_pipe)) {
            perror("URING: submit");
            exit(1);
        }
        return;
    ...
    for (int q = 0; q < njobs; q++)
        atomic_store_explicit(&g_pp.ready[q], 0, memory_order_relaxed); /* reset BEFORE publish */
    ...
    pthread_cond_broadcast(&g_pp.cv);
...
static inline void pipe_wait(int q) {
#ifdef __linux__
    if (g_uring) {
        if (uring_finalize_load(&g_ub_pipe, q, 1)) {
            perror("URING: expert load");
            exit(1);
        }
        return;
    }
    ...
    if (g_pipe_block) {
        ...
        pthread_mutex_lock(&g_pp.mx);
        while (!atomic_load_explicit(&g_pp.ready[q], memory_order_acquire)) pthread_cond_wait(&g_pp.cv_done, &g_pp.mx);
        pthread_mutex_unlock(&g_pp.mx);
        return;
    }
...
#endif
        s->slab = NULL;
        s->fslab = NULL; /* detach, keep caps/arena ownership */
    } else {
        compat_aligned_free(s->slab);
        free(s->fslab);
        s->slab = NULL;
...
static void expert_host_ensure(Model *m, int layer, ESlot *s) {
    ...
    if (s->aslab) {
        s->slab = s->aslab;
        s->fslab = s->afslab;
    } /* re-attach the arena slice; caps survived release */
...
static void qt_addrow(const QT *t, int row, float coef, float *acc) {
    int I = t->I;
    if (t->fmt == 0) {
        const float *w = t->qf + (int64_t)row * I;
        for (int i = 0; i < I; i++) acc[i] += coef * w[i];
        return;
    }
    ...
    if (t->fmt == 4) {
        ...
        if (I & 1) {
            uint8_t b = w[I >> 1];
            acc[I - 1] += coef * scl[(I - 1) / gs] * ((int)(b & 0xF) - 8);
        }
        return;
    }
    ...
    if (t->fmt == 5) {
        ...
        for (int64_t g = 0; g < ng; g++) {
            const uint8_t *lo = w + g * I3_GBYTES, *hi = lo + 16;
            float cg = coef * sr[g];
    ...
    if (t->fmt != 1 && t->fmt != 2 && t->fmt != 3) {
        fprintf(stderr,
                "qt_addrow: unsupported fmt=%d for the per-row-scale absorb path "
    ...
    if (t->fmt == 1) {
        const int8_t *w = t->q8 + (int64_t)row * I;
        for (int i = 0; i < I; i++) acc[i] += c * (float)w[i];
        return;
    }
    if (t->fmt == 2) {
        const uint8_t *w = t->q4 + (int64_t)row * ((I + 1) / 2);
#if defined(__AVX512F__) && defined(__AVX512BW__)
        axpy_i4f_avx512(w, c, acc, I);
        return; /* bit-identical: one fma per element */
#endif
        for (int i = 0; i + 1 < I; i += 2) {
            uint8_t b = w[i >> 1];
            acc[i] += c * ((int)(b & 0xF) - 8);
    ...
    for (int j = 0; j < n; j++) {
        ...
        if (t->fmt == 0) {
            ...
            for (int g = 0; g * gs < I; g++) {
                ...
                for (int i = base; i + 1 < base + glen; i += 2) {
                    uint8_t b = w[i >> 1];
                    acc += ((int)(b & 0xF) - 8) * x[i] + ((int)(b >> 4) - 8) * x[i + 1];
                }
                if (glen & 1) {
                    uint8_t b = w[(base + glen - 1) >> 1];
                    acc += ((int)(b & 0xF) - 8) * x[base + glen - 1];
                }
                a += acc * sc;
            }
        } else if (t->fmt == 1) {
            ...
            float s = t->s[row];
            float acc = 0;
            for (int i = 0; i < I; i++) acc += (float)w[i] * x[i];
            a = acc * s;
        } else if (t->fmt == 2) {
            const uint8_t *w = t->q4 + (int64_t)row * ((I + 1) / 2);
            float s = t->s[row];
            ...
            const float *scl = t->s + (int64_t)row * ng;
            for (int g = 0; g * gs < I; g++) {
                int base = g * gs, end = base + gs > I ? I : base + gs;
                float acc = 0;
         ...
         * crashed here exactly like qt_addrow did before its own fix; refuse instead. */
            ...
            for (int i = 0; i < I; i++) {
                uint8_t b = w[i >> 2];
                acc += ((int)((b >> ((i & 3) * 2)) & 3) - 2) * x[i];
            }
            a = acc * s;
        } else {
            fprintf(stderr,
                    "qt_matvec_rows: unsupported fmt=%d for the per-row-scale absorb "
...
static int g_metal_prefill = 0; /* default 0: S>4 prefill attention stays on the CPU (bit-exact). COLI_METAL_PREFILL=1
                                   opts it onto the GPU (~4x, near-tie divergence — see docs/metal.md, #622) */
#ifdef COLI_CUDA
...
static int kv_dev_sync(Model *m, Layer *l, int layer, int upto) {
    Cfg *c = &m->c;
    int kvl = c->kv_lora, R = c->qk_rope, dev = l->kv_b.cuda_device;
    ...
    if (v < upto) {
        if (!coli_cuda_pipe_upload(dev, m->kv_dev_L[layer] + (size_t)v * kvl, coli_kv_row(m->kv->Lc[layer], v, kvl),
                                   (size_t)(upto - v) * kvl * 4) ||
            !coli_cuda_pipe_upload(dev, m->kv_dev_R[layer] + (size_t)v * R, coli_kv_row(m->kv->Rc[layer], v, R),
                                   (size_t)(upto - v) * R * 4))
            return 0;
...
static int attn_pipe_prefill(Model *m, Layer *l, int layer, const float *x, int x_is_dev, int S, int pos_base,
                             float *out, float *out_dev) {
    Cfg *c = &m->c;
    int H = c->n_heads, D = c->hidden, qh = c->qk_head;
    int kvl = c->kv_lora, R = c->qk_rope, ql = c->q_lora;
    int dev = l->kv_b.cuda_device;
    if (l->q_a.cuda_device != dev || l->q_b.cuda_device != dev || l->kv_a.cuda_device != dev || l->o.cuda_device != dev)
        return 0;
    ...
    if ((!x_is_dev && !coli_cuda_pipe_upload(dev, xd, x, xb)) ||
        ...
        !coli_cuda_pipe_upload(dev, w2, l->kv_a_ln, (size_t)kvl * 4))
        goto done;
    ...
    if (old > 0) {
        if (!coli_cuda_pipe_upload(dev, ld_, coli_kv_row(m->Lc[layer], st0, kvl), (size_t)old * kvl * 4) ||
            !coli_cuda_pipe_upload(dev, rd, coli_kv_row(m->Rc[layer], st0, R), (size_t)old * R * 4))
            goto done;
    }
    ...
    if (out_dev && l->n_kv_b_shard > 1 && getenv("COLI_CUDA_PIPE_SHARD") && atoi(getenv("COLI_CUDA_PIPE_SHARD"))) {
...
#pragma omp parallel for schedule(static) reduction(& : ok_sh)
            for (int d2 = 0; d2 < n; d2++) {
                ...
                okd = okd && coli_cuda_attention_absorb_batch_dev(l->kv_b_shard[d2], cx_r, qs_r, ld_r, rr_r, S, hn,
                                                                  c->qk_nope, R, vh, kvl, T, c->attn_scale);
        ...
        if (!ok)
            ok = coli_cuda_attention_project_batch_dev_out(l->kv_b.cuda, l->o.cuda, out_dev, qd, ld_, rd, S, H,
                                                           c->qk_nope, R, c->v_head, kvl, T, c->attn_scale);
    } else
#endif
        ok = out_dev ? coli_cuda_attention_project_batch_dev_out(l->kv_b.cuda, l->o.cuda, out_dev, qd, ld_, rd, S, H,
                                                                 c->qk_nope, R, c->v_head, kvl, T, c->attn_scale)
                     : coli_cuda_attention_project_batch_dev(l->kv_b.cuda, l->o.cuda, out, qd, ld_, rd, S, H,
                                                             c->qk_nope, R, c->v_head, kvl, T, c->attn_scale);
    m->t_acore += now_s() - t0;
done:
...
static int attn_pipe_prefix(Model *m, Layer *l, const float *x, int S, float *QR, float *Q, float *comp) {
    Cfg *c = &m->c;
    int D = c->hidden, ql = c->q_lora, kvl = c->kv_lora, R = c->qk_rope;
    ...
    if (!coli_cuda_pipe_upload(dev, xd, x, xb) || !coli_cuda_pipe_gemm(l->q_a.cuda, qrd, xd, S) ||
        !coli_cuda_pipe_download(dev, qrd, QR, qrb))
        return 0;
    for (int s = 0; s < S; s++) rmsnorm(QR + (int64_t)s * ql, QR + (int64_t)s * ql, l->q_a_ln, ql, c->eps);
    if (!coli_cuda_pipe_upload(dev, qrd, QR, qrb) || !coli_cuda_pipe_gemm(l->q_b.cuda, qd, qrd, S) ||
        !coli_cuda_pipe_gemm(l->kv_a.cuda, cd, xd, S) || !coli_cuda_pipe_download(dev, qd, Q, qb) ||
        !coli_cuda_pipe_download(dev, cd, comp, cb))
        return 0;
    return 1;
}
#endif
...
static void attention_rows(Model *m, Layer *l, int layer, float *x, int S, int pos_base, KVState *const *kvs,
                           const int *positions, float *out) {
    Cfg *c = &m->c;
    int H = c->n_heads, D = c->hidden, qh = c->qk_head, vh = c->v_head;
    ...
    if (g_metal_enabled && !kvs && g_absorb != 0 && (S <= 4 || g_metal_prefill) && m->kv_start[layer] == 0 &&
        D == 6144 && H == 64 && c->q_lora == 2048 && c->kv_lora == 512 && c->qk_nope == 192 && c->qk_rope == 64 &&
        vh == 256 && l->kv_b.fmt == 2 && metal_fused_fmt_ok(l->q_a.fmt) && metal_fused_fmt_ok(l->q_b.fmt) &&
...
#define WP_(q) ((q).fmt == 1 ? (const void *)(q).q8 : (const void *)(q).q4)
            int ok = coli_metal_attn_decode(x, WP_(l->q_a), l->q_a.s, l->q_a.fmt, l->q_a.gs, l->q_a_ln, WP_(l->q_b),
                                            l->q_b.s, l->q_b.fmt, l->q_b.gs, WP_(l->kv_a), l->kv_a.s, l->kv_a.fmt,
                                            l->kv_a.gs, l->kv_a_ln, WP_(l->kv_b), l->kv_b.s, l->kv_b.fmt, WP_(l->o),
                                            l->o.s, l->o.fmt, l->o.gs, m->Lc[layer], m->Rc[layer], S, pos_base,
                                            m->kv_start[layer], c->eps, c->theta, c->attn_scale, out);
#undef WP_
            if (ok) {
                m->t_attn += now_s() - ta0;
                return;
    ...
    if (g_cuda_pipe && !kvs && S >= 8 && layer < c->n_layers && g_cuda_enabled && c->kv_lora <= 512 &&
        !(m->has_dsa && pos_base + S > c->index_topk) && l->q_a.cuda_eligible && l->q_b.cuda_eligible &&
        l->kv_a.cuda_eligible && l->kv_b.cuda_eligible && l->o.cuda_eligible && qt_cuda_upload(&l->q_a) &&
        qt_cuda_upload(&l->q_b) && qt_cuda_upload(&l->kv_a) && qt_cuda_upload(&l->kv_b) && qt_cuda_upload(&l->o))
        pipe_done = attn_pipe_prefill(m, l, layer, x, 0, S, pos_base, out, NULL);
    if (!pipe_done && getenv("COLI_CUDA_ATTN_PREFIX") && atoi(getenv("COLI_CUDA_ATTN_PREFIX")) && !kvs && S <= 4 &&
        g_cuda_enabled && (!m->has_dsa || pos_base + S <= c->index_topk) && l->q_a.cuda_eligible &&
        l->q_b.cuda_eligible && l->kv_a.cuda_eligible && qt_cuda_upload(&l->q_a) && qt_cuda_upload(&l->q_b) &&
        qt_cuda_upload(&l->kv_a))
    ...
    if (!pipe_done && !prefix_done) {
        ...
        if (g_vk_qprep == 2) {
            dbgQ = falloc((int64_t)S * l->q_b.O);
            dbgC = falloc((int64_t)S * l->kv_a.O);
        }
        if (g_vk_qprep && g_vk_dense && l->q_a.fmt == l->kv_a.fmt && l->q_a.fmt == l->q_b.fmt && VK_FMT_OK(&l->q_a) &&
            l->q_a.gs == l->kv_a.gs && l->q_a.gs == l->q_b.gs)
            vk_qp = coli_vk_attn_qprep(
                layer, &l->q_a.vk, l->q_a.fmt == 1 ? (const void *)l->q_a.q8 : (const void *)l->q_a.q4, l->q_a.s,
                l->q_a.O, &l->kv_a.vk, l->kv_a.fmt == 1 ? (const void *)l->kv_a.q8 : (const void *)l->kv_a.q4,
                l->kv_a.s, l->kv_a.O, &l->q_b.vk, l->q_b.fmt == 1 ? (const void *)l->q_b.q8 : (const void *)l->q_b.q4,
                l->q_b.s, l->q_b.O, l->q_a.fmt, l->q_a.gs, l->q_a_ln, c->eps, x, S, l->q_a.I,
        ...
        if (!vk_qp) {
            ...
            for (int s = 0; s < S; s++) {
                float *qr = QR + (int64_t)s * c->q_lora;
                rmsnorm(qr, qr, l->q_a_ln, c->q_lora, c->eps);
            } /* q_b legge il residuo NORMATO */
#ifdef COLI_VULKAN
        ...
        if (dbgQ) {
            float mq = 0, mc = 0;
            for (int64_t i2 = 0; i2 < (int64_t)S * l->q_b.O; i2++) {
                float d = fabsf(dbgQ[i2] - Q[i2]) / (fabsf(Q[i2]) + 1e-3f);
                if (d > mq) mq = d;
            }
            for (int64_t i2 = 0; i2 < (int64_t)S * l->kv_a.O; i2++) {
                float d = fabsf(dbgC[i2] - comp[i2]) / (fabsf(comp[i2]) + 1e-3f);
                if (d > mc) mc = d;
            }
    ...
    const int *dsel = NULL, *dnsel = NULL;
    int dtopk = 0;
...
#pragma omp parallel for schedule(dynamic, 1)
            for (int s = 0; s < S; s++) {
                ...
                if (ks->kv_start[layer] != 0) {
                    m->dsa_nsel[s] = 0;
                    continue;
                }
                if (nk <= dtopk && !g_dsa_force) {
                    m->dsa_nsel[s] = 0;
                    continue;
                }
                ...
                for (int t = 0; t < nk; t++) {
                    ...
                    for (int h = 0; h < nh; h++) {
                        ...
                        float d0 = 0;
                        for (int i = 0; i < hd; i++) d0 += qhp[i] * kt[i];
                        d0 *= rs;
                        if (d0 > 0) a += w32[h] * d0; /* ReLU sullo score, poi peso */
                    }
                    isc[t] = a * wsc;
                }
                ...
                for (int t = 1; t < keep; t++)
                    if (tmp[t] < thr) thr = tmp[t];
                int *dst = m->dsa_sel + (int64_t)s * dtopk, nd = 0;
                for (int t = 0; t < nk && nd < keep; t++)
                    if (isc[t] > thr) dst[nd++] = t;
                for (int t = 0; t < nk && nd < keep; t++)
                    if (isc[t] == thr) dst[nd++] = t;
                m->dsa_nsel[s] = nd;
                free(qi);
...
#ifdef COLI_CUDA
        if (kvs && g_cuda_enabled && getenv("COLI_CUDA_ATTN") && atoi(getenv("COLI_CUDA_ATTN")) && !dnsel &&
            l->kv_b.cuda_eligible && l->o.cuda_eligible && qt_cuda_upload(&l->kv_b) && qt_cuda_upload(&l->o)) {
            const float **rl = malloc((size_t)S * sizeof(*rl)), **rr = malloc((size_t)S * sizeof(*rr));
            const void **rk = malloc((size_t)S * sizeof(*rk));
            int *rn = malloc((size_t)S * sizeof(*rn));
            int mt = 0;
            if (rk && rl && rr && rn) {
                for (int s = 0; s < S; s++) {
                    int pos = positions[s], st0 = kvs[s]->kv_start[layer];
                    rn[s] = pos + 1 - st0;
                    rk[s] = kvs[s];
                ...
                cuda_core = cuda_projected =
                    coli_cuda_attention_project_ragged(l->kv_b.cuda, l->o.cuda, out, Q, rk, rl, rr, rn, S, H,
                                                       c->qk_nope, c->qk_rope, vh, kvl, mt, c->attn_scale);
            }
            free(rk);
            free(rl);
            free(rr);
            ...
            for (int d = 0; d < n; d++)
                for (int s = 0; s < S; s++)
                    memcpy(qs + (int64_t)l->shard_h0[d] * S * qh + (int64_t)s * l->shard_hn[d] * qh,
                           Q + ((int64_t)s * H + l->shard_h0[d]) * qh, (size_t)l->shard_hn[d] * qh * sizeof(float));
#pragma omp parallel for schedule(static) reduction(& : ok)
            for (int d = 0; d < n; d++)
                ok &= coli_cuda_attention_absorb_batch(
                    l->kv_b_shard[d], cs + (int64_t)l->shard_h0[d] * S * vh, qs + (int64_t)l->shard_h0[d] * S * qh,
                    coli_kv_row(m->Lc[layer], st0, kvl), coli_kv_row(m->Rc[layer], st0, c->qk_rope), S, l->shard_hn[d],
                    c->qk_nope, c->qk_rope, vh, kvl, nt, c->attn_scale);
            if (ok)
                for (int d = 0; d < n; d++)
                    for (int s = 0; s < S; s++)
                        memcpy(ctx + ((int64_t)s * H + l->shard_h0[d]) * vh,
                               cs + (int64_t)l->shard_h0[d] * S * vh + (int64_t)s * l->shard_hn[d] * vh,
                               (size_t)l->shard_hn[d] * vh * sizeof(float));
            free(qs);
            free(cs);
            cuda_core = ok;
        } else if (cuda_absorb && l->kv_b.cuda_eligible && l->o.cuda_eligible && qt_cuda_upload(&l->kv_b) &&
                   qt_cuda_upload(&l->o)) {
            int st0 = m->kv_start[layer], nt = pos_base + S - st0;
            cuda_core = cuda_projected = coli_cuda_attention_project_batch(
                l->kv_b.cuda, l->o.cuda, out, Q, coli_kv_row(m->Lc[layer], st0, kvl),
                coli_kv_row(m->Rc[layer], st0, c->qk_rope), S, H, c->qk_nope, c->qk_rope, vh, kvl, nt, c->attn_scale);
            ...
            for (int s = 0; s < S && cuda_core; s++) {
                ...
                if (dnsel && dnsel[s] > 0) {
                    ...
                    int ns = dnsel[s];
                    const int *tl = dsel + (int64_t)s * dtopk;
                    float *gl = falloc((int64_t)ns * kvl), *gr = falloc((int64_t)ns * c->qk_rope);
                    for (int jj = 0; jj < ns; jj++) {
                        memcpy(gl + (int64_t)jj * kvl, coli_kv_row(ks->Lc[layer], tl[jj], kvl),
                               (size_t)kvl * sizeof(float));
                        memcpy(gr + (int64_t)jj * c->qk_rope, coli_kv_row(ks->Rc[layer], tl[jj], c->qk_rope),
                               (size_t)c->qk_rope * sizeof(float));
                    }
                    cuda_core =
                        coli_cuda_attention_absorb(l->kv_b.cuda, ctx + (int64_t)s * H * vh, Q + (int64_t)s * H * qh, gl,
                                                   gr, H, c->qk_nope, c->qk_rope, vh, kvl, ns, c->attn_scale);
                    free(gl);
                    free(gr);
                ...
                if (g_cuda_pipe && ks == m->kv && layer < c->n_layers && kv_dev_sync(m, l, layer, pos + 1))
                    cuda_core = coli_cuda_attention_absorb_kvdev(
                        l->kv_b.cuda, ctx + (int64_t)s * H * vh, Q + (int64_t)s * H * qh,
                        m->kv_dev_L[layer] + (size_t)st0 * kvl, m->kv_dev_R[layer] + (size_t)st0 * c->qk_rope, H,
                        c->qk_nope, c->qk_rope, vh, kvl, nt, c->attn_scale);
                if (!cuda_core)
                    cuda_core = coli_cuda_attention_absorb(
                        l->kv_b.cuda, ctx + (int64_t)s * H * vh, Q + (int64_t)s * H * qh,
                        coli_kv_row(ks->Lc[layer], st0, kvl), coli_kv_row(ks->Rc[layer], st0, c->qk_rope), H,
                        c->qk_nope, c->qk_rope, vh, kvl, nt, c->attn_scale);
            }
        }
#endif
        ...
        if (!cuda_core && g_vk_attn && !kvs && !positions && S <= 4 && layer < c->n_layers && VK_FMT_OK(&l->kv_b) &&
            kvl <= 512 && c->qk_nope <= 256 && c->qk_rope <= 64 && m->vk_kv_valid && m->Lc[layer] &&
            m->Rc[layer]) { /* f32 KV only: a quantized-KV cache
            ...
            if (!dsa_on && T <= m->max_t && coli_vk_kv_ensure(layer, m->max_t, kvl, c->qk_rope)) {
                ...
                if (ok) {
                    ...
                    if (VK_FMT_OK(&l->o) &&
                        coli_vk_attention_absorb_project(&l->kv_b.vk, kw, l->kv_b.s, l->kv_b.fmt, l->kv_b.gs, &l->o.vk,
                                                         l->o.fmt == 1 ? (const void *)l->o.q8 : (const void *)l->o.q4,
                                                         l->o.s, l->o.fmt, l->o.gs, out, Q, layer, S, H, c->qk_nope,
                                                         c->qk_rope, vh, kvl, st0, T, c->attn_scale, D))
                        vk_core = vk_projected = 1;
                    if (!vk_core)
                        vk_core =
                            coli_vk_attention_absorb(&l->kv_b.vk, kw, l->kv_b.s, l->kv_b.fmt, l->kv_b.gs, ctx, Q, layer,
                                                     S, H, c->qk_nope, c->qk_rope, vh, kvl, st0, T, c->attn_scale);
                }
            }
        }
#endif
    ...
    for (int s = 0; s < S; s++)
        for (int h = 0; h < H; h++) {
            ...
            for (int jj = 0; jj < nt; jj++) {
                ...
                float a = 0;
                for (int d = 0; d < c->qk_nope; d++) a += qp[d] * kn[d];
                for (int d = 0; d < c->qk_rope; d++) a += qr[d] * kr[d];
                sc[jj] = a * c->attn_scale;
            }
            softmax(sc, nt);
            ...
            for (int jj = 0; jj < nt; jj++) {
                ...
                float a = sc[jj];
                for (int d = 0; d < vh; d++) cx[d] += a * vv[d];
            }
        }
    m->t_acore += now_s() - tac;
    double tao = now_s();
...
static int expert_is_resident(Model *m, int layer, int eid) {
    ...
    for (int z = 0; z < m->npin[layer]; z++)
        if (P[z].eid == eid) return 1;
    ESlot *Sl = m->ecache[layer];
    for (int z = 0; z < m->ecn[layer]; z++)
        if (Sl[z].eid == eid) return 1;
    return 0;
}
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out, int with_shared) {
    if (g_pilot_real) { /* barriera cross-layer: prendi possesso di QUESTO layer e aspetta
        ...
        while (layer >= 0 && layer < 256 && g_pilot_inflight[layer] > 0) pthread_cond_wait(&g_pilot_cv, &g_pilot_mx);
    ...
    unsigned char touched[E];
    if (need_classify) memset(touched, 0, (size_t)E);
    ...
    int *idxs = xalloc((size_t)S * K * sizeof(int), "moe idxs");
    float *ws = xalloc((size_t)S * K * sizeof(float), "moe ws");
        ...
        for (int s = 0; s < S; s++) {
            ...
            for (int kk = 0; kk < keff[s]; kk++) {
                ...
                if (need_classify) { /* DISK-CLASS private recency -- snapshot BEFORE this call's own bump,
                    ...
                    int e = idxs[(int64_t)s * K + kk];
                    if (!touched[e]) {
                        m->elast_pre[layer][e] = m->elast_dc[layer][e];
                        touched[e] = 1;
                    }
                    m->elast_dc[layer][e] = ++m->eaccess_clock_dc;
                }
                if (m->eheat[layer][idxs[(int64_t)s * K + kk]] < UINT32_MAX)
        ...
        for (int s = 0; s < S; s++) {
            ...
            if (do_cache_route) {
                ...
                if (g_route_p > 0.f && g_route_p < 1.f) {
                    ...
                    int Mmax = g_route_m > Ksel * 4 ? g_route_m : Ksel * 4;
                    if (Mmax > E) Mmax = E;
                    if (Mmax > rank_cap) Mmax = rank_cap;
                    for (int kk = 0; kk < Mmax; kk++) {
                        ...
                        rank_buf[kk] = best;
                        rank_w[kk] = logit[best];
                    }
                    float tot = 1e-20f;
                    for (int kk = 0; kk < Mmax; kk++) tot += rank_w[kk] > 0 ? rank_w[kk] : 0;
                    float cum = 0;
                    Mwin = Ksel;
                    for (int kk = 0; kk < Mmax; kk++) {
                        cum += rank_w[kk] > 0 ? rank_w[kk] : 0;
                        if (cum >= g_route_p * tot) {
                            Mwin = kk + 1;
                            break;
                        }
                        Mwin = kk + 1;
                ...
                while (chosen < Ksel) {
                    idx[chosen] = rank_buf[chosen];
                    w[chosen] = rank_w[chosen];
                    chosen++;
                }
            ...
            if (g_topp > 0 && g_topp < 1.f) {
                ...
                float tot = 1e-20f;
                for (int kk = 0; kk < Ksel; kk++) tot += w[kk];
                float cum = 0;
                for (int kk = 0; kk < Ksel; kk++) {
                    cum += w[kk];
                    if (cum >= g_topp * tot) {
            ...
            for (int kk = 0; kk < Ke; kk++) {
                ...
                if (need_classify) { /* DISK-CLASS private recency -- snapshot BEFORE this call's own bump,
                    ...
                    if (!touched[idx[kk]]) {
                        m->elast_pre[layer][idx[kk]] = m->elast_dc[layer][idx[kk]];
                        touched[idx[kk]] = 1;
                    }
    ...
    if (g_looka && S == 1 && layer < c->n_layers) {
        ...
        if (m->enr[layer] > 0) { /* [0] vs routing del token precedente */
            for (int kk = 0; kk < Ke; kk++)
                for (int z = 0; z < m->enr[layer]; z++)
                    if (m->eroute[layer][z] == idxs[kk]) {
                        la_hit[0]++;
                        break;
                    }
            la_tot[0] += Ke;
        ...
        for (int kind = 0; kind < 3; kind++)
            if (la_val[kind][layer]) { /* score all prediction kinds */
                for (int kk = 0; kk < Ke; kk++)
                    for (int z = 0; z < K; z++)
                        if (la_pred[kind][layer][z] == idxs[kk]) {
                            la_hit[1 + kind]++;
                            break;
                        }
                la_tot[1 + kind] += Ke;
    ...
    m->enr[layer] = keff[S - 1];
    for (int kk = 0; kk < keff[S - 1]; kk++) m->eroute[layer][kk] = idxs[(int64_t)(S - 1) * K + kk];
    /* ---- FASE B: union degli expert del batch ---- */
    int *uniq = xalloc((size_t)E * sizeof(int), "moe uniq");
    int nu = 0;
    unsigned char seen[E];
    ...
    if (g_expert_budget > 0 && S <= 4 && nu > g_expert_budget) {
        ...
        float *wsum = falloc(nu);
        for (int j = 0; j < nu; j++) wsum[j] = 0;
        for (int s = 0; s < S; s++)
            for (int kk = 0; kk < keff[s]; kk++) {
                int e = idxs[(int64_t)s * K + kk];
                for (int j = 0; j < nu; j++)
                    if (uniq[j] == e) {
                        wsum[j] += ws[(int64_t)s * K + kk];
                        break;
                    }
            }
        /* residency pre-scan: which experts are already in pin or ecache (hits)? */
        unsigned char *is_hit = xzalloc((size_t)nu, "moe is_hit");
        int nhits = 0;
        for (int j = 0; j < nu; j++) {
            int eid = uniq[j];
            int found = 0;
        ...
        int miss_budget = g_expert_budget - nhits;
        ...
        unsigned char *keep = xzalloc((size_t)nu, "moe keep");
        int nkeep = 0;
        for (int j = 0; j < nu; j++)
            if (is_hit[j]) {
                keep[j] = 1;
        ...
        for (int s = 0; s < S; s++) {
            ...
            for (int kk = 0; kk < keff[s] && !alive; kk++)
                if (seen[idxs[(int64_t)s * K + kk]]) alive = 1;
        ...
        int dropped = nu - nkeep;
        g_budget_dropped += dropped;
        ...
        for (int s = 0; s < S; s++) {
            ...
            if (w < keff[s]) {
                ...
                if (c->norm_topk && w > 0) {
                    ...
                    float sm = 0;
                    for (int kk = 0; kk < w; kk++) sm += ws[(int64_t)s * K + kk];
                    sm += 1e-20f;
...
#define E8_XE(e)                                                                                                       \
    ((e)->g.fmt == 6 ? (xe ? xe                                                                                        \
                           : (xe = falloc((int64_t)S * D), memcpy(xe, x, (size_t)S * D * sizeof(float)),               \
                              e8_rot_rows(xe, S, D), xe))                                                              \
                     : x)
    int *rows = xalloc((size_t)S * sizeof(int), "moe rows");
    float *rw = xalloc((size_t)S * sizeof(float), "moe rw");
#ifdef COLI_CUDA
    ...
    (void)vk_active;
#ifdef COLI_VULKAN
    vk_active = g_vulkan && (g_vk_reg_n + g_vk_reg_n2) > 0 && !omp_in_parallel() &&
                S <= 4; /* empty
...
#endif
    int shared_on_gpu = 0;
    (void)shared_on_gpu; /* set by the Metal path when Phase E was fused */
    for (int base = 0; base < nu; base += 64) {
        ...
        for (int j = 0; j < nb; j++) {
            ...
            if (!use[j]) {
                ...
                m->miss++;
                if (g_disk_split) {
                    if (m->ld_ctx == 1) m->miss_draft++;
                    else if (m->ld_ctx == 2) m->miss_absorb++;
                }
            }
        }
        int metal_done = 0;
...
#define MB_BUILD(WANTMISS, TRY_SH)                                                                                     \
    do {                                                                                                               \
        ...
        sh_in = 0;                                                                                                     \
        for (int j = 0; j < nb; j++) {                                                                                 \
            if (is_miss[j] != (WANTMISS)) continue;                                                                    \
            int eid = uniq[base + j];                                                                                  \
            ESlot *e = use[j];                                                                                         \
            int cnt = 0;                                                                                               \
            for (int s = 0; s < S; s++)                                                                                \
        ...
        if (TRY_SH) {                                                                                                  \
            int shf = mfmt < 0 ? l->sh_gate.fmt : mfmt;                                                                \
            if (c->n_shared == 1 && sI == I && l->sh_gate.fmt == shf && l->sh_up.fmt == shf &&                         \
                l->sh_down.fmt == shf) {                                                                               \
                if (mfmt < 0) mfmt = shf;                                                                              \
        ...
        if (g_metal_enabled) {
            ...
            mrows = xalloc((size_t)(nb + 1) * S * sizeof(int), "moe mrows");
            mrw = xalloc((size_t)(nb + 1) * S * sizeof(float), "moe mrw");
            MB_BUILD(0, base == 0 && !g_pre_sh);
            if (nbb > 0) {
                double t0 = now_s();
                if (mfmt == 6) metal_stage_rot_e8(mxg, mrows, Rtot, D);
                mh =
                    coli_metal_moe_block_begin(nbb, D, I, mfmt, MG, MU, MD, MGS, MUS, MDS, mxg, xoffb, nrb, mrows, mrw);
                m->t_emm += now_s() - t0;
                if (mh) {
                    cpu_res = 0;
                    mh_shared = sh_in;
        ...
        if (nmiss) {
            if (g_pipe) { /* PIPE: launch loads async, matmul overlaps them */
                ...
                double t0 = now_s(); /* ORIGINALE: blocking parallel load */
#pragma omp parallel for schedule(dynamic, 1)
                for (int q = 0; q < nmiss; q++)
                    expert_load(m, layer, uniq[base + missk[q]], &m->ws[q], 1, 1); /* demand=1: this IS the miss path */
                m->t_ewait += now_s() - t0;
            } /* compute thread blocked for the whole load */
        }
...
#endif
                for (int z = 0; z < m->npin[layer] && !found; z++)
                    if (P[z].eid == eid) found = 1;
                ESlot *Sl = m->ecache[layer];
                for (int z = 0; z < m->ecn[layer] && !found; z++)
                    if (Sl[z].eid == eid) found = 1;
        ...
        ESlot *eg_e[64];
        int eg_n[64], eg_row[64][4], eg_npg = 0;
        float eg_w[64][4];
        int dev_nc0[COLI_CUDA_MAX_DEVICES], dev_off0[COLI_CUDA_MAX_DEVICES], dev_total0[COLI_CUDA_MAX_DEVICES],
            dev_which0[COLI_CUDA_MAX_DEVICES][64];
        memset(dev_nc0, 0, sizeof(dev_nc0));
        (void)eg_npg;
        (void)dev_total0;
        (void)dev_off0;
#endif
        ...
        if (g_metal_enabled) {
            ...
            if (nbb > 0) {
                double t0 = now_s();
                if (mfmt == 6) metal_stage_rot_e8(mxg, mrows, Rtot, D);
                if (coli_metal_moe_block(nbb, D, I, mfmt, MG, MU, MD, MGS, MUS, MDS, mxg, xoffb, nrb, mrows, mrw, out,
                                         S))
                    cpu_miss = 0;
                m->t_emm += now_s() - t0;
            } else cpu_miss = 0;
            if (mh) {
                double t0 = now_s();
                if (coli_metal_moe_block_end(mh, out)) {
                    if (mh_shared) shared_on_gpu = 1;
                } else cpu_res = 1;
                m->t_emm += now_s() - t0;
                mh = NULL;
            }
...
#undef MB_BUILD
        ...
        int early_issued = 0, done_j[64] = {0};
        {
            ...
            if (!metal_done && g_group_async2 && group_enabled && S <= 4 && g_cuda_enabled && g_cuda_ndev > 0 &&
                !omp_in_parallel()) {
                ESlot *pg_e[64];
                int pg_n[64], pg_j[64], npg = 0;
                int prow[64][4];
                float pw[64][4];
                for (int j = 0; j < nb; j++) {
                    ESlot *e = use[j];
                    ...
                    int nr = 0;
                    for (int s = 0; s < S && nr < 4; s++)
                        for (int kk = 0; kk < keff[s]; kk++)
                            if (idxs[(int64_t)s * K + kk] == eid) {
                                prow[npg][nr] = s;
                                pw[npg][nr] = ws[(int64_t)s * K + kk];
                                nr++;
                                break;
                            }
                    if (!nr) continue;
                ...
                if (npg) {
                    ...
                    ColiCudaTensor *pd_g[COLI_CUDA_MAX_DEVICES][64], *pd_u[COLI_CUDA_MAX_DEVICES][64],
                        *pd_d[COLI_CUDA_MAX_DEVICES][64];
                    int pd_rows[COLI_CUDA_MAX_DEVICES][64], pd_which[COLI_CUDA_MAX_DEVICES][64];
                    int pd_nc[COLI_CUDA_MAX_DEVICES] = {0}, pd_total[COLI_CUDA_MAX_DEVICES] = {0},
                        pd_off[COLI_CUDA_MAX_DEVICES] = {0};
                    ...
                    for (int di = 0; di < g_cuda_ndev; di++) {
                        ...
                        for (int q = 0; q < npg; q++)
                            if (pg_e[q]->g.cuda_device == device) {
                                int nc = pd_nc[di]++;
                                ESlot *e = pg_e[q];
                                pd_g[di][nc] = e->g.cuda;
                                pd_u[di][nc] = e->u.cuda;
                                pd_d[di][nc] = e->d.cuda;
                                pd_rows[di][nc] = pg_n[q];
                                pd_which[di][nc] = q;
                                const float *xsrc = E8_XE(e); /* fmt=6 feeds the rotated copy */
                                for (int r = 0; r < pg_n[q]; r++)
                                    memcpy(group_x + (int64_t)(pd_off[di] + cursor + r) * D,
                    ...
                    for (int di = 0; di < g_cuda_ndev && all; di++)
                        if (pd_nc[di])
                            all = issued[di] =
                                coli_cuda_expert_group_issue(pd_g[di], pd_u[di], pd_d[di], pd_rows[di], pd_nc[di],
                                                             group_x + (int64_t)pd_off[di] * D);
                    g_ovl_issue += now_s() - tg0;
                    if (all) {
                        static int announced2;
                        if (!announced2) {
                            announced2 = 1;
                            fprintf(stderr, "[CUDA] expert group overlap active\n");
                        }
                        early_issued = 1;
                        g_ovl_mark = now_s();
                        ...
                        for (int di = 0; di < g_cuda_ndev; di++) {
                            dev_nc0[di] = pd_nc[di];
                            dev_off0[di] = pd_off[di];
                            dev_total0[di] = pd_total[di];
        ...
        if (g_xexp && !metal_done && !vk_active && S == 1 && !nmiss && !spec_pinned() && g_idot && g_i4s <= 1
            ...
            for (int j = 0; j < nb; j++) {
                ESlot *e = use[j];
                if (e->g.fmt != 2 || e->u.fmt != 2 || e->d.fmt != 2 || e->g.I != D || e->g.O != I || e->d.I != I ||
                    e->d.O != D) {
                    allq4 = 0;
                    break;
                }
            ...
            if (allq4) {
                ...
                if (okw) {
                    ...
                    float gsc[64];
                    if (!xq8) {
                        fprintf(stderr, "OOM xexp scratch\n");
                        exit(1);
                    }
                    int8_t *GQ = xq8 + D;
...
#pragma omp for schedule(dynamic, 1)
                        for (int it = 0; it < nb * C1; it++) {
                            int j = it / C1, c0 = (it % C1) * r1;
                            int c1 = c0 + r1 < I ? c0 + r1 : I;
                            ESlot *e = use[j];
                            const uint8_t *qg = e->g.q4, *qu = e->u.q4;
                            float *gj = GG + (int64_t)j * I, *uj = UU + (int64_t)j * I;
                            for (int o = c0; o < c1; o++)
                                gj[o] = (float)dot_i4i8(qg + (int64_t)o * rbD, xq8, D) * e->g.s[o] * sx0;
                            for (int o = c0; o < c1; o++)
                                uj[o] = (float)dot_i4i8(qu + (int64_t)o * rbD, xq8, D) * e->u.s[o] * sx0;
                            for (int o = c0; o < c1; o++) gj[o] = siluf(gj[o]) * uj[o];
                        } /* implicit barrier */
#pragma omp for schedule(static)
                        for (int j = 0; j < nb; j++) gsc[j] = qrow_i8(GG + (int64_t)j * I, GQ + (int64_t)j * I, I);
#pragma omp for schedule(dynamic, 1)
                        for (int it = 0; it < nb * C2; it++) {
                            int j = it / C2, c0 = (it % C2) * r2;
                            int c1 = c0 + r2 < D ? c0 + r2 : D;
                            ESlot *e = use[j];
                            const uint8_t *qd = e->d.q4;
                            const int8_t *gq = GQ + (int64_t)j * I;
                            float *hj = HH + (int64_t)j * D;
                            for (int o = c0; o < c1; o++)
                                hj[o] = (float)dot_i4i8(qd + (int64_t)o * rbI, gq, I) * e->d.s[o] * gsc[j];
                        }
                    }
                    for (int j = 0; j < nb; j++) {
                        float w = wj[j], *hj = HH + (int64_t)j * D;
                    ...
                    if (g_prof) {
                        m->t_ecpu += dt;
                        m->cpu_expert_rows += (uint64_t)nb;
                        for (int j = 0; j < nb; j++)
                            m->cpu_expert_bytes += qt_bytes(&use[j]->g) + qt_bytes(&use[j]->u) + qt_bytes(&use[j]->d);
                    }
                    free(xq8);
                    free(GG);
                    free(UU);
        ...
        if (vk_active && !metal_done) {
            ...
            int veid[64];
            int vrows[64], voff[64], vrmap[64 * 4];
            float vwmap[64 * 4];
            ColiVkTensor *vg2[64], *vu2[64], *vd2[64];
            int veid2[64];
            int vrows2[64], voff2[64], vrmap2[64 * 4];
            float vwmap2[64 * 4];
            ESlot *ce[64];
            int cnr[64], crmap[64 * 4];
            float cwmap[64 * 4];
            int cqof[64];
            ...
            for (int j = 0; j < nb; j++) {
                ...
                if (vk_hit[j]) { /* registry-served: no RAM slot, no disk load */
                    ...
                    if (vk2_on && coli_vk_tensor_dev(reg[0]) == 1) { /* dev2 tier expert */
                        voff2[nvk2] = vtot2;
                        for (int r = 0; r < nr; r++) {
                            memcpy(vk_xh2 + (int64_t)(vtot2 + r) * D, x + (int64_t)rows[r] * D, D * sizeof(float));
                            vrmap2[nvk2 * S + r] = rows[r];
                            vwmap2[nvk2 * S + r] = rw[r];
                        }
                        vg2[nvk2] = reg[0];
                        vu2[nvk2] = reg[1];
                        vd2[nvk2] = reg[2];
                        ...
                        voff[nvk] = vtot;
                        for (int r = 0; r < nr; r++) {
                            memcpy(vk_xh + (int64_t)(vtot + r) * D, x + (int64_t)rows[r] * D, D * sizeof(float));
                            vrmap[nvk * S + r] = rows[r];
                            vwmap[nvk * S + r] = rw[r];
                        }
                        vg[nvk] = reg[0];
                        vu[nvk] = reg[1];
             ...
             * meaning); the waits land in t_ewait. */
            {
                ...
                for (int oi = 0; oi < no; oi++) {
                    ...
                    if (!e->slab)
                        expert_load(m, layer, e->eid, e, 1, 1); /* demand=1: moe miss path (FASE A snapshot valid) */
                    for (int r = 0; r < nr; r++)
                        memcpy(xg + (int64_t)r * D, x + (int64_t)crmap[c2 * S + r] * D, D * sizeof(float));
                    double te0 = now_s();
                    expert_gate_up(gg, uu, xg, &e->g, &e->u, nr);
                    for (int64_t z = 0; z < (int64_t)nr * I; z++) gg[z] = siluf(gg[z]) * uu[z];
                    matmul_qt(hh, gg, &e->d, nr);
                    for (int r = 0; r < nr; r++) {
                        float *os = out + (int64_t)crmap[c2 * S + r] * D, wgt = cwmap[c2 * S + r],
                              *hr = hh + (int64_t)r * D;
            ...
            for (int c2 = 0; c2 < nvk; c2++) {
                ...
                if (vk_ok) {
                    int o = voff[c2];
                    for (int r = 0; r < nr; r++) {
                        float *os = out + (int64_t)vrmap[c2 * S + r] * D, wgt = vwmap[c2 * S + r],
                              *src = vk_yh + (int64_t)(o + r) * D;
                    ...
                    if (e->eid != veid[c2] || !e->slab)
                        expert_load(m, layer, veid[c2], e, 1,
                                    0); /* device-lost recovery: DISK-CLASS leaves it unclassified */
                    for (int r = 0; r < nr; r++)
                        memcpy(xg + (int64_t)r * D, x + (int64_t)vrmap[c2 * S + r] * D, D * sizeof(float));
                    expert_gate_up(gg, uu, xg, &e->g, &e->u, nr);
                    for (int64_t z = 0; z < (int64_t)nr * I; z++) gg[z] = siluf(gg[z]) * uu[z];
                    matmul_qt(hh, gg, &e->d, nr);
                    for (int r = 0; r < nr; r++) {
            ...
            if (iss2_threaded) {
                double t_j0 = now_s();
                pthread_join(iss2_th, NULL);
                if (g_prof) {
                    g_vkb_join += now_s() - t_j0;
                    g_vkb_wrk += iss2.dt;
                }
            }
            int vk2_ok = iss2.rc && coli_vk_expert_group_take2(vk_yh2);
            for (int c2 = 0; c2 < nvk2; c2++) {
                int nr = vrows2[c2];
                if (vk2_ok) {
                    int o = voff2[c2];
                    for (int r = 0; r < nr; r++) {
                        float *os = out + (int64_t)vrmap2[c2 * S + r] * D, wgt = vwmap2[c2 * S + r],
                              *src = vk_yh2 + (int64_t)(o + r) * D;
                        for (int d = 0; d < D; d++) os[d] += wgt * src[d];
                    }
                } else {
                    ESlot *e = &m->ws[nmiss < 63 ? nmiss : 63];
                    ...
                    for (int r = 0; r < nr; r++)
                        memcpy(xg + (int64_t)r * D, x + (int64_t)vrmap2[c2 * S + r] * D, D * sizeof(float));
                    expert_gate_up(gg, uu, xg, &e->g, &e->u, nr);
                    for (int64_t z = 0; z < (int64_t)nr * I; z++) gg[z] = siluf(gg[z]) * uu[z];
                    matmul_qt(hh, gg, &e->d, nr);
                    for (int r = 0; r < nr; r++) {
        ...
        if (!metal_done && !xexp_done && !vk_active)
            for (int j = 0; j < nb; j++) {
                ...
                for (int r = 0; r < nr; r++)
                    memcpy(xg + (int64_t)r * D, xsrc + (int64_t)rows[r] * D, D * sizeof(float));
                double t0 = now_s();
#ifdef COLI_CUDA
            ...
            for (int di = 0; di < g_cuda_ndev; di++)
                if (dev_nc0[di]) {
                    ...
                    for (int q = 0; q < dev_nc0[di]; q++) {
                        ...
                        if (hy) {
                            for (int r = 0; r < nr; r++) {
                                float *os = out + (int64_t)eg_row[gi][r] * D;
                                float wgt = eg_w[gi][r];
                            ...
                            ESlot *e = eg_e[gi];
                            for (int r = 0; r < nr; r++)
                                memcpy(xg + (int64_t)r * D, x + (int64_t)eg_row[gi][r] * D, D * sizeof(float));
        ...
        for (int di = 0; di < g_cuda_ndev; di++) {
            int cursor = 0, device = g_cuda_devices[di];
            for (int q = 0; q < ngroup; q++)
                if (group_e[q]->g.cuda_device == device) {
                    int nc = dev_nc[di]++;
                    ESlot *e = group_e[q];
                    dev_g[di][nc] = e->g.cuda;
                    dev_u[di][nc] = e->u.cuda;
                    dev_d[di][nc] = e->d.cuda;
                    dev_rows[di][nc] = group_n[q];
                    dev_which[di][nc] = q;
                    const float *xsrc = E8_XE(e); /* fmt=6 feeds the rotated copy */
                    for (int r = 0; r < group_n[q]; r++)
                        memcpy(group_x + (int64_t)(dev_off[di] + cursor + r) * D,
        ...
        if (g_group_async && S <= 4 && g_cuda_ndev > 0) {
            ...
            for (int di = 0; di < g_cuda_ndev && all; di++)
                if (dev_nc[di])
                    all = issued[di] = coli_cuda_expert_group_issue(dev_g[di], dev_u[di], dev_d[di], dev_rows[di],
                                                                    dev_nc[di], group_x + (int64_t)dev_off[di] * D);
            if (all) {
                static int announced;
                if (!announced) {
                    announced = 1;
                    fprintf(stderr, "[CUDA] expert group async path active\n");
                }
                async_done = 1;
                for (int di = 0; di < g_cuda_ndev; di++)
                    if (dev_nc[di]) {
                ...
                for (int di = 0; di < g_cuda_ndev; di++)
                    if (dev_nc[di]) {
                        ...
                        if (coli_cuda_expert_group_resident_issue(dev_g[di], dev_u[di], dev_d[di], wbuf, dev_nc[di],
                                                                  g_pres_home, g_pres_xsrc,
                                                                  g_pres_slots + (int64_t)g_pres_nused * D)) {
...
#pragma omp parallel for if (g_cuda_ndev > 1) schedule(static)
        ...
        for (int di = 0; di < g_cuda_ndev; di++) {
            ...
            for (int q = 0; q < dev_nc[di]; q++) {
                int gi = dev_which[di][q], nr = group_n[gi];
                ESlot *e = group_e[gi];
                if (!dev_ok[di]) {
                    const float *xsrc = E8_XE(e); /* fmt=6 feeds the rotated copy */
                    for (int r = 0; r < nr; r++)
                        memcpy(xg + (int64_t)r * D, xsrc + (int64_t)group_row[(int64_t)gi * S + r] * D,
                               D * sizeof(float));
                ...
                for (int r = 0; r < nr; r++) {
                    float *os = out + (int64_t)group_row[(int64_t)gi * S + r] * D,
                          wgt = group_weight[(int64_t)gi * S + r];
        ...
        if (g_prof) {
            double mx = 0;
            for (int di = 0; di < g_cuda_ndev; di++)
                if (dev_time[di] > mx) mx = dev_time[di];
            m->t_egpu += mx;
        }
        m->t_emm += now_s() - tg;
#endif
        ...
        {
            ESlot *Sl = m->ecache[layer];
            int *nn = &m->ecn[layer]; /* promozione LRU (swap buffer) */
    ...
    {
        float *sg = NULL, *su = NULL;
        int shared_cuda = 0;
#ifdef COLI_METAL
        if (g_pre_sh) {
            for (int64_t z = 0; z < (int64_t)S * D; z++) out[z] += g_pre_sh[z];
            shared_on_gpu = 1;
        }
...
#ifdef COLI_CUDA
        int shared_min =
            getenv("COLI_CUDA_SHARED_W4A16_MIN_ROWS") ? atoi(getenv("COLI_CUDA_SHARED_W4A16_MIN_ROWS")) : 32;
        if (shared_min < 16) shared_min = 16;
        if (shared_cuda == 0 && S >= shared_min && !l->shared_w4a16_failed && !omp_in_parallel() && g_cuda_enabled &&
            l->sh_gate.fmt == 2 && l->sh_up.fmt == 2 && l->sh_down.fmt == 2 && getenv("COLI_CUDA_SHARED_W4A16") &&
            atoi(getenv("COLI_CUDA_SHARED_W4A16")) && qt_cuda_upload(&l->sh_gate) && qt_cuda_upload(&l->sh_up) &&
            qt_cuda_upload(&l->sh_down)) {
            shared_cuda = coli_cuda_shared_mlp_w4a16(l->sh_gate.cuda, l->sh_up.cuda, l->sh_down.cuda, hh, x, S);
        ...
        if (!shared_cuda) {
            ...
            if (g_vk_dense && !omp_in_parallel() && (fsh == 1 || fsh == 2 || fsh == 5) && l->sh_up.fmt == fsh &&
                l->sh_down.fmt == fsh) {
...
#undef SW_
            ...
            if (!shared_cuda) {
#endif
                sg = falloc((int64_t)S * sI);
                su = falloc((int64_t)S * sI);
#ifdef COLI_VULKAN
...
static void la_predict(Model *m, int target, const float *h, int kind) {
    ...
    if (kind == 2) {
        ...
        if (src_layer < 0 || src_layer >= c->n_layers || !m->L[src_layer].sparse || c->n_shared <= 0 ||
            c->moe_inter <= 0) {
            la_val[2][target] = 0;
            free(nrm);
            free(ch);
...
static struct {
    _Atomic int l, e;
} pilot_q[4096]; /* payload atomico (relaxed): la claim SPMC legge speculativamente prima della CAS e scarta se perde ->
                    senza _Atomic sarebbe una data race C11 col produttore. int e' sempre lock-free: stessa size/align.
                  */
...
static void pilot_realload(Model *m, int layer, int eid) {
    ...
    if (layer < 0 || layer >= 256 || layer <= atomic_load_explicit(&g_cur_moe_layer, memory_order_acquire)) {
        atomic_fetch_add_explicit(
            &g_pilot_drops, 1, memory_order_relaxed); /* fuori range (come il ramo URING) o main gia' su questo layer */
        pthread_mutex_unlock(&g_pilot_mx);
        return;
    }
    ESlot *P = m->pin[layer]; /* gia' residente (pin o ecache)? skip */
#ifdef COLI_VULKAN
    if (vk_reg_served(layer, eid)) {
        pthread_mutex_unlock(&g_pilot_mx);
        return;
    } /* VK-tier-served: no load */
#endif
    for (int z = 0; z < m->npin[layer]; z++)
        if (P[z].eid == eid) {
            pthread_mutex_unlock(&g_pilot_mx);
    ...
    ESlot *Sl = m->ecache[layer];
    int nn = m->ecn[layer]; /* dedup contro residenti E prenotazioni in volo -(eid+2) */
    for (int z = 0; z < nn; z++)
        if (Sl[z].eid == eid || Sl[z].eid == -(eid + 2)) {
            pthread_mutex_unlock(&g_pilot_mx);
            return;
        }
    ...
    if (nn < m->ecap) {
        ...
        isnew = 1;
        m->ecn[layer] = nn + 1;
    } /* cresci: pubblica subito lo slot (marcato prenotato) */
    else {
        slot = -1;
        for (int z = 0; z < nn; z++) {
            if (Sl[z].eid == -1) {
    ...
    int rc = expert_load(m, layer, eid, dst, 0,
                         0); /* pread VERO — fuori dal lock, concorrente fra worker (come i PIPE demand); fatal=0: una
                                speculazione fallita NON uccide il server; demand=0: speculative, never classified. Al
                                successo expert_load setta dst->eid=eid. */

    pthread_mutex_lock(&g_pilot_mx);
    if (rc == 0) {
        dst->used = (uint64_t)__atomic_add_fetch(
            &m->eclock, 1, __ATOMIC_RELAXED); /* eid gia' reale (expert_load); timbra used fresco */
    ...
    if (rc != 0) /* mai swallow silenzioso: logga (una riga) e prosegui */
        fprintf(stderr,
                "[PILOT] load speculativo abbandonato: layer %d expert %d (I/O error/short read) — nessun impatto "
                "sull'output\n",
                layer, eid);
}
#ifdef __linux__
...
static void pilot_uring_batch(Model *m) {
    PilotUringDone done[URING_LOAD_MAX];
    int nd = 0;
    ...
    while (r != w && nd < URING_LOAD_MAX) {
        int layer = atomic_load_explicit(&pilot_q[r & 4095].l, memory_order_relaxed),
            eid = atomic_load_explicit(&pilot_q[r & 4095].e, memory_order_relaxed);
        r++;
        ...
        ESlot *Sl = m->ecache[layer];
        int nn = m->ecn[layer];
        for (int z = 0; z < nn && !found; z++)
            if (Sl[z].eid == eid || Sl[z].eid == -(eid + 2)) found = 1;
        if (found) {
            pthread_mutex_unlock(&g_pilot_mx);
            continue;
        }
    ...
    if (uring_submit_batch(&g_ub_pilot) < 0) {
        ...
        for (int i = 0; i < g_ub_pilot.nload; i++) {
            g_ub_pilot.load[i].error = err;
            g_ub_pilot.load[i].done = 1;
        }
    }
    for (int i = 0; i < nd; i++) {
        ...
        pthread_mutex_unlock(&g_pilot_mx);
        if (rc)
            fprintf(stderr, "[PILOT/URING] load speculativo abbandonato: layer %d expert %d: %s\n", d->layer, d->eid,
                    strerror(g_ub_pilot.load[d->li].error));
    }
}
#endif
...
static int pilot_ring_claim(int *out_l, int *out_e) {
    for (;;) {
        ...
        int l = atomic_load_explicit(&pilot_q[r & 4095].l, memory_order_relaxed),
            e = atomic_load_explicit(&pilot_q[r & 4095].e, memory_order_relaxed);
        if (__atomic_compare_exchange_n(&pilot_r, &r, r + 1, /*weak=*/1, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
            *out_l = l;
            *out_e = e;
            return 1; /* claimed exactly one item, payload valid */
        }
...
static void couple_load(Model *m, const char *path) {
    Cfg *c = &m->c;
    int E = c->n_experts, NL = c->n_layers;
    FILE *f = fopen(path, "rb");
    if (!f) {
        fprintf(stderr, "[COUPLE] cannot open %s\n", path);
        return;
    }
    char magic[16];
    int ver = 0;
    ...
    cp_pred = malloc(cells * sizeof(int16_t));
    cp_cnt = calloc(cells, sizeof(float));
    if (!cp_pred || !cp_cnt) {
        fprintf(stderr, "[COUPLE] OOM\n");
        free(cp_pred);
        free(cp_cnt);
        cp_pred = NULL;
        fclose(f);
...
static void couple_prefetch(Model *m, int layer, const int *idx, int Ke) {
    ...
    for (int dL = 1; dL <= g_couple_d; dL++) {
        ...
        float sc[512];
        memset(sc, 0, (size_t)E * sizeof(float));
        for (int kk = 0; kk < Ke; kk++) {
        ...
        for (int kk = 0; kk < g_couple_k; kk++) {
...
#endif
            for (int z = 0; z < m->npin[lt] && !found; z++)
                if (P[z].eid == best) found = 1;
            ...
            if (!found) {
                ...
                if (w - __atomic_load_n(&pilot_r, __ATOMIC_ACQUIRE) < 4096) {
                    atomic_store_explicit(&pilot_q[w & 4095].l, lt, memory_order_relaxed);
...
static void pilot_prefetch(Model *m, int lnext, const float *x, int S) {
    ...
    int sI = 0;
    int can_two = g_pilot_two && src_layer >= 0 && src_layer < c->n_layers && m->L[src_layer].sparse &&
...
#endif
            for (int z = 0; z < m->npin[lnext] && !found; z++)
                if (P[z].eid == best) found = 1;
...
static int pipe_layer_sparse(Model *m, Layer *l, int li, float *x_dev, int S, int pos_base, float *nrm_host,
                             float *out_host) {
    Cfg *c = &m->c;
    int D = c->hidden, dev = l->kv_b.cuda_device;
    int sI = c->moe_inter * c->n_shared;
    ...
    if (!l->sh_gate.cuda_eligible || !l->sh_up.cuda_eligible || !l->sh_down.cuda_eligible ||
        ...
        l->sh_gate.cuda_device != dev || l->sh_up.cuda_device != dev || l->sh_down.cuda_device != dev)
        return 0;
    ...
    static int lr_idx[64];
    static float lr_w[64];
    static int lr_keff[1];
    int dev_routed = 0;
    if (g_cuda_router && S == 1 && !g_cache_route && g_route_p <= 0.f && !rt_tracing() && c->n_experts <= 4096 &&
        c->topk <= 64 && !l->router_cuda_bad) {
        ...
        if (!l->router_cuda) {
            ...
            if (rw && rb && coli_cuda_pipe_upload(dev, rw, l->router, (size_t)E * D * 4) &&
                ...
                l->router_cuda = rw;
                l->router_bias_cuda = rb;
            } else {
                if (rw) coli_cuda_pipe_free(dev, rw);
                if (rb) coli_cuda_pipe_free(dev, rb);
                l->router_cuda_bad = 1;
            }
        }
        if (l->router_cuda && coli_cuda_pipe_router(dev, nrm_d, l->router_cuda, l->router_bias_cuda, D, E, Ksel, tp,
                                                    c->norm_topk, c->routed_scale, lr_idx, lr_w, lr_keff))
            dev_routed = 1;
    }
    if (!coli_cuda_pipe_download(dev, nrm_d, nrm_host, xb)) return 0;
    m->t_attn += now_s() - ta;
...
static void layer_forward_rows(Model *m, Layer *l, int li, float *x, int S, int pos_base, KVState *const *kvs,
                               const int *positions, float *nrm, float *tmp) {
    Cfg *c = &m->c;
    int D = c->hidden;
    ...
    if (g_metal_enabled && !kvs && S <= 4 && li < c->n_layers && l->sparse &&
        (g_absorb == 1 || (g_absorb < 0 && S <= 4)) && m->kv_start[li] == 0 && D == 6144 && c->n_heads == 64 &&
        c->q_lora == 2048 && c->kv_lora == 512 && c->qk_nope == 192 && c->qk_rope == 64 && c->v_head == 256 &&
        l->kv_b.fmt == 2 && c->n_experts == 256 && c->topk == 8 && c->n_shared == 1 && c->moe_inter == 2048 &&
        metal_fused_fmt_ok(l->q_a.fmt) && metal_fused_fmt_ok(l->q_b.fmt) && metal_fused_fmt_ok(l->kv_a.fmt) &&
        metal_fused_fmt_ok(l->o.fmt) && metal_fused_fmt_ok(l->sh_gate.fmt) && metal_fused_fmt_ok(l->sh_up.fmt) &&
        metal_fused_fmt_ok(l->sh_down.fmt)) {
        int sel_active = m->has_dsa && c->idx_type[li] && (pos_base + S) > c->index_topk;
        if (!sel_active) {
            static float *linrm, *lnrm, *lsh, *lw;
            static int *lidx, *lkeff;
            if (!linrm) {
                linrm = falloc(4 * (int64_t)D);
                lnrm = falloc(4 * (int64_t)D);
                lsh = falloc(4 * (int64_t)D);
                lidx = malloc(4 * 8 * sizeof(int));
                lw = malloc(4 * 8 * sizeof(float));
                lkeff = malloc(4 * sizeof(int));
            }
            int Ksel = g_topk > 0 ? (g_topk < 8 ? g_topk : 8) : 8;
            float tp = (g_topp > 0 && g_topp < 1.f) ? g_topp : 0.f;
            double ta0 = now_s();
#define WP_(q) ((q).fmt == 1 ? (const void *)(q).q8 : (const void *)(q).q4)
            int ok = coli_metal_layer_decode(
                x, l->in_ln, l->post_ln, WP_(l->q_a), l->q_a.s, l->q_a.fmt, l->q_a.gs, l->q_a_ln, WP_(l->q_b), l->q_b.s,
                l->q_b.fmt, l->q_b.gs, WP_(l->kv_a), l->kv_a.s, l->kv_a.fmt, l->kv_a.gs, l->kv_a_ln, WP_(l->kv_b),
                l->kv_b.s, l->kv_b.fmt, WP_(l->o), l->o.s, l->o.fmt, l->o.gs, WP_(l->sh_gate), l->sh_gate.s,
                l->sh_gate.fmt, l->sh_gate.gs, WP_(l->sh_up), l->sh_up.s, l->sh_up.fmt, l->sh_up.gs, WP_(l->sh_down),
                l->sh_down.s, l->sh_down.fmt, l->sh_down.gs, l->router, l->router_bias, c->n_experts, c->topk, Ksel, tp,
                c->norm_topk, c->routed_scale, m->Lc[li], m->Rc[li], S, pos_base, m->kv_start[li], c->eps, c->theta,
                c->attn_scale, linrm, lnrm, lsh, lidx, lw, lkeff);
#undef WP_
            if (ok) {
                m->t_attn += now_s() - ta0;
...
static void layers_forward_rows(Model *m, float *x, int S, int pos_base, KVState *const *kvs, const int *positions) {
    Cfg *c = &m->c;
    int D = c->hidden;
    ...
    int pipe_s_min = getenv("COLI_CUDA_PIPE_S_MIN") ? atoi(getenv("COLI_CUDA_PIPE_S_MIN")) : (g_cuda_ndev <= 1 ? 1 : 8);
...
#ifdef COLI_CUDA
        Layer *l = &m->L[i];
        if (pipe2 && l->sparse && i < c->n_layers && l->q_a.cuda_eligible && l->q_b.cuda_eligible &&
            l->kv_a.cuda_eligible && l->kv_b.cuda_eligible && l->o.cuda_eligible && qt_cuda_upload(&l->q_a) &&
            qt_cuda_upload(&l->q_b) && qt_cuda_upload(&l->kv_a) && qt_cuda_upload(&l->kv_b) && qt_cuda_upload(&l->o) &&
...
static void kv_alloc(Model *m, int max_t) {
...
#ifdef COLI_CUDA
    if (m->kv_dev_L)
        for (int i = 0; i < c->n_layers + 1; i++) { /* dimensioni cambiate: ombra da rifare */
            if (m->kv_dev_L[i]) {
                coli_cuda_pipe_free(m->L[i < c->n_layers ? i : 0].kv_b.cuda_device, m->kv_dev_L[i]);
                m->kv_dev_L[i] = NULL;
            }
            if (m->kv_dev_R[i]) {
                coli_cuda_pipe_free(m->L[i < c->n_layers ? i : 0].kv_b.cuda_device, m->kv_dev_R[i]);
                m->kv_dev_R[i] = NULL;
            }
            m->kv_dev_valid[i] = 0;
        }
...
#endif
    if (k->Lc) {
        for (int i = 0; i < c->n_layers + 1; i++) {
#ifdef COLI_METAL
            if (g_metal_enabled) {
                coli_metal_unregister(k->Lc[i]);
                coli_metal_unregister(k->Rc[i]);
            }
#endif
            free(k->Lc[i]);
            free(k->Rc[i]);
    ...
    if (k->Ic) {
        for (int i = 0; i < c->n_layers; i++) free(k->Ic[i]);
        free(k->Ic);
        k->Ic = NULL;
    }
    if (m->has_dsa) {
        ...
        for (int i = 0; i < c->n_layers; i++)
            if (c->idx_type[i]) k->Ic[i] = falloc((int64_t)max_t * c->index_hd);
    }
    k->max_t = max_t;
    int NR = c->n_layers + 1; /* riga extra: KV del layer MTP */
    k->Lc = calloc(NR, sizeof(float *));
    k->Rc = calloc(NR, sizeof(float *));
    for (int i = 0; i < NR; i++) {
        ...
        if (g_metal_enabled) {
            ...
            free(k->Rc[i]);
            void *lp, *rp;
            if (posix_memalign(&lp, 16384, lb) || posix_memalign(&rp, 16384, rb)) {
                fprintf(stderr, "OOM kv\n");
                exit(1);
            }
            k->Lc[i] = lp;
            k->Rc[i] = rp;
            coli_metal_register(k->Lc[i], lb);
            coli_metal_register(k->Rc[i], rb);
        }
#endif
    }
    m->Lc = k->Lc;
...
static float *step(Model *m, const int *ids, int S, int pos_base) {
    ...
    matmul_qt(logit, last, &m->lm_head, 1);
    m->t_head += now_s() - th0;
    free(x);
    free(last);
    return logit;
...
static float *step_decode_batch(Model *m, const DecodeRow *rows, int S) {
    ...
    float *x = falloc((int64_t)S * D);
    for (int s = 0; s < S; s++) {
        if (!rows[s].kv || !rows[s].kv->Lc || !rows[s].kv->Rc || !rows[s].kv->kv_start || rows[s].token < 0 ||
            rows[s].token >= c->vocab || rows[s].pos < 0 || rows[s].pos >= rows[s].kv->max_t) {
            free(x);
            return NULL;
        }
        for (int l = 0; l < c->n_layers; l++) {
            if (!rows[s].kv->Lc[l] || !rows[s].kv->Rc[l] || rows[s].kv->kv_start[l] < 0 ||
                rows[s].kv->kv_start[l] > rows[s].pos ||
                (m->has_dsa && c->idx_type[l] && (!rows[s].kv->Ic || !rows[s].kv->Ic[l]))) {
                free(x);
                return NULL;
            }
        }
        for (int p = 0; p < s; p++)
            if (rows[p].kv == rows[s].kv) {
                free(x);
                return NULL;
            }
        kvs[s] = rows[s].kv;
        positions[s] = rows[s].pos;
...
static int *g_corp = NULL;
static long g_corp_n = 0;
static int g_corp_k = 0;
...
static void corpus_load(void) {
    ...
    long cap = 1 << 16;
    g_corp = malloc((size_t)cap * sizeof(int));
    g_corp_n = 0;
    int v;
    while (g_corp && fscanf(f, "%d", &v) == 1) {
        if (g_corp_n >= cap) {
            cap *= 2;
            int *n2 = realloc(g_corp, (size_t)cap * sizeof(int));
            if (!n2) {
                free(g_corp);
                g_corp = NULL;
                break;
...
static int corpus_draft(const int *ctx, int nctx, int *out, int k, int minn, int maxn) {
    ...
    for (int n = maxn; n >= minn; n--) {
        ...
        for (long i = g_corp_n - n - 1; i >= 0; i--) {
            int ok = 1;
            for (int j = 0; j < n; j++)
                if (g_corp[i + j] != suf[j] || g_corp[i + j] < 0) {
                    ok = 0;
                    break;
                }
            if (!ok) continue;
...
static int ngram_draft(const int *ids, int len, int G, int *draft) {
    ...
    for (int i = len - 3; i >= 1; i--)
        if (ids[i - 1] == a && ids[i] == b) {
            int n = 0;
            for (int j = i + 1; j < len && n < G; j++) draft[n++] = ids[j];
            return n;
        }
    return 0;
}
...
static int mtp_draft(Model *m, int next_tok, int kv, int G, int *draft) {
    ...
    for (int g = 0; g < G; g++) {
        ...
        if (getenv("MTP_SWAP")) {
            memcpy(cat, h, D * sizeof(float));
            memcpy(cat + D, x, D * sizeof(float));
        } else {
            memcpy(cat, x, D * sizeof(float));
            memcpy(cat + D, h, D * sizeof(float));
        }
        matmul_qt(hx, cat, &m->eh_proj, 1);
        double n_eh = 0;
        for (int d = 0; d < D; d++) n_eh += hx[d] * hx[d];
        ...
        double n_post = 0;
        for (int d = 0; d < D; d++) n_post += hx[d] * hx[d];
        rmsnorm(row, hx, m->mtp_norm, D, c->eps);
        matmul_qt(logit, row, &m->lm_head, 1);
        int t2 = mtp_argmax(logit, c->vocab);
        if (dbg)
            fprintf(stderr, "[mtp2] pos=%d in_tok=%d ||eh||=%.1f ||post||=%.1f pre_blk=%d post_blk=%d\n", pos, tok,
                    sqrt(n_eh), sqrt(n_post), t_pre, t2);
        draft[n++] = t2;
        tok = t2;
        memcpy(h, hx, D * sizeof(float));
    }
    m->ld_ctx = 0;
    free(x);
    free(cat);
...
static void mtp_absorb(Model *m, const int *next_ids, const float *x, int S, int pos_base) {
    ...
    for (int i = 0; i < S; i++) {
        ...
        if (prenorm) rmsnorm(hn, x + (int64_t)i * D, m->hnorm, D, c->eps);
            ...
            rmsnorm(hf, x + (int64_t)i * D, m->final_norm, D, c->eps); /* vLLM: h POST model.norm */
            rmsnorm(hn, hf, m->hnorm, D, c->eps);
        }
        if (getenv("MTP_SWAP")) {
            memcpy(cat, hn, D * sizeof(float));
            memcpy(cat + D, e, D * sizeof(float));
        } else {
            memcpy(cat, e, D * sizeof(float));
            memcpy(cat + D, hn, D * sizeof(float));
        }
...
static int grammar_setup_text(GrDraft *g, Tok *T, char *txt, const char *label) {
    ...
    if (*p == '{') {
        ...
        free(txt);
        if (!gbnf) {
            fprintf(stderr, "[SCHEMA] %s: %s (running without grammar)\n", label, serr);
            return -1;
        }
        txt = gbnf;
    }
    if (gr_parse(&g->gram, txt)) {
...
static void grammar_setup(GrDraft *g, Tok *T) {
    ...
    FILE *f = fopen(path, "rb");
    if (!f) {
        fprintf(stderr, "[GRAMMAR] cannot open %s\n", path);
        return;
    }
    fseek(f, 0, SEEK_END);
    long n = ftell(f);
...
static void gr_feed(GrDraft *g, int t) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (r < 0) {
            g->on = 0;
            return;
        } /* walker spento: fine dei draft */
        if (!g->armed) continue; /* preambolo: aspetta l'inizio */
        gr_state_init(&g->st, &g->gram);
        g->armed = 0; /* desync: riparti dalla radice */
        if (!g->st.alive) {
            g->on = 0;
            return;
        }
...
static int grammar_draft(GrDraft *g, int *draft, int cap) {
   ...
   acceptance sotto il 50% = tokenizzazione fuori asse, meglio spegnersi */
    ...
    char fb[512];
    int nb = gr_forced(&g->st, fb, (int)sizeof fb - 1);
    if (nb <= 0) return 0;
...
static int spec_decode(Model *m, int *all, int kv, int n_new, int eos, float *logit, void (*emit)(int, void *),
                       void *ud, int *kv_out, float **logit_out) {
    Cfg *c = &m->c;
    int V = c->vocab;
    int emitted = 0, done = 0;
    int draft[64];
    ...
    if (spec_pinned() && m->has_mtp) {
        static int once = 0;
        if (!once) {
            once = 1;
            fprintf(
                stderr,
                "[SPEC_PIN] draft+verify pinned to the S=1 kernel family: int4=%s int8=%s (#163; SPEC_PIN=0 for A/B)\n",
    ...
    uint64_t gd_prop0 = m->mtp_prop, gd_acc0 = m->mtp_acc;
    int gd_pause = 0;
    uint64_t cp_prop0 = g_corp_prop, cp_acc0 = g_corp_acc;
    int cp_pause = 0;
    while (emitted < n_new && !done && !g_intr && !g_mux_stop && !g_mux_cancel) {
        /* g_intr / g_mux_*: stessa uscita del tetto n_new (#678) */
        int next = pick_tok(logit, V, carry_ban);
        carry_ban = -1;
        free(logit);
        logit = NULL;
        ...
        if (!g && g_corp && g_corp_k > 0) { /* corpus: uno span intero per forward
            ...
            if (cp_pause > 0) {
                ...
                if (minacc > 100) minacc = 100;
                if (pw >= 24 && aw * 100 < pw * (uint64_t)minacc) {
                    fprintf(stderr,
                            "[CORPUS] %.0f%% acceptance over the last %llu proposals (<%d%%): "
        ...
        if (!g && g_draft > 0 && !(m->has_mtp && gd_pause > 0)) {
            if (m->has_mtp) {
                g = mtp_draft(m, next, kv, g_draft, draft);
                m->mtp_prop += g;
                if (g) gsrc = 2;
            } else {
                g = ngram_draft(all, kv + 1, g_draft, draft);
                if (g) gsrc = 2;
            }
        }
        ...
        if (g > 0 && getenv("MTP_DEBUG")) {
            int veri = argmax_v(lo, V);
        ...
        while (k < g && emitted < n_new) {
            ...
            if (g_temp <= 0) accept = (argmax_v(lo + (int64_t)k * V, V) == draft[k]);
                ...
                dist_build(lo + (int64_t)k * V, V); /* rejection sampling: p(draft) */
                accept = (rndu() < g_pbuf[draft[k]]);
            }
            if (!accept) {
                if (g_temp > 0) carry_ban = draft[k];
                break;
            }
            if ((eos >= 0 && draft[k] == eos) || is_stop(draft[k])) {
                done = 1;
                break;
            }
            emit(draft[k], ud);
...
static void emit_store(int t, void *ud) {
    EmitStore *e = (EmitStore *)ud;
    e->dst[e->n++] = t;
}
...
static void emit_stream(int t, void *ud) {
    EmitStream *e = (EmitStream *)ud;
    char dec[64];
    int dn = tok_decode(e->T, &t, 1, dec, 63);
    dec[dn] = 0;
    fputs(dec, stdout);
    fflush(stdout);
    if (!e->quiet && ++e->count % 16 == 0) {
        double tt = e->m->hits + e->m->miss;
        if (g_cache_route && e->m->route_slots) {
            ...
            fprintf(stderr, "\n[t=%d  RSS %.2f GB  hit %.0f%%  %.2f tok/s  %.2f tok/fw]\n", e->count, rss_gb(),
                    tt ? 100.0 * e->m->hits / tt : 0.0, e->count / (now_s() - e->t0),
                    e->m->n_fw ? (double)e->m->n_emit / e->m->n_fw : 1.0);
        }
    }
}
...
static void dump_top5_logits(int pos, const float *lo, int V, int expected, int got) {
    ...
    fprintf(stderr, "[LOGITS] pos=%d top1-top2 gap=%.3e | expected=%d (%.5f)  got=%d (%.5f) | top5:", pos, gap,
            expected, (expected >= 0 && expected < V) ? (double)lo[expected] : 0.0, got,
            (got >= 0 && got < V) ? (double)lo[got] : 0.0);
...
static void run_score(Model *m, const char *snap, const char *path) {
    ...
    if (!getenv("SCORE_PREFIX") || atoi(getenv("SCORE_PREFIX"))) {
        ...
        if (mt_is_glm(mt ? mt->str : NULL)) {
            ...
            Tok T;
            tok_load(&T, tkp);
            pfx[0] = tok_id_of(&T, "[gMASK]");
            pfx[1] = tok_id_of(&T, "<sop>");
            if (pfx[0] >= 0 && pfx[1] >= 0) {
                pfx_on = 1;
                fprintf(stderr,
                        "[SCORE] GLM snapshot: prepending [gMASK]<sop> (ids %d,%d) to unprefixed requests — disable "
                        "with SCORE_PREFIX=0\n",
                        pfx[0], pfx[1]);
    ...
    int maxT = 1;
    {
        ...
        while (getline(&ln, &cp, f) > 0) {
            int a, b;
            if (sscanf(ln, "%d %d", &a, &b) == 2 && a + b > maxT) maxT = a + b;
        }
        free(ln);
    }
    ...
    while (getline(&ln, &cp, f) > 0) {
        char *p = ln;
        int ctxlen = strtol(p, &p, 10), contlen = strtol(p, &p, 10), T = ctxlen + contlen;
        if (T <= 0 || ctxlen < 1) {
            printf("0 0 0\n");
            fflush(stdout);
            continue;
        ...
        for (int pos = ctxlen - 1; pos < T - 1; pos++) {
            ...
            matmul_qt(lo, row, &m->lm_head, 1);
            int am;
            lp += logprob_target(lo, c->vocab, ids[pos + 1], &am);
            if (!am) greedy = 0;
        }
        printf("%.6f %d %d\n", lp, contlen, greedy);
        fflush(stdout);
        if (++nreq % 5 == 0)
            fprintf(stderr, "[score %d req | %.1fs | RSS %.2f GB | hit %.0f%%]\n", nreq, now_s() - t0, rss_gb(),
...
static void run_ablate_score(Model *m, const char *path) {
    ...
    if (outp && !of) {
        fprintf(stderr, "[ablate] cannot open ABLATE_OUT=%s\n", outp);
    }
    ...
    int tk_id[ABL_LOGIT_TOPK];
    float tk_val[ABL_LOGIT_TOPK];
    rewind(f);
    char *ln = NULL;
    size_t cp = 0;
    int nreq = 0;
    ...
    while (getline(&ln, &cp, f) > 0) {
        char *p = ln, *e;
        long item = strtol(p, &e, 10);
        if (e == p) continue;
        p = e; /* blank line */
        long T = strtol(p, &e, 10);
        if (e == p) {
            fprintf(stderr, "[ablate] bad T\n");
            continue;
        }
        p = e;
        long np = strtol(p, &e, 10);
        if (e == p) {
            fprintf(stderr, "[ablate] bad n_prompt\n");
            continue;
        }
        p = e;
        long mode = strtol(p, &e, 10);
        if (e == p) {
            fprintf(stderr, "[ablate] bad mode\n");
            continue;
        }
        p = e;
        long nc = strtol(p, &e, 10);
        if (e == p) {
            fprintf(stderr, "[ablate] bad ncells\n");
            continue;
        }
        p = e;
        int Ls[ABL_MAX_CELLS], Es[ABL_MAX_CELLS], As[ABL_MAX_CELLS];
        int bad = 0;
        long ncc = nc < 0 ? 0 : (nc > ABL_MAX_CELLS ? ABL_MAX_CELLS : nc);
        ...
        if (bad) {
            fprintf(stderr, "[ablate] ERR item %ld (bad field/token)\n", item);
            continue;
        }
        ...
        if (of)
            for (long pos = (np > 0 ? np - 1 : 0); pos < T - 1; pos++) {
                rmsnorm(row, x + (int64_t)pos * D, m->final_norm, D, c->eps);
                matmul_qt(lo, row, &m->lm_head, 1);
                int gold = ids[pos + 1];
                float mx = lo[0];
                ...
                float molo = -1e30f;
                for (int i = 0; i < V; i++) {
                    if (i != gold && lo[i] > molo) molo = lo[i];
                }
                float mgn = lo[gold] - molo; /* gold-vs-best-competitor logit margin */
                for (int k = 0; k < ABL_LOGIT_TOPK; k++) {
                    tk_id[k] = -1;
                    tk_val[k] = -1e30f;
                }
                for (int i = 0; i < V; i++) {
                    float v = lo[i];
                    int mn = 0;
                    for (int k = 1; k < ABL_LOGIT_TOPK; k++)
                        if (tk_val[k] < tk_val[mn]) mn = k;
                    if (v > tk_val[mn]) {
                        tk_val[mn] = v;
                        tk_id[mn] = i;
                    }
                ...
                fprintf(of,
                        "{\"t\":\"lg\",\"item\":%ld,\"pos\":%ld,\"gold\":%d,\"nll\":%.6f,"
                        "\"glogit\":%.6g,\"molo\":%.6g,\"mgn\":%.6g,\"am\":%d,\"amlogit\":%.6g,"
                        "\"logZ\":%.6f,\"corr\":%d,\"tk\":[",
                        item, pos, gold, gnll, (double)lo[gold], (double)molo, (double)mgn, am, (double)mx, logZ,
                        (am == gold) ? 1 : 0);
                for (int k = 0; k < ABL_LOGIT_TOPK; k++)
                    fprintf(of, "%s[%d,%.5g]", k ? "," : "", tk_id[k], (double)tk_val[k]);
                fprintf(of, "]}\n");
            }
        if (of) fflush(of);
        if (++nreq % 8 == 0)
            fprintf(stderr, "[ablate %d item | %.1fs | RSS %.2f GB | hit %.0f%%]\n", nreq, now_s() - t0, rss_gb(),
                    (m->hits + m->miss) ? 100.0 * m->hits / (m->hits + m->miss) : 0.0);
    }
...
static void profile_print(Model *m, double elapsed) {
    ...
    printf("ATTENTION: projection/RoPE %.3fs | score-softmax-value %.3fs | output projection %.3fs\n", m->t_aproj,
           m->t_acore, m->t_aout);
    if (g_prof)
        printf("P0-EXEC: routed CPU %.3fs / %.2f GB/s (%llu row) | routed GPU critical %.3fs | router %.3fs | residual "
               "P2P %.3fs / %llu hop | orchestration %.3fs\n",
               ...
               elapsed - m->t_ewait - m->t_emm - m->t_attn - m->t_head - m->t_route - m->t_p2p > 0
                   ? elapsed - m->t_ewait - m->t_emm - m->t_attn - m->t_head - m->t_route - m->t_p2p
                   : 0);
#ifdef COLI_VULKAN
    if (g_prof && g_vkb_blocks)
        printf("VK-BLOCK: %lld blocks | avg nvk %.2f (dev2 %.2f) / ncpu %.2f | classify %.3fs | issue %.3fs | take+acc "
               "%.3fs (take-wait %.3fs) | d2-iss wrk %.3fs join %.3fs | cpu exec %.3fs wait %.3fs\n",
               (long long)g_vkb_blocks, (double)g_vkb_nvk / g_vkb_blocks, (double)g_vkb_nvk2 / g_vkb_blocks,
               (double)g_vkb_ncpu / g_vkb_blocks, g_vkb_cls, g_vkb_issue, g_vkb_acc, m->t_egpu, g_vkb_wrk, g_vkb_join,
               m->t_ecpu, m->t_ewait);
#endif
    if (g_mirror) {
        double br[MIR_REPS] = {0}, bt = 0;
        for (int r = 0; r < g_mir_nrep; r++) {
            br[r] = atomic_load_explicit(&g_mir_bytes[r], memory_order_relaxed) / 1e9;
            bt += br[r];
        }
        printf("MIRROR: primary %.2f GB (%lld reads)", br[0],
               (long long)atomic_load_explicit(&g_mir_nread[0], memory_order_relaxed));
        for (int r = 1; r < g_mir_nrep; r++)
            printf(" | mirror%d %.2f GB (%lld reads)", r, br[r],
                   (long long)atomic_load_explicit(&g_mir_nread[r], memory_order_relaxed));
        printf(" — %.0f%% of expert bytes from the mirrors\n", bt > 0 ? 100.0 * (bt - br[0]) / bt : 0.0);
    }
#ifdef COLI_METAL
    if (g_metal_enabled) {
        uint64_t ok = 0, fb = 0, ex = 0;
        double su = 0, gp = 0, sc = 0;
        coli_metal_moe_counts(&ok, &fb, &ex);
        coli_metal_moe_times(&su, &gp, &sc);
        {
            uint64_t aok = 0;
            double aw = 0, ak = 0;
            coli_metal_attn_counts(&aok, &aw, &ak);
            if (aok) {
                double ks = 0, gs = 0;
                coli_metal_attn_lat(&ks, &gs);
                printf("METAL-ATTN: layer GPU %llu | gpu-wall %.2fs (kernel %.2fs | cpu-sched %.2fs gpu-sched %.2fs)\n",
                       (unsigned long long)aok, aw, ak, ks, gs);
            }
        }
        printf("METAL: blocchi GPU %llu | fallback CPU %llu | expert su GPU %llu | setup %.2fs gpu-wall %.2fs (kernel "
               "%.2fs) scatter %.2fs\n",
               (unsigned long long)ok, (unsigned long long)fb, (unsigned long long)ex, su, gp,
               coli_metal_moe_kernel_time(), sc);
        {
            double rsf = 0;
            if (coli_metal_resset_stats(&rsf)) /* E5: printed only when the gate is on */
...
static void profile_reset(Model *m) {
    ...
    m->t_ecpu = m->t_egpu = m->t_route = m->t_p2p = 0;
    m->n_p2p = 0;
    m->cpu_expert_bytes = 0;
    m->cpu_expert_rows = 0;
#ifdef COLI_VULKAN
    g_vkb_cls = g_vkb_issue = g_vkb_acc = g_vkb_wrk = g_vkb_join = 0;
    g_vkb_blocks = g_vkb_nvk = g_vkb_nvk2 = g_vkb_ncpu = 0;
#endif
...
static void prof_report(Model *m, const ProfBase *b, double elapsed, int tokens, FILE *f) {
    ...
    uint64_t nw = g_prof_nlat - b->nlat;
    if (nw > PROF_LAT_CAP) nw = PROF_LAT_CAP; /* ring keeps the tail */
    ...
    if (nw) {
        ...
        if (v) {
            ...
            double p50 = v[(nw - 1) / 2], p90 = v[(uint64_t)((nw - 1) * 0.90)], p99 = v[(uint64_t)((nw - 1) * 0.99)],
                   mx = v[nw - 1];
            fprintf(f,
                    "[PROF] decode forwards: %llu | latency p50 %.1f ms | p90 %.1f ms | p99 %.1f ms | max %.1f ms | "
                    "%.2f tok/forward\n",
                    (unsigned long long)nfw, p50 * 1e3, p90 * 1e3, p99 * 1e3, mx * 1e3, nfw ? (double)nem / nfw : 0.0);
            if (nw >= 32 && p99 > 3 * p50)
                fprintf(f,
                        "[PROF] tail: p99 is %.1fx p50 — the slow forwards are cold-cache expert loads; "
    ...
    int pinned = 0, lru = 0;
    for (int i = 0; i <= c->n_layers; i++) {
        if (m->npin) pinned += m->npin[i];
        if (m->ecn) lru += m->ecn[i];
    }
    ...
    fprintf(f,
            "[PROF] expert I/O: %.3f GB fetched (%.1f MB/token, %.2f GB/s over the run%s) | "
            ...
            g_mmap ? "; COLI_MMAP=1: page cache may serve part" : "", hitp, (unsigned long long)dhp,
            (unsigned long long)dhe, (unsigned long long)dm, tokens > 0 ? (double)dq / tokens : 0.0, io_svc, io_w);
     ...
     * the profile window. */
    {
        uint64_t dcn[2], dcdn[2];
        int64_t dcbytes[2], dcns[2], dcwall[2], wnow[2], dcwall_all, wall_now;
        ...
        if (dcn[DC_COLD] + dcn[DC_WARM]) {
            ...
            fprintf(f,
                    "[PROF] DISK-CLASS (recency split): cold %llu x %.2f GB @ %.2f GB/s-thread, wall %.1fs @ %.2f "
                    "GB/s-wall, avg-conc %.1f (direct %llu/%llu) | "
                    "warm %llu x %.2f GB @ %.2f GB/s-thread, wall %.1fs @ %.2f GB/s-wall, avg-conc %.1f (direct "
                    "%llu/%llu) | "
                    ...
                    (unsigned long long)dcdn[DC_WARM], (unsigned long long)dcn[DC_WARM], dcwall_all / 1e9,
                    100.0 * (dcwall_all / 1e9) / elapsed);
        }
    }
    fprintf(f, "[PROF] resident experts: %d pinned (%.1f GB) + %d in LRU (%.1f GB, cap %d/layer)\n", pinned,
            pinned * eb / 1e9, lru, lru * eb / 1e9, m->ecap);
    ...
    double other = elapsed - io_w - emm - attn - head - route - p2p;
    if (other < 0) other = 0;
    double f_io = io_w / elapsed, f_emm = emm / elapsed, f_attn = attn / elapsed;
    fprintf(f,
            "[PROF] time shares: expert-I/O %.0f%% | expert-matmul %.0f%% | attention %.0f%% | lm_head %.0f%% | other "
            "%.0f%%\n",
            100 * f_io, 100 * f_emm, 100 * f_attn, 100 * head / elapsed, 100 * other / elapsed);
    double slow = ecpu > egpu ? ecpu : egpu, fast = ecpu < egpu ? ecpu : egpu;
    fprintf(f,
            "[PROF] P0 execution: routed CPU %.3fs / %.2f GB/s (%llu row) | routed GPU critical %.3fs | tier straggler "
            "%.2fx | "
            "router %.3fs | residual P2P %.3fs (%llu hop, %.3f ms/hop) | orchestration %.3fs\n",
            ecpu, ecpu > 0 ? cpu_bytes / 1e9 / ecpu : 0.0, (unsigned long long)cpu_rows, egpu,
            fast > 1e-9 ? slow / fast : 0.0, route, p2p, (unsigned long long)np2p, np2p ? p2p * 1e3 / np2p : 0.0,
            other);
    if (f_io >= 0.30) {
        fprintf(f, "[PROF] verdict: I/O-bound — %.0f%% of the time waits on expert reads (hit %.0f%%).", 100 * f_io,
                hitp);
        if (hitp < 90) fprintf(f, " More cache is the lever: raise RAM_GB (or add RAM).");
        else
            fprintf(f, " The cache is already warm — the routed working set streams from disk; a faster disk or a "
                       "bigger pinned tier (PIN_GB) is the lever.");
        if (!g_pipe) fprintf(f, " Try PIPE=1 (overlap reads with matmul).");
        if (!g_direct) fprintf(f, " On NVMe try DIRECT=1.");
        fprintf(f, "\n");
    } else if (f_emm >= 0.40) {
        fprintf(f,
                "[PROF] verdict: compute-bound in expert matmuls (%.0f%%) — more cores/threads help; keep IDOT=1, or "
                "move hot experts to a GPU tier (COLI_CUDA / COLI_METAL).%s\n",
                100 * f_emm, g_mmap ? " Note: with COLI_MMAP=1 page-fault I/O is accounted inside matmul." : "");
    } else if (f_attn >= 0.35) {
        fprintf(f,
                "[PROF] verdict: attention-bound (%.0f%%) — context length is the cost (DSA %s). A lower CTX helps if "
                "the workload allows.\n",
                100 * f_attn, m->has_dsa ? "on" : "not available for this model");
    } else {
        fprintf(f,
                "[PROF] verdict: balanced — no phase dominates (I/O %.0f%%, matmul %.0f%%, attention %.0f%%); this "
                "config is a reasonable fit for this machine.\n",
...
static void run_replay(Model *m, const int *full, int nfull, int np) {
    if (np < 2 || nfull <= np) {
        fprintf(stderr, "REPLAY requires a non-empty prompt and continuation\n");
        return;
    }
    kv_alloc(m, nfull + 2);
    float *logit = step(m, full, np - 1, 0);
    free(logit);
    m->hits = m->miss = m->ereq = m->gpu_expert_calls = 0;
    m->hit_pin = m->hit_ecache = 0;
    m->hit_vk = 0;
    profile_reset(m);
    ProfBase pb;
    prof_base(m, &pb);
    ...
    if (m->gpu_expert_count)
        printf("CUDA expert tier: %d resident experts (%.2f GB) | %llu calls served from VRAM\n", m->gpu_expert_count,
               m->gpu_expert_bytes / 1e9, (unsigned long long)m->gpu_expert_calls);
...
static void run_text(Model *m, const char *snap, const char *prompt, int ngen) {
    ...
    if (g_temp < 0)
        g_temp = 0.7f; /* auto: 0.7, NON l'1.0 ufficiale — la coda della
                        * distribuzione int4 e' rumore di quantizzazione */
    int cap = (int)strlen(prompt) + 16;
    int *pids = malloc((cap + 2) * sizeof(int));
    int np = tok_encode(&T, prompt, (int)strlen(prompt), pids, cap);
    if (np < 1) {
        fprintf(stderr, "prompt is empty after tokenization\n");
        return;
    }
    ...
    int *all = malloc((np + ngen + g_draft + 2) * sizeof(int));
    memcpy(all, pids, np * sizeof(int));
    ...
    printf("\n---\nprefill %d tokens in %.2fs | decode %d tokens in %.2fs (%.2f tok/s) | "
           ...
           m->hit_vk ? ({
               static char vkb[40];
               snprintf(vkb, sizeof(vkb), " + vk %.1f%%", tot ? 100.0 * m->hit_vk / tot : 0.0);
               vkb;
           })
                     : "",
           rss_gb());
    if (g_cache_route && m->route_slots)
        printf(" | swap %.1f%% (%llu/%llu)", 100.0 * m->route_swaps / m->route_slots,
    ...
    printf("experts loaded/token: %.1f (per-layer %.2f across %d; baseline topk=%d) | TOPK=%d TOPP=%.2f",
           produced ? (double)m->ereq / produced : 0.0, (produced && nsp) ? (double)m->ereq / produced / nsp : 0.0, nsp,
           c->topk, g_topk, g_topp);
    if (g_cache_route)
        printf(" | CACHE_ROUTE J=%d M=%d P=%.2f alpha=%.2f", g_route_j, g_route_m, g_route_p, g_route_alpha);
    if (g_expert_budget) {
        printf(" | EXPERT_BUDGET=%d (dropped %lld experts, ~%.1f GB I/O saved)", g_expert_budget,
               (long long)g_budget_dropped, g_budget_dropped * 18.9e6 / 1e9);
        if (g_budget_rescued)
            printf(" [%lld rescued: budget too tight, position would have had 0 routed experts]",
                   (long long)g_budget_rescued);
    }
    printf("\n");
    printf("speculation: %.2f tokens/forward (%llu forwards per %llu tokens) | MTP acceptance %.0f%% (%llu/%llu)\n",
           ...
           m->mtp_prop ? 100.0 * m->mtp_acc / m->mtp_prop : 0.0, (unsigned long long)m->mtp_acc,
           (unsigned long long)m->mtp_prop);
    if (g_corp_prop)
        printf("corpus drafts: %.0f%% acceptance (%llu/%llu proposed from %ld frozen ids)\n",
               100.0 * g_corp_acc / g_corp_prop, (unsigned long long)g_corp_acc, (unsigned long long)g_corp_prop,
               g_corp_n);
    if (g_cp_enq) printf("couple: %ld cross-layer prefetch hints enqueued\n", g_cp_enq);
    if (g_grd.prop)
        printf("grammar: %.0f%% acceptance (%llu/%llu forced drafts)\n", 100.0 * g_grd.acc / g_grd.prop,
               (unsigned long long)g_grd.acc, (unsigned long long)g_grd.prop);
    if (g_disk_split)
        printf("disk-load split: draft %llu + absorb %llu + verify/main %llu misses | "
               ...
               (unsigned long long)(m->miss - m->miss_draft - m->miss_absorb), (unsigned long long)m->ld_mtp,
               m->bytes_mtp / 1e9, (unsigned long long)m->ld_main, m->bytes_main / 1e9,
    ...
    if (g_pilot_real)
        printf("PILOT_REAL: %ld load cross-layer completati, %ld scartati (main gia' sul layer) | PILOT_K=%d\n",
    ...
    if (g_pilot_two)
        printf("PILOT_TWO: two-step shared-expert-corrected prefetch active (3 extra matmuls/prediction)\n");
    if (g_looka) {
        const char *nm[4] = {"previous token (=SPEC prefetch)", "layer input, skip attention",
                             "next layer (PILOT, stale)", "next layer (two-step, shared-expert)"};
        printf("LOOKAHEAD routing — recall of true experts in predicted top-8:\n");
        for (int i = 0; i < 4; i++)
            printf("  %-42s %5.1f%%  (%lld/%lld)\n", nm[i], la_tot[i] ? 100.0 * la_hit[i] / la_tot[i] : 0.0,
                   (long long)la_hit[i], (long long)la_tot[i]);
    }
...
static int repin_pick(Model *m, RepinCand *out, int maxc) {
    ...
    for (int l = 0; l < c->n_layers; l++) {
        ...
        if (!tier_pick_lfru(m->eheat[l], m->elast[l], m->eaccess_clock, c->n_experts, ids, np, &zp, &eu, &g)) continue;
        if (nb < maxc) {
            out[nb] = (RepinCand){g, l, zp, eu, 0};
            nb++;
        } else {
            int w = 0;
            for (int b = 1; b < maxc; b++)
...
static void rss_guard(Model *m) {
    ...
    Cfg *c = &m->c;
    int64_t need = (int64_t)((rss - lim) * 1e9), freed = 0;
    int dropped = 0;
    ...
    if (dropped)
        fprintf(stderr,
                "[RAM-GUARD] RSS %.1f GB over the %.1f GB budget (#403): "
    ...
    for (int b = 0; b < nb; b++) {
        ...
        if (cd[b].gpu_swap) {
            ...
            for (int z = 0; z < m->npin[cd[b].l]; z++)
                if (m->pin[cd[b].l][z].eid == cd[b].eid) {
                    hot = &m->pin[cd[b].l][z];
                    break;
                }
        ...
        expert_load(m, cd[b].l, cd[b].eid, s, 1,
                    0); /* disk -> RAM, same resident slot; demand=0: repin, never classified */
        const char *tier = "RAM";
#ifdef COLI_CUDA
        if (gpu) { /* refresh the same VRAM slot now, not lazily */
            if (qt_cuda_upload(&s->g) && qt_cuda_upload(&s->u) && qt_cuda_upload(&s->d)) {
                int64_t now_gpu = (int64_t)coli_cuda_tensor_bytes(s->g.cuda) +
                                  (int64_t)coli_cuda_tensor_bytes(s->u.cuda) +
                                  (int64_t)coli_cuda_tensor_bytes(s->d.cuda);
                m->gpu_expert_bytes += now_gpu - old_gpu;
                tier = "VRAM";
        ...
        fprintf(stderr, "[REPIN] %s layer %d: evict %d (heat=%u) <- admit %d (heat=%u) in %.0f ms\n", tier, cd[b].l,
                old, old_heat, cd[b].eid, new_heat, (now_s() - t0) * 1e3);
    }
    if (gpu_swaps)
        fprintf(stderr, "[REPIN] VRAM: %d expert scambiati/swapped in %.0f ms\n", gpu_swaps, (now_s() - pass_t0) * 1e3);
    for (int l = 0; l < m->c.n_layers; l++)
        if (m->eheat[l]) tier_decay(m->eheat[l], m->c.n_experts);
}
...
static void serve_ctx_init(Model *m, ServeCtx *s, const char *snap, int slot, int maxctx) {
    ...
    if (!s->hist) {
        fprintf(stderr, "OOM serve_ctx_init hist\n");
        exit(1);
    }
    s->first = 1;
...
static void serve_ctx_free(Model *m, ServeCtx *s) {
    ...
    if (k->Ic)
        for (int i = 0; i < m->c.n_layers; i++) free(k->Ic[i]);
    free(k->Lc);
    free(k->Rc);
    free(k->Ic);
    free(k->kv_start);
...
static void mux_data(Tok *T, unsigned long long id, int token) {
    char out[256];
    int n = tok_decode(T, &token, 1, out, sizeof(out));
    printf("DATA %llu %d\n", id, n);
        ...
        if (!strncmp(line, "STOP ", 5) && sscanf(line + 5, "%llu %c", &cid, &tail) == 1 && cid) {
            ...
            if (coli_submit_parse(line, &sub)) {
                ...
                while (left > 0) {
                    ...
                    size_t got = fread(buf, 1, take, stdin);
                    if (!got) break;
                    left -= (long long)got;
                }
...
static void mux_done(Model *m, ServeCtx *sc, ServeReq *r) {
    ...
    printf("PROF %.3f %d %d %.3f %.3f %.3f %.3f %.3f %llu\n", dt, r->prompt_tokens, r->emitted, edisk_s() - r->pb.edisk,
           m->t_ewait - r->pb.ewait, m->t_emm - r->pb.emm, m->t_attn - r->pb.attn, m->t_head - r->pb.head,
           (unsigned long long)(m->n_fw - r->pb.n_fw));
    printf("DONE %llu STAT %d %.2f %.1f %.2f %d %d\n", r->id, r->emitted, r->emitted / dt,
           (dh + dm) > 0 ? 100.0 * dh / (dh + dm) : 0.0, rss_gb(), r->prompt_tokens, r->length_limited);
    fflush(stdout);
    kv_bind(m, &sc->kv);
...
static int mux_submit(Model *m, Tok *T, ServeCtx *ctx, ServeReq *req, GrDraft *grd, int nctx, int maxctx, int eos) {
    char *line = NULL;
    size_t cap = 0;
    ssize_t nr = getline(&line, &cap, stdin);
    if (nr < 0) {
    ...
    if (!strncmp(line, "STOP ", 5)) {
        ...
        for (int i = 0; i < nctx; i++)
            if (req[i].active && req[i].id == id) {
                mux_done(m, &ctx[i], &req[i]);
                free(line);
                return 0;
            }
        printf("ERROR %llu NOT_FOUND\n", id);
    ...
    if (!strncmp(line, "CANCEL ", 7)) {
        ...
        for (int i = 0; i < nctx; i++)
            if (req[i].active && req[i].id == id) {
                ...
                kv_bind(m, &ctx[i].kv);
                if (req[i].spec_logit) {
                    free(req[i].spec_logit);
                    req[i].spec_logit = NULL;
                }
                req[i].spec = 0;
    ...
    ColiSubmit sub;
    int valid = coli_submit_parse(line, &sub);
    if (!valid) {
        printf("ERROR 0 BAD_REQUEST\n");
        fflush(stdout);
        free(line);
    ...
    if (!raw) {
        fprintf(stderr, "OOM multiplex payload\n");
        exit(1);
    }
    if (fread(raw, 1, (size_t)sub.bytes, stdin) != (size_t)sub.bytes) {
        free(raw);
        free(line);
        return -1;
    }
    ...
    for (int i = 0; i < nctx; i++)
        if (req[i].active && req[i].id == sub.id) {
    ...
    if (!tmp) {
        fprintf(stderr, "OOM mux_submit tmp\n");
        free(raw);
        free(line);
        exit(1);
    }
    ...
    if (nt > maxctx - 2) {
        free(tmp);
        fprintf(stderr,
                "[API] prompt does not fit: >=%d token, context is %d (CTX=%d). "
    ...
    fflush(stdout);
    int prefix = 0;
    while (prefix < sc->len && prefix < nt && sc->hist[prefix] == tmp[prefix]) prefix++;
    if (prefix < sc->len) {
        sc->len = prefix;
    ...
    if (kvshare && nctx > 1 && prefix >= sc->len) {
        ...
        if (best >= 0) {
            ServeCtx *dn = &ctx[best];
            Cfg *cc = &m->c;
            int NR = cc->n_layers + 1;
            int from = sc->len, n = blen - from;
            for (int l = 0; l < NR; l++) {
                if (sc->kv.Lc[l] && dn->kv.Lc[l])
                    memcpy(coli_kv_row(sc->kv.Lc[l], from, cc->kv_lora), coli_kv_row(dn->kv.Lc[l], from, cc->kv_lora),
                           (size_t)n * cc->kv_lora * sizeof(float));
                if (sc->kv.Rc[l] && dn->kv.Rc[l])
                    memcpy(coli_kv_row(sc->kv.Rc[l], from, cc->qk_rope), coli_kv_row(dn->kv.Rc[l], from, cc->qk_rope),
                           (size_t)n * cc->qk_rope * sizeof(float));
                if (sc->kv.Ic && dn->kv.Ic && sc->kv.Ic[l] && dn->kv.Ic[l])
                    memcpy(coli_kv_row(sc->kv.Ic[l], from, cc->index_hd), coli_kv_row(dn->kv.Ic[l], from, cc->index_hd),
    ...
    free(tmp);
    float *logit =
        add > 0 ? step(m, sc->hist + sc->len, add, sc->len) : step(m, sc->hist + sc->len - 1, 1, sc->len - 1);
    sc->len += add;
    sc->first = 0;
    ServeReq *r = &req[sub.slot];
    memset(r, 0, sizeof(*r));
    r->id = sub.id;
    r->maximum = sub.max_tokens;
    r->temp = sub.temperature;
    r->top_p = sub.top_p;
    r->prompt_tokens = nt;
    r->started = now_s();
    r->hits0 = m->hits;
    ...
    if (r->maximum > room) r->maximum = room;
    g_temp = r->temp;
    g_nuc = r->top_p;
    ...
    m->n_emit++;
    if (grd[sub.slot].on) {
        grammar_reset(&grd[sub.slot]);
        gr_feed(&grd[sub.slot], next);
    }
    mux_data(T, r->id, next);
    if (r->emitted >= r->maximum) {
        r->length_limited = 1;
        mux_done(m, sc, r);
    }
    return 1;
...
static void run_serve_mux(Model *m, const char *snap) {
    ...
    if (nctx < 1 || nctx > 512) {
        fprintf(stderr, "KV_SLOTS must be between 1 and 512\n");
        exit(2);
    }
    ...
    if (nctx > 1) g_draft = 0;
    else if (g_draft > 0) fprintf(stderr, "[MTP] single-slot serve: speculation active (draft=%d)\n", g_draft);
    g_kvsave = getenv("KVSAVE") ? atoi(getenv("KVSAVE")) : 1;
    KVState *initial = m->kv;
    free(initial->kv_start);
    free(initial);
    ServeCtx *ctx = calloc(nctx, sizeof(*ctx));
    ServeReq *req = calloc(nctx, sizeof(*req));
    GrDraft *grd = calloc(nctx, sizeof(*grd)); /* per-slot request grammars (SUBMIT 7th field) */
    for (int i = 0; i < nctx; i++) {
        serve_ctx_init(m, &ctx[i], snap, i, maxctx);
        grd[i].max = 24;
    }
#ifdef _WIN32
    ...
    for (;;) {
        if (g_intr) {
            g_intr = 0; /* stop morbido: ogni request attiva finisce ORA per la
                         * via normale di mux_done (DONE+stats+KV coerenti) */
            for (int i = 0; i < nctx; i++)
                if (req[i].active) mux_done(m, &ctx[i], &req[i]);
        }
        int active = 0;
        for (int i = 0; i < nctx; i++) active += req[i].active;
        ...
        for (int i = 0; i < nctx; i++)
            if (req[i].active) {
                ...
                if (r->spec) {
                    ...
                    g_temp = r->temp;
                    g_nuc = r->top_p;
                    float *lg = r->spec_logit;
                    r->spec_logit = NULL; /* spec_decode takes ownership */
                    MuxEmit ud = {&T, r->id};
                    g_mux_stop = 0;
                    g_mux_cancel = 0; /* fresh per turn (#678) */
                    int prod = spec_decode(m, sc->hist, sc->len, r->maximum - r->emitted, eos, lg, mux_spec_emit, &ud,
                                           &sc->len, NULL);
                    r->emitted += prod;
                    if (g_mux_cancel) {
                ...
                if (k > 0 && sc->len + 1 + k < maxctx - 1) {
                    ...
                    if (k < 1) {
                        rows[S] = (DecodeRow){&sc->kv, r->pending, sc->len};
                        slots[S++] = i;
                        continue;
                    }
                    kv_bind(m, &sc->kv);
                    ...
                    float *lo = step_all(m, seq, 1 + k, sc->len);
                    m->n_fw++;
                    gd->prop += (uint64_t)k;
                    int done = 0;
        ...
        double tf0 = g_prof ? now_s() : 0;
        float *lo = step_decode_batch(m, rows, S);
        if (!lo) {
            fprintf(stderr, "decode batch failed\n");
            break;
        }
        m->n_fw++;
        if (g_prof) prof_lat(now_s() - tf0);
    ...
    usage_save(m);
    for (int i = 0; i < nctx; i++) {
        serve_ctx_free(m, &ctx[i]);
        grammar_teardown(&grd[i]);
    }
    free(ctx);
    free(req);
    free(grd);
...
la storia e' gia' in KV, basta ri-forwardare l'ULTIMO token per riavere i logits */
            ...
            int cur = ngen;
            if (len + cur + g_draft + 2 >= maxctx) cur = maxctx - len - g_draft - 2;
            uint64_t h0 = m->hits, ms0 = m->miss;
            double tt0 = now_s();
            ProfBase pb;
            if (g_prof) prof_base(m, &pb);
            ...
            repin_pass(m);
            continue;
        } /* RFC: re-pin a caldo tra i turni / live re-pin between turns */
        if (nr < 1) {
            printf("\x01\x01"
                   "END"
                   "\x01\x01\n");
        ...
        if (!strncmp(line, "\x02PROMPT ", 8)) {
            ...
            if (nf < 4 || nb > (16u << 20) || req_ngen < 1 || rt < 0 || rt > 2 || rp <= 0 || rp > 1 || slot < 0 ||
                slot >= nctx) {
                printf("\x01\x01"
                       "END"
                       "\x01\x01\n");
            ...
            if (!raw) {
                fprintf(stderr, "OOM raw prompt\n");
                exit(1);
            }
            if (fread(raw, 1, (size_t)nb, stdin) != (size_t)nb) {
                free(raw);
                break;
            }
            int delim = fgetc(stdin);
        ...
        if (raw_mode) {
            ...
            k = tok_encode(&T, buf, bl, hist + len, maxctx - len);
            prompt_tokens = k;
            if (len + k + 8 + g_draft >= maxctx) {
                ...
                k = tok_encode(&T, buf, bl, hist, maxctx);
        ...
        first = 0;
        int cur = req_ngen;
        if (len + k + cur + g_draft + 2 >= maxctx) cur = maxctx - len - k - g_draft - 2;
        uint64_t h0 = m->hits, ms0 = m->miss;
        uint64_t rs0 = m->route_slots, rw0 = m->route_swaps;
        uint64_t agh0 = m->route_agree_hit, agt0 = m->route_agree_tot;
        uint64_t kln0 = m->route_kl_n;
        double kls0 = m->route_kl_sum;
        double tt0 = now_s();
        ProfBase pb;
        if (g_prof) prof_base(m, &pb);
        float *logit;
        if (k > 0) {
            logit = step(m, hist + len, k, len);
            len += k;
        ...
        uint64_t kl_n = m->route_kl_n - kln0;
        double kl_sum = m->route_kl_sum - kls0;
        ...
        printf("%s\x01\x01"
               ...
               "\x01\x01\n",
               raw_mode ? "" : "\n");
        printf("STAT %d %.2f %.1f %.2f %d %d", prod, prod / tdt, (dh + dm) > 0 ? 100.0 * dh / (dh + dm) : 0.0, rss_gb(),
        ...
        if (g_cache_route || rslots || ag_tot)
            printf(" swap_pct=%.1f route_swaps=%llu route_slots=%llu"
                   ...
                   swap_pct, (unsigned long long)rswaps, (unsigned long long)rslots, agree_pct, kl_mean);
        printf("\n");
        fflush(stdout);
        if (g_prof) prof_report(m, &pb, tdt, prod, stderr); /* per-turn window; stdout is the framed protocol */
        free(raw);
...
static void vk_dense_preload(Model *m) {
    ...
    for (int i = 0; i <= c->n_layers && !full; i++) {
        ...
        for (size_t k = 0; k < sizeof(ts) / sizeof(ts[0]); k++) {
            ...
            bytes += coli_vk_tensor_bytes(t->vk);
            nt++;
        }
    }
...
static void vk_registry_fill(Model *m) {
    ...
    if (!nz) {
        fprintf(stderr,
                "[VK] expert tier: no usage history yet — tier empty this run "
    ...
    g_vk_reg = calloc((size_t)NL * E * 3, sizeof(*g_vk_reg));
    g_vk_reg_E = E;
    g_vk_reg_NL = NL;
    if (!g_vk_reg) {
        free(cand);
    ...
    int vkr_stopped = 0;
    double vkr_used = 0, vkr_budget = 0;
    int64_t i2 = 0;
    ...
    for (; i2 < n && g_vk_reg_n < g_vk_budget; i2++) {
        if (vkr_reserve > 0 && (g_vk_reg_n & 7) == 0 && coli_vk_mem_budget(&vkr_used, &vkr_budget) &&
            vkr_budget - vkr_used < vkr_reserve) {
            vkr_stopped = 1;
            break;
        }
        int layer = cand[i2].layer, eid = cand[i2].eid;
        tried++;
        ESlot *src = NULL, *P = m->pin[layer];
        for (int z = 0; z < m->npin[layer]; z++)
            if (P[z].eid == eid && P[z].slab) {
                src = &P[z];
                break;
            }
        if (!src) {
            if (expert_load(m, layer, eid, &tmp, 0, 0) !=
                0) { /* 0 = success (impl convention); demand=0: startup tier fill */
        ...
        if ((xf != 2 && xf != 4 && xf != 5) || src->u.fmt != xf || src->u.gs != src->g.gs ||
            (src->d.fmt != 2 && src->d.fmt != 4 && src->d.fmt != 5) || (xf == 4 && (src->g.gs < 8 || src->g.gs % 8)) ||
            (src->d.fmt == 4 && (src->d.gs < 8 || src->d.gs % 8))) {
            if (tried < 4)
                fprintf(stderr, "[VK] tier fill: (%d,%d) fmt %d/%d/%d not int4/int3-g64 (src=%s)\n", layer, eid,
                        src->g.fmt, src->u.fmt, src->d.fmt, src == &tmp ? "load" : "pin");
            continue;
        }
        ...
        if (!coli_vk_tensor_ensure(&slot[0], src->g.q4, src->g.s, xf, c->hidden, c->moe_inter, src->g.gs) ||
            ...
            if (slot[0]) {
                coli_vk_tensor_free(slot[0]);
                slot[0] = NULL;
            }
            if (slot[1]) {
                coli_vk_tensor_free(slot[1]);
                slot[1] = NULL;
            }
    ...
    fprintf(stderr, "[VK] expert tier: %d hot experts resident (%.2f GB VRAM, %.1fs, top-%d of history)\n", g_vk_reg_n,
            bytes / 1e9, now_s() - t0, tried);
    if (vkr_stopped)
        fprintf(stderr,
                "[VK] expert tier: budget stop at %d experts — %.1f of %.1f GB device-local used, %.1f GB reserved "
                "(COLI_VK_RESERVE_GB)\n",
    ...
    if (g_vk_budget2 > 0 && coli_vk_dev2_available()) {
        ...
        for (; i2 < n && g_vk_reg_n2 < g_vk_budget2; i2++) {
            if (vkr2_reserve > 0 && (g_vk_reg_n2 & 7) == 0 && coli_vk_mem_budget2(&u2, &b2) && b2 - u2 < vkr2_reserve) {
                vkr2_stopped = 1;
                break;
            }
            int layer = cand[i2].layer, eid = cand[i2].eid;
            ...
            if (!src) {
                if (expert_load(m, layer, eid, &tmp, 0, 0) != 0) {
            ...
            if (!coli_vk_tensor_ensure2(&slot[0], src->g.q4, src->g.s, xf, c->hidden, c->moe_inter, src->g.gs) ||
                ...
                !coli_vk_tensor_ensure2(&slot[2], src->d.q4, src->d.s, src->d.fmt, c->moe_inter, c->hidden,
                                        src->d.gs)) {
                if (slot[0]) {
                    coli_vk_tensor_free(slot[0]);
                    slot[0] = NULL;
        ...
        fprintf(stderr, "[VK] dev2 tier: %d experts resident (%.2f GB VRAM, %.1fs, next-%d of history)\n", g_vk_reg_n2,
                bytes2 / 1e9, now_s() - t20, tried2);
        if (vkr2_stopped)
            fprintf(stderr,
                    "[VK] dev2 tier: budget stop at %d experts — %.1f of %.1f GB device-local used, %.1f GB reserved "
                    "(COLI_VK_RESERVE2_GB)\n",
                    g_vk_reg_n2, u2, b2, vkr2_reserve);
    }
    if (tmp.slab) {
        compat_aligned_free(tmp.slab);
        free(tmp.fslab);
    }
    free(cand);
}
...
static void qt_unwire_mmap(QT *t) {
    ...
    if (!t->q8 && !t->q4) return;
    int64_t weight_b, scale_b;
    qt_wire_split(t, &weight_b, &scale_b);
...
static void qt_wire_mmap(QT *t, int64_t *wired, long *failed) {
    ...
    void *wp = t->q8 ? (void *)t->q8 : (void *)t->q4;
    if (weight_b > 0) {
        if (mem_wire(wp, (size_t)weight_b) == 0) *wired += weight_b;
        else (*failed)++;
    }
    if (t->s && scale_b > 0) {
        if (mem_wire(t->s, (size_t)scale_b) == 0) *wired += scale_b;
        else (*failed)++;
    }
}
static void pin_wire(Model *m) {
    ...
    if (g_mmap) {
        ...
        fprintf(stderr, "[PIN] mlock (mmap): %.1f GB wired in physical RAM%s in %.0fs\n", g_mmap_wired / 1e9,
                g_mmap_wire_failed ? " (some allocations failed -- raise: ulimit -l unlimited)" : "", now_s() - t0);
        return;
    }
    Cfg *c = &m->c;
    double t0 = now_s();
    ...
    for (int i = 0; i < c->n_layers; i++)
        for (int z = 0; z < m->npin[i]; z++) {
            ESlot *s = &m->pin[i][z];
            if (s->slab) {
                if (mem_wire(s->slab, s->slab_cap) == 0) wired += s->slab_cap;
                else failed++;
            }
            if (s->fslab) {
                size_t fl = (size_t)s->fslab_cap * sizeof(float);
                if (mem_wire(s->fslab, fl) == 0) wired += fl;
                else failed++;
            }
        }
    if (failed)
        fprintf(stderr,
                "[PIN] mlock: %.1f GB wired, %ld allocations failed "
                "(raise the limit: ulimit -l unlimited) in %.0fs\n",
                wired / 1e9, failed, now_s() - t0);
    else
        fprintf(stderr,
                "[PIN] mlock: %.1f GB wired in physical RAM "
...
static double mirror_probe_bw(shards *S, int rep) {
    ...
    if (dfd < 0)
        fprintf(stderr,
                "[MIRROR] no O_DIRECT on replica %d: the probe may read the page cache — "
...
static void mirror_setup(Model *m) {
    ...
    if (!snap || !*snap) snap = getenv("COLI_MODEL");
    st_mirror_reset(&m->S);
    int nrep = 1;
    {
        char buf[4096];
        snprintf(buf, sizeof(buf), "%s", g_mirror_dir);
        char *p = buf;
        while (p && *p) {
            char *sep = p;
            ...
            *sep = 0;
            ...
            if (*p) {
                if (snap && !strcmp(snap, p)) fprintf(stderr, "[MIRROR] %s equals the model dir — ignored\n", p);
    ...
    double wt[MIR_REPS];
    int have = 0;
    const char *w = getenv("COLI_DISK_WEIGHTS");
    const char *how = "COLI_DISK_WEIGHTS";
    if (w && *w) {
        char wb[256];
        snprintf(wb, sizeof(wb), "%s", w);
        int wn = 0, bad = 0;
        ...
        if (!bad && wn == nrep) have = 1;
            ...
            fprintf(stderr,
                    "[MIRROR] invalid COLI_DISK_WEIGHTS '%s' (want %d positive comma-separated "
    ...
    if (!have) {
        ...
        for (int r = 0; r < nrep; r++) {
            wt[r] = mirror_probe_bw(&m->S, r);
            if (wt[r] <= 0) have = 0;
        }
        if (have) {
            fprintf(stderr, "[MIRROR] probe:");
            for (int r = 0; r < nrep; r++)
                fprintf(stderr, "%s %s %.2f GB/s", r ? " |" : "", r ? "mirror" : "primary", wt[r]);
            fprintf(stderr, "\n");
        } else {
            for (int r = 0; r < nrep; r++) wt[r] = 1;
            how = "fallback 1:1 (probe failed)";
        }
    }
    double W = 0;
    for (int r = 0; r < nrep; r++) W += wt[r];
    int acc = 0;
    double cum = 0;
    for (int r = 0; r < nrep; r++) {
        cum += wt[r];
...
static void pin_arena_bind(Model *m, PinRec *r, int *slot_of, int from, int to) {
    ...
    Cfg *c = &m->c;
    int NR = c->n_layers + 1;
    int *cnt = calloc((size_t)NR, sizeof(int));
    int *first = malloc((size_t)NR * sizeof(int));
    if (!cnt || !first) {
        free(cnt);
        free(first);
        return;
    ...
    for (int a = from; a < to; a++) {
        if (first[r[a].l] < 0) first[r[a].l] = a;
        cnt[r[a].l]++;
    }
    const char suf[3][16] = {"gate_proj", "up_proj", "down_proj"}; /* bounded suf: see #484 */
    for (int l = 0; l < NR; l++) {
        if (cnt[l] < 2) continue;
        int64_t wtot = 0, qtot = 0;
        int ok = 1;
        ...
        if (posix_memalign((void **)&af, 4096, (size_t)cnt[l] * fs)) {
            free(aw);
            continue;
        }
        ...
        for (int a = from; a < to; a++) {
            ...
            s->slab = aw + (size_t)i * ws;
            s->slab_cap = (int64_t)ws;
            s->aslab = s->slab;
            s->fslab = (float *)((uint8_t *)af + (size_t)i * fs);
            s->fslab_cap = (int64_t)(fs / sizeof(float));
            s->afslab = s->fslab;
            i++;
        }
    }
...
static void pin_load(Model *m, const char *statspath, double gb, int trusted) {
    ...
    PinRec *r = malloc((size_t)cap * sizeof(PinRec));
    int n = 0;
    ...
    {
        PinCollect pc = {m, r, &n, cap, seen};
        rt_read_ex(statspath, pin_collect_cb, &pc, trusted);
    }
    ...
    if (fill)
        for (int li = 0; li <= c->n_layers; li++) {
            ...
            if (sparse)
                for (int ei = 0; ei < c->n_experts; ei++)
                    if (!seen[(int64_t)li * c->n_experts + ei]) r[n++] = (PinRec){li, ei, 0};
        }
    free(seen);
    ...
    if (g_cuda_enabled && (g_cuda_expert_gb > 0 || g_cuda_expert_auto))
        for (int i = 0; i < g_cuda_ndev; i++) {
    ...
    for (int i = 0; i <= c->n_layers; i++)
        if (cnt_l[i]) m->pin[i] = calloc(cnt_l[i], sizeof(ESlot));
    ...
    if (g_cuda_enabled && g_cuda_release_host && gpu_prefix > 0 && budget > 0) {
        ...
        double cap = 4e9;
        if (budget / 8.0 < cap) cap = budget / 8.0;
        int st = (int)(cap / eb);
        if (st < 1) st = 1;
        if (st < stage) stage = st;
        if (stage < pre_n)
            fprintf(stderr,
                    "[CUDA] tier staging: %d experts per round (%.1f GB host peak) "
...
#pragma omp parallel for schedule(dynamic, 1)
            ...
            for (int a = base; a < hi && m->gpu_expert_bytes < budget; a++) {
                int li = r[a].l;
                {
                    ...
                    for (int attempt = 0; attempt < g_cuda_ndev && !placed; attempt++) {
                        int best = -1;
                        for (int i = 0; i < g_cuda_ndev; i++)
                            if (!tried[i] && remaining[i] >= projected && (best < 0 || placed_b[i] < placed_b[best]))
                                best = i;
                        if (best < 0) break;
                        tried[best] = 1;
                        ...
                        if (uploaded) {
                            ...
                            remaining[best] -= actual;
                            placed_b[best] += actual;
                            placed_n[best]++;
                            if (g_cuda_release_host) expert_host_release(m, s);
                            placed = 1;
                        } else {
    ...
    if (g_cuda_enabled && budget > 0) {
        fprintf(stderr, "[CUDA] hot expert tier: %d/%d experts, VRAM %.2f GB (budget %.1f GB%s, reserve %.1f GB)\n",
                ...
                g_cuda_expert_auto ? safe_total / 1e9 : budget / 1e9, g_cuda_expert_auto ? ", auto" : "",
                g_cuda_reserve_gb);
        for (int i = 0; i < g_cuda_ndev; i++)
            fprintf(stderr, "[CUDA]   device %d: %d experts, %.2f GB\n", g_cuda_devices[i], placed_n[i],
                    placed_b[i] / 1e9);
        ...
        if (g_cuda_ndev > 0 && m->gpu_expert_bytes > 0 && g_mem_avail_boot > 0 &&
            ...
            fprintf(stderr,
                    "[CUDA] integrated/unified memory: expert tier shares physical RAM; "
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    fprintf(stderr, "[PIN] placement: %d VRAM + %d RAM expert (%.1f GB warm) in %.0fs da %s\n", m->gpu_expert_count,
            npin - m->gpu_expert_count, (npin - m->gpu_expert_count) * eb / 1e9, now_s() - t0, statspath);
...
static double kv_pool_bytes(Model *m, int max_ctx) {
    Cfg *c = &m->c;
    double one = (double)(c->n_layers + 1) * max_ctx * (c->kv_lora + c->qk_rope) * 4.0;
    if (m->has_dsa)
        for (int i = 0; i < c->n_layers; i++)
            if (c->idx_type[i]) one += (double)max_ctx * c->index_hd * 4.0;
    int slots = kv_slot_count();
    if (slots < 1 || slots > 16) slots = 1;
    return one * slots;
}
...
static double expert_avail(Model *m, double ram_gb, int ebits, int max_ctx) {
    ...
    if (ram_gb <= 0) {
        ram_gb = g_mem_avail_boot * 0.88;
        if (ram_gb < 4) ram_gb = 8;
    }
    double ws_b =
        (g_expert_budget > 0 && g_expert_budget < 64) ? (double)(g_expert_budget + 4) * (double)eb : 64.0 * (double)eb;
    double slack = 1.2e9 + 2.5e9 + ws_b + kv_pool_bytes(m, max_ctx) +
...
static void cap_for_ram(Model *m, double ram_gb, int ebits, int max_ctx) {
    ...
    int auto_b = ram_gb <= 0;
    if (auto_b) {
        ram_gb = g_mem_avail_boot * 0.88; /* misurata PRIMA del load: il residente gia'
                                           * allocato viene sottratto sotto, non due volte */
        if (ram_gb < 4) {
            fprintf(stderr, "[RAM] MemAvailable is unreadable or too low; assuming 8 GB\n");
            ram_gb = 8;
        }
    }
    ...
    if (floored) {
        ...
        fprintf(stderr,
                "[RAM_GB=%.1f%s] WARNING: cap=1 is the floor, projected peak %.1f GB is "
                "%.1f GB OVER the budget (resident %.1f GB + reserve %.1f GB, expert %.1f MB).%s\n",
                ram_gb, auto_b ? " auto" : "", peak / 1e9, (peak - ram_gb * 1e9) / 1e9, m->resident_bytes / 1e9,
                slack / 1e9, (double)eb / 1e6,
        ...
        if (g_cuda_enabled && g_cuda_ndev == 1 && !g_cuda_release_host && (g_cuda_expert_gb > 0 || g_cuda_expert_auto))
        ...
        if (g_mem_avail_boot > 0 && peak > g_mem_avail_boot * 1e9 &&
            ...
            fprintf(stderr,
                    "[RAM] refusing to start: that peak also exceeds the %.1f GB actually "
    ...
    if (capmax < m->ecap) {
        fprintf(stderr,
                "[RAM_GB=%.1f%s] resident %.1f GB + reserve %.1f GB (ws %.1f, KV %dx%d %.1f, kvb %.1f), "
                "experts %.1f MB x %d layers -> cap lowered %d->%d (projected peak %.1f GB)\n",
                ram_gb, auto_b ? " auto" : "", m->resident_bytes / 1e9, slack / 1e9, ws_b / 1e9, kv_slot_count(),
                max_ctx, kv_b / 1e9, kvb_b / 1e9, eb / 1e6, nsp, m->ecap, capmax,
        ...
        if (raise_on && newcap > m->ecap) {
            ...
            fprintf(stderr,
                    "[RAM_GB=%.1f%s] cap raised %d->%d: budget allows it "
            ...
            fprintf(stderr, "[RAM_GB=%.1f%s] cap=%d ok (projected peak %.1f GB)\n", ram_gb, auto_b ? " auto" : "",
                    m->ecap, (m->resident_bytes + (double)m->ecap * nsp * eb + slack) / 1e9);
    }
}
...
static const char *coli_user_prompt(void) {
    ...
    if (p)
        for (const char *q = p; q[0]; q++)
            if (q[0] == '$' && q[1] && strchr("ABCDEFGHLNPQSTV_+|$", q[1] & ~0x20)) {
                p = NULL;
                break;
            }
#endif
...
static void prof_config(Model *m, double ram_env, int est_ctx) {
    ...
    double eb = (double)expert_bytes_probe(m, m->ebits);
    fprintf(stderr,
            "[PROF] machine: %s | %d cores (%d omp threads) | RAM %.1f GB total, %.1f GB available | backend %s\n",
            cpu[0] ? cpu : "unknown CPU", cores, omp_get_max_threads(), rt, ra, backend);
    fprintf(stderr,
            "[PROF] config: RAM_GB=%s%.1f CTX=%d | expert cache cap %d/layer (up to %.1f GB) | pinned %d (%.1f GB) | "
            "DRAFT=%d PIPE=%d DIRECT=%d MMAP=%d IDOT=%d DSA=%s PILOT=%d CACHE_ROUTE=%d\n",
            ram_env <= 0 ? "auto " : "", ram_env <= 0 ? g_mem_avail_boot * 0.88 : ram_env, est_ctx, m->ecap,
            (double)m->ecap * rows * eb / 1e9, pinned, pinned * eb / 1e9, g_draft, g_pipe, g_direct, g_mmap, g_idot,
...
static int coli_ssd_probe_underallocated(long long allocated, long long size) { return allocated < size - size / 8; }
...
static int coli_ssd_cache_parse(const char *buf, size_t len, double *gbs_out, unsigned long long *stdev_out) {
...
static int coli_ssd_cache_format(char *buf, size_t bufsz, double gbs, unsigned long long stdev) {
...
static size_t coli_ssd_probe_cold_tiles(const unsigned char *vec, size_t npages, size_t page_sz, long long *tiles,
...
static int coli_ssd_probe_vetoed(long long cold_bytes) { return cold_bytes < COLI_SSD_PROBE_COLD_FLOOR; }
...
static double coli_ssd_probe_raw(const char *snap_dir, int *veto_out) {
    ...
    struct stat st;
    if (fstat(fd, &st) != 0 || st.st_size < (off_t)(COLI_SSD_PROBE_BLK * 2)) {
        close(fd);
        return -1;
    }
...
static double coli_ssd_probe_cached(const char *snap_dir) {
    ...
    if (f) {
        ...
        if (coli_ssd_cache_parse(buf, n, &cached, &sdev) == 2 && sdev == cur_dev) return cached;
    ...
    if (veto != COLI_SSD_VETO_NONE) {
        ...
        const char *why = veto == COLI_SSD_VETO_SPARSE  ? "the shard is not fully allocated on disk (partial "
        ...
        fprintf(stderr,
                "METAL: storage probe deferred -- %s; keeping the conservative cache "
    ...
    if (!(gbs > 0 && gbs < 1000)) {
        ...
        fprintf(stderr,
                "METAL: storage probe measured an implausible %g GB/s -- ignoring the "
    ...
    if (coli_ssd_cache_format(line, sizeof(line), gbs, cur_dev) > 0) {
        ...
        if (tfd < 0) cache_errno = errno;
            ...
            mode_t um = umask(0);
            umask(um); /* read it back; umask() has no getter */
        ...
        if (cache_errno)
            ...
            fprintf(stderr,
                    "METAL: could not cache the storage probe in %s (%s) -- "
...
static int coli_env_on(const char *name) {
    ...
    if (!v || !*v) return 0;
    return !(strcmp(v, "0") == 0 || strcmp(v, "false") == 0 || strcmp(v, "off") == 0 || strcmp(v, "no") == 0);
}
...
int main(int argc, char **argv) {
     ...
     * misurato (#707). */
    if (!getenv("COLI_OMP_TUNED") && !getenv("COLI_NO_OMP_TUNE") && !coli_env_on("COLI_CUDA") &&
        !coli_env_on("COLI_METAL")) {
    ...
    if (g_expert_budget > 0 && !getenv("EXPERT_BUDGET_EXPERIMENTAL")) {
        fprintf(stderr,
                "[EXPERT_BUDGET] ignored: measured empty operating window (issue #303).\n"
    ...
    if (g_cache_route)
        fprintf(stderr, "[CACHE_ROUTE] on J=%d M=%d P=%.2f alpha=%.2f (pin∪LRU prefer; never default)\n", g_route_j,
                g_route_m, g_route_p, g_route_alpha);
    if (g_route_agree) fprintf(stderr, "[ROUTE_AGREE] telemetry on (overlap%% + mean KL vs true top-K)\n");
    ...
    const char *policy = getenv("COLI_POLICY");
    ...
    g_mlock = getenv("MLOCK") ? atoi(getenv("MLOCK"))
                              : -1; /* -1 auto (ON macOS), 0 off, 1 force / auto (ON macOS), 0 off, 1 force */
    g_spec = getenv("SPEC") ? atoi(getenv("SPEC")) : 1;
    g_draft = getenv("DRAFT") ? atoi(getenv("DRAFT")) : -1;
    g_no_fused_pair =
        getenv("COLI_NO_FUSED_PAIR") ? atoi(getenv("COLI_NO_FUSED_PAIR")) : 0; /* -1 = auto: 3 se MTP, 0 senza */
    ...
    g_pilot_real = getenv("PILOT_REAL")
                       ? atoi(getenv("PILOT_REAL"))
                       : 0; /* default OFF: load VERI cross-layer (value-preserving prefetch); PILOT_REAL=1 opta in */
    if (g_pilot_real) g_pilot = 1; /* PILOT_REAL implica il pilota attivo */
    g_pilot_two = getenv("PILOT_TWO")
                      ? atoi(getenv("PILOT_TWO"))
                      : 0; /* 1 = two-step: shared-expert-corrected router prediction (+2.3% recall, 3 extra matmuls) */
    ...
    if (g_pilot_nw > 16) g_pilot_nw = 16;
    g_pilot_evict_guard =
        getenv("PILOT_EVICT_GUARD") ? atoi(getenv("PILOT_EVICT_GUARD")) : 1; /* 0 = old LRU eviction (A/B) */
    g_disk_split = getenv("DISK_SPLIT") ? atoi(getenv("DISK_SPLIT")) : 0;    /* 1 = split dei disk load nelle stats */
    g_pipe =
        getenv("PIPE") ? atoi(getenv("PIPE")) :
#ifdef _WIN32
    ...
    if (pipe_workers_imply_pipe(getenv("PIPE"), getenv("PIPE_WORKERS"), g_pipe)) {
        g_pipe = 1;
        fprintf(stderr,
                "[PIPE] PIPE_WORKERS is set — enabling the async pipe (PIPE=1 implied; set PIPE=0 to override)\n");
    }
    g_pipe_nw = getenv("PIPE_WORKERS") ? atoi(getenv("PIPE_WORKERS")) : 8; /* I/O worker threads */
    if (g_pipe_nw < 1) g_pipe_nw = 1;
    g_pipe_block =
        getenv("COLI_PIPE_BLOCK") ? atoi(getenv("COLI_PIPE_BLOCK")) : 0; /* blocking pipe_wait (default: spin) */
    g_direct = getenv("DIRECT") ? atoi(getenv("DIRECT")) : 0;
    {
        const char *dh = getenv("COLI_DISKCLASS_WINDOW"); /* DISK-CLASS recency window, see its declaration */
        if (dh) {
            g_direct_heat_ticks = (uint32_t)strtoul(dh, NULL, 10);
            g_direct_heat_explicit = 1;
        }
    }
...
#ifdef __linux__
        if (g_mmap) {
            fprintf(stderr, "URING=1 is incompatible with COLI_MMAP=1\n");
            return 2;
        }
        g_pipe = 1;
    ...
    g_spec_pin = getenv("SPEC_PIN") ? atoi(getenv("SPEC_PIN"))
                                    : 1; /* #163: 0 = gate S-dipendenti storici / legacy S-dependent gates */
    corpus_load();                       /* COLI_DRAFT_CORPUS: external draft source */
    rt_trace_open();                     /* same place as before, so the log order is identical */
    g_repin = getenv("REPIN")
                  ? atoi(getenv("REPIN"))
                  : 0; /* RFC: re-pin ogni n token emessi (0=off) / live re-pin every n emitted tokens (0=off) */
    g_absorb = getenv("ABSORB") ? atoi(getenv("ABSORB")) : -1; /* -1 auto: assorbita per S<=4 */
    g_metal_prefill = getenv("COLI_METAL_PREFILL")
                          ? atoi(getenv("COLI_METAL_PREFILL"))
                          : 0; /* default 0: S>4 attention on CPU (bit-exact); =1 opt-in GPU prefill */
    ...
    g_nuc = getenv("NUCLEUS") ? atof(getenv("NUCLEUS"))
                              : 0.90f; /* piu' stretto dell'ufficiale 0.95: la coda int4 e' rumore */
    if (getenv("SEED")) g_rng = (uint64_t)atoll(getenv("SEED")) * 0x9E3779B97F4A7C15ULL + 1;
    else {
        struct timespec ts;
        clock_gettime(CLOCK_MONOTONIC, &ts);
        g_rng ^= (uint64_t)ts.tv_nsec << 20 ^ (uint64_t)getpid();
    }
    ...
    if (getenv("COLI_CUDA") && atoi(getenv("COLI_CUDA"))) {
        ...
        if (one && many) {
            fprintf(stderr, "use COLI_GPU or COLI_GPUS, not both\n");
            return 2;
        }
        ...
        if (g_cuda_ndev < 1) {
            fprintf(stderr, "invalid COLI_GPUS: use a list such as 0,1,2\n");
            return 2;
        }
        g_cuda_enabled = coli_cuda_init(g_cuda_devices, g_cuda_ndev);
        if (!g_cuda_enabled) {
            fprintf(stderr, "[CUDA] requested backend is unavailable\n");
            return 2;
        }
    ...
    if (getenv("COLI_VULKAN") && atoi(getenv("COLI_VULKAN"))) {
        char spvbuf[512];
        const char *spv = coli_vk_resolve_spv(spvbuf, sizeof(spvbuf));
        g_vulkan = coli_vk_init(spv);
        if (!g_vulkan) {
            fprintf(stderr,
                    "[VK] Vulkan backend unavailable (tried %s; need libvulkan + "
        ...
        fprintf(stderr, "[VK] expert tier active: routed quantized experts on the GPU (budget %d)%s%s\n", g_vk_budget,
                g_vk_dense ? " + dense projections + shared expert" : "", g_vk_attn ? " + absorb attention core" : "");
...
#endif
    if (!getenv("REPIN") && g_cuda_expert_auto && getenv("PIN_GB") && !strcmp(getenv("PIN_GB"), "all")) g_repin = 16;
#ifdef COLI_ANS
    ...
    if ((getenv("COLI_GPU") || getenv("COLI_GPUS")) && !g_cuda_enabled) {
        fprintf(stderr, "COLI_GPU(S) requires COLI_CUDA=1\n");
        return 2;
    }
    if (g_cuda_dense && !g_cuda_enabled) {
        fprintf(stderr, "CUDA_DENSE requires COLI_CUDA=1\n");
        return 2;
    }
    if ((g_cuda_expert_gb > 0 || g_cuda_expert_auto) && !g_cuda_enabled) {
        fprintf(stderr, "CUDA_EXPERT_GB requires COLI_CUDA=1\n");
        return 2;
    }
    if (g_cuda_enabled)
        fprintf(stderr, "[CUDA] mode: routed experts%s%s\n",
                g_cuda_dense ? " + resident dense tensors" : " only (resident dense on CPU)",
                g_cuda_release_host ? "; VRAM experts without host backing" : "");
#else
    if ((getenv("COLI_CUDA") && atoi(getenv("COLI_CUDA"))) || getenv("COLI_GPU") || getenv("COLI_GPUS") ||
    ...
    if (getenv("COLI_METAL") && atoi(getenv("COLI_METAL"))) {
        ...
        if (!g_metal_enabled) {
            fprintf(stderr, "[METAL] backend requested but not available\n");
            return 2;
        }
        fprintf(stderr, "[METAL] mode: batched routed experts on GPU (unified-memory zero-copy)\n");
        if (getenv("COLI_METAL_SPIN") && atoi(getenv("COLI_METAL_SPIN"))) {
            coli_metal_spin_start();
            fprintf(stderr, "[METAL] keep-alive spinner ON\n");
        }
    ...
    if (g_ssd_fast && !cap_explicit)
        fprintf(stderr,
                "METAL: fast SSD (%.1f GB/s) — page cache favored, expert cache minimal (cap 1); override with --cap\n",
                coli_ssd_gbs);
    ...
    Model m;
    double t0 = now_s();
    model_init(&m, snap, cap, ebits, dbits);
    ...
    int mux_will_disable_mtp = getenv("SERVE") && getenv("SERVE_BATCH") && atoi(getenv("SERVE_BATCH")) && mux_slots > 1;
    int eff_draft = mux_will_disable_mtp ? 0 : g_draft;
    printf("loaded in %.2fs | resident dense: %.2f MB | layers=%d experts=%d | MTP %s (draft=%d)\n", now_s() - t0,
           m.resident_bytes / (1024.0 * 1024.0), m.c.n_layers, m.c.n_experts,
    ...
    if (mux_will_disable_mtp && m.has_mtp)
        ...
        fprintf(stderr, "[MTP] %s (draft=%d)\n", m.has_mtp ? "active: native speculative decoding" : "absent",
                eff_draft);
#ifdef __linux__
    { /* Only warn for a GENUINE 9p mount (WSL Windows drives, magic 0x01021997), where
       ...
       * false-positives on native-Linux ZFS/ext4/xfs/NFS mounts that also live under /mnt. */
        ...
        if (statfs(snap, &sfb) == 0 && (unsigned long)sfb.f_type == 0x01021997UL)
            fprintf(stderr,
                    "WARNING: the model is on %s (9p/Windows filesystem; fadvise is ineffective).\n"
    ...
    if (getenv("PIN")) {
        ...
        if (!strcmp(pin, "auto")) {
            ...
            if (psz <= 0) {
                snprintf(pauto, sizeof(pauto), "%s/stats.txt", snap);
                pf = fopen(pauto, "rb");
                psz = 0;
                if (pf) {
                    fseek(pf, 0, SEEK_END);
            ...
            if (psz > 0) {
                pin = pauto;
                fprintf(stderr, "[PIN] auto: seeding from %s\n", pauto);
            } else {
                pin = NULL;
                fprintf(stderr, "[PIN] auto: no .coli_usage or stats.txt in %s yet (no pin this run)\n", snap);
            }
        }
        if (pin) {
            const char *pin_gb = getenv("PIN_GB");
            pin_load(&m, pin,
                     pin_gb && !strcmp(pin_gb, "all") ? -1.0
                     : pin_gb                         ? atof(pin_gb)
                                                      : 10.0,
                     pin_named); /* PIN_GB=all (#80) */
        }
    }
    ...
    if (getenv("COUPLE") && *getenv("COUPLE")) { /* coupling-scored cross-layer prefetch (#176) */
        ...
        if (g_couple_k < 1) g_couple_k = 1;
        if (g_couple_k > 32) g_couple_k = 32;
        g_couple_d = getenv("COUPLE_D") ? atoi(getenv("COUPLE_D")) : 1;
        if (g_couple_d < 1) g_couple_d = 1;
        if (g_couple_d > 2) g_couple_d = 2;
    ...
    if (getenv("ABLATE_SCORE")) {
        run_ablate_score(&m, getenv("ABLATE_SCORE"));
    ...
    if (!b) {
        fprintf(stderr, "%s: cannot read oracle file (missing, unreadable, short, or > %lld bytes)\n", refpath,
                (long long)CFG_MAX_BYTES);
        return 1;
    }
    char *ar = NULL;
    jval *ref = json_parse(b, &ar);
    int np = 0, nfull = 0;
    int *prompt = read_arr(ref, "prompt_ids", &np);
    int *full = read_arr(ref, "full_ids", &nfull);
    if (!prompt || !full || np < 1 || nfull < np) {
        fprintf(stderr, "ref file missing prompt_ids/full_ids or empty\n");
        return 1;
    }
    int n_new = nfull - np;
     ...
     * Non e' un bug del motore — vedi #76. */
    {
        ...
        if (m.c.vocab > 1000 && maxid < 1000 && !getenv("REF_FORCE")) {
            fprintf(stderr,
                    ...
                    "         Engine self-test:  SNAP=./glm_tiny TF=1 ./glm 64 16 16      (expect ~30-32/32; FP "
                    "near-ties are toolchain-dependent)\n"
    ...
    if (getenv("TF")) {
        ...
        double tt = now_s();
        forward_all(&m, full, nfull, pred, tf);
        double tdt = now_s() - tt;
        int ok = 0;
        for (int i = 0; i < nfull; i++) {
            if (pred[i] == tf[i]) ok++;
        ...
        if (ok < nfull)
            fprintf(stderr, "[ORACLE] %d/%d mismatches — run: TF=1 DEBUG_LOGITS=1 for top-5 logit dump\n", nfull - ok,
                    nfull);
        profile_print(&m, tdt);
#ifdef COLI_CUDA
        if (g_cuda_enabled) cuda_stats_print();
