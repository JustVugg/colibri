...
static inline unsigned coli_uring_load_acquire(unsigned *p) { return __atomic_load_n(p, __ATOMIC_ACQUIRE); }
static inline void coli_uring_store_release(unsigned *p, unsigned v) { __atomic_store_n(p, v, __ATOMIC_RELEASE); }
...
static inline int coli_uring_init(ColiUring *r, unsigned entries) {
    ...
    if (r->p.features & IORING_FEAT_SINGLE_MMAP) {
        ...
        r->sq_map =
            mmap(NULL, r->sq_map_sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, r->fd, IORING_OFF_SQ_RING);
        if (r->sq_map == MAP_FAILED) {
            r->sq_map = NULL;
            coli_uring_close(r);
            return -1;
        ...
        r->cq_map =
            mmap(NULL, r->cq_map_sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, r->fd, IORING_OFF_CQ_RING);
...
static inline int coli_uring_prep_read(ColiUring *r, int fd, void *buf, size_t len, int64_t off, uint64_t user_data) {
