...
static void topp_siftdown(int *h, int n, int i) {
    ...
    for (;;) {
        ...
        if (l >= n) break;
        int b = l;
        if (l + 1 < n && g_pbuf[h[l + 1]] > g_pbuf[h[l]]) b = l + 1;
...
static void dist_build(const float *lo, int V) {
    if (!g_pbuf) {
        g_pbuf = falloc(V);
        g_pidx = malloc(V * sizeof(int));
    }
    int mxi = -1;
    float mx = 0;
    for (int i = 0; i < V; i++)
        if (isfinite(lo[i]) && (mxi < 0 || lo[i] > mx)) {
            mx = lo[i];
            mxi = i;
        }
    double s = 0;
    ...
    if (mxi < 0 || !isfinite(s) || s <= 0.0) {
        ...
        if (!warned) {
            warned = 1;
            fprintf(stderr, "[SAMPLE] warning: non-finite logits (NaN/Inf) — falling back to argmax; "
...
static void stops_arm_tok(const Cfg *c, int tok_eos, Tok *T) {
    ...
    int nsp = 0;
    if (T)
        for (int id = 0; id < T->n_ids && g_nstop < 64; id++)
            if (T->id_special[id] && !is_stop(id)) {
                g_stop[g_nstop++] = id;
                nsp++;
            }
    ...
    if (getenv("SERVE") && getenv("SERVE_BATCH") && atoi(getenv("SERVE_BATCH")) && tok_eos >= 0 &&
        !getenv("COLI_SERVE_ALL_STOPS")) {
        int kept = 0;
        for (int i = 0; i < g_nstop; i++)
            if (g_stop[i] == tok_eos) g_stop[kept++] = g_stop[i];
        if (kept < g_nstop)
            fprintf(stderr, "[stop] batched serve mode: filtered %d non-EOS stop tokens (tool-call safety, #401)\n",
                    g_nstop - kept);
        g_nstop = kept;
        nsp = 0;
    }
