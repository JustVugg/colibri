...
typedef struct {
    ...
    int gs, qbits;
    int64_t qn;
} Wt; /* qn = byte in q4, per il guard OOB */
...
typedef struct {
    ...
    uint8_t *p13, *p2;
    float *s13, *s2;  /* container: packed rows + row scales */
...
typedef struct {
    ...
    float **K, **V;
    int kv_len, max_t; /* per-layer [kv][max_t][hd] */
...
static void matmul_i4g(float *y, const float *x, const uint8_t *p, const float *scale, int S, int I, int O, int gs) {
    int ng = (I + gs - 1) / gs;
    int64_t rb = (I + 1) / 2;
#pragma omp parallel for schedule(static)
    for (int o = 0; o < O; o++) {
...
static void matmul_i8r(float *y, const float *x, const int8_t *q, const float *scale, int S, int I, int O) {
#pragma omp parallel for schedule(static)
    for (int o = 0; o < O; o++) {
...
#pragma omp parallel for schedule(static)
        for (int o = 0; o < O; o++) {
            ...
            float acc = 0.f;
            for (int b = 0; b < nb; b++) {
                __m256i vacc = i8dot_block(_mm256_setzero_si256(), _mm256_loadu_si256((const __m256i *)(xi + b * 32)),
...
#pragma omp parallel for schedule(static)
        for (int o = 0; o < O; o++) {
            ...
            for (int b = 0; b < nb; b++) {
                ...
                __m256i vacc =
                    i8dot_block(_mm256_setzero_si256(), _mm256_loadu_si256((const __m256i *)(xi + b * 32)), nib);
...
static void load_cfg(Cfg *c, const char *snap) {
    ...
    jval *r = json_get(root, "text_config");
    if (!r) r = root;
    ...
    if (c->n_layers > MAXL) {
        fprintf(stderr, "n_layers %d > MAXL\n", c->n_layers);
        exit(1);
    }
...
static int load_w_quant(Model *m, const char *name, int64_t orig_numel, Wt *out) {
    ...
    char qn[352];
    snprintf(qn, sizeof(qn), "%s.qs", name);
    ...
    *out = w;
    m->q_loaded++;
    m->q_bytes += t->nbytes + (int64_t)s->numel * 4;
    return 1;
}
...
static void wt_deq_row(Wt w, int64_t row, float *out, int I) {
    if (w.qbits == 8) {
        const int8_t *q = (const int8_t *)w.q4 + row * I;
        float s = w.qs[row];
...
static void wt_row_f32(Wt w, int64_t off, float *out, int n) {
    if (w.q4) {
        wt_deq_row(w, off / n, out, n);
        return;
    } /* n = dim di contrazione */
...
static void read_f32_slice(shards *S, const char *name, float *out, int64_t off, int64_t cnt) {
    ...
    if (t->dtype == 3) {
        fprintf(stderr, "%s: U8 container has no f32 view\n", name);
        exit(1);
    }
    int esz = (t->dtype == 2) ? 4 : 2;
    void *raw = malloc((size_t)cnt * esz);
    if (!raw) {
        fprintf(stderr, "OOM slice %s\n", name);
        exit(1);
    }
    if (pread(t->fd, raw, (size_t)cnt * esz, t->off + off * esz) != (ssize_t)(cnt * esz)) {
        perror("pread slice");
        exit(1);
    }
    if (t->dtype == 2) memcpy(out, raw, (size_t)cnt * 4);
        ...
        uint16_t *p = raw;
        for (int64_t i = 0; i < cnt; i++) out[i] = bf16_to_f32(p[i]);
    } else {
        uint16_t *p = raw;
        for (int64_t i = 0; i < cnt; i++) out[i] = f16_to_f32(p[i]);
    }
    free(raw);
...
static void read_u8_slice(shards *S, const char *name, uint8_t *out, int64_t boff, int64_t nb) {
    ...
    if (pread(t->fd, out, (size_t)nb, t->off + boff) != (ssize_t)nb) {
        perror("pread u8 slice");
        exit(1);
    }
...
static void model_init(Model *m, const char *snap, int cap, int bits) {
    ...
    {
        char qd[2100];
        snprintf(qd, sizeof(qd), "%s/dense-int4g64", snap);
...
#define LD(field, suffix)                                                                                              \
    snprintf(nm, sizeof(nm), "model.layers.%d." suffix, i);                                                            \
    l->field = load_t(m, nm)
#define LDW(field, suffix)                                                                                             \
    snprintf(nm, sizeof(nm), "model.layers.%d." suffix, i);                                                            \
    l->field = load_w(m, nm, 1)
        LD(in_ln, "input_layernorm.weight");
        LD(post_ln, "post_attention_layernorm.weight");
        LDW(q, "self_attn.q_proj.weight");
        LDW(k, "self_attn.k_proj.weight");
        LDW(v, "self_attn.v_proj.weight");
        LDW(r, "self_attn.r_proj.weight");
        ...
        if (!c->sparse[i]) {
            LDW(dg, "mlp.gate_proj.weight");
            LDW(du, "mlp.up_proj.weight");
            LDW(dd, "mlp.down_proj.weight");
            snprintf(nm, sizeof(nm), "model.layers.%d.mlp.global_scale", i);
            l->dgs = load_scalar(m, nm, 1.f);
        } else {
            LD(router, "mlp.gate.weight");
            LD(rbias, "mlp.gate.e_score_correction_bias");
            snprintf(nm, sizeof(nm), "model.layers.%d.mlp.gate.global_scale", i);
            l->rgs = load_scalar(m, nm, 1.f);
...
#ifdef COLI_METAL
            if (g_metal) {
                coli_metal_register(wsl, wlen);
                coli_metal_register(ssl, slen);
            }
#endif
        }
    }
...
static double mem_avail_bytes(void) {
    ...
    if (host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info64_t)&vs, &n) != KERN_SUCCESS) return 0;
...
static Slot *slot_acquire(Model *m, int layer, int eid) {
    ...
    if (lc->n < lc->cap) {
        ...
        if (lru < 0) {
            fprintf(stderr, "layer %d: cache cap %d entirely pinned\n", layer, lc->cap);
            exit(1);
        }
        s = &lc->slots[lru];
    }
...
static void attention(Model *m, Layer *l, int li, float *x, int S, int pos0, float *out) {
    ...
    for (int s = 0; s < S; s++) {
        for (int h = 0; h < H; h++)
            rmsnorm_row(q + (int64_t)s * qdim + h * hd, q + (int64_t)s * qdim + h * hd, l->qn, hd, c->eps);
        for (int h = 0; h < KV; h++)
            rmsnorm_row(k + (int64_t)s * kvdim + h * hd, k + (int64_t)s * kvdim + h * hd, l->kn, hd, c->eps);
    }
...
static void dense_mlp(Model *m, Layer *l, float *x, int S, float *out) {
    Cfg *c = &m->c;
    int D = c->hidden, I = c->dense_inter;
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out) {
    ...
    for (int s = 0; s < S; s++) {
        ...
        float sum = 0.f;
        for (int kk = 0; kk < K; kk++) {
            w[kk] = sigmoidf(lg[si[kk]]);
            sum += w[kk];
        }
        for (int j = 0; j < ns; j++) {
            w[K + j] = sigmoidf(lg[E + j]);
            sum += w[K + j];
        }
    ...
    float *mxg = NULL, *mrw = NULL;
    const void **mgp = NULL, **mup = NULL, **mdp = NULL;
    const float **mgs = NULL, **mus = NULL, **mds = NULL;
    Slot **mslot = NULL;
    ...
    if (g_metal && m->xq && S >= metal_min_s) {
        mxg = falloc((int64_t)cap * D);
        mrw = falloc(cap);
        mgp = malloc(cap * sizeof(void *));
        mup = malloc(cap * sizeof(void *));
        mdp = malloc(cap * sizeof(void *));
        mgs = malloc(cap * sizeof(float *));
        mus = malloc(cap * sizeof(float *));
        mds = malloc(cap * sizeof(float *));
        mslot = malloc(cap * sizeof(Slot *));
        mxoff = malloc((cap + 1) * sizeof(int));
        mnr = malloc(cap * sizeof(int));
        mrows = malloc(cap * sizeof(int));
        mgi = malloc(cap * sizeof(int));
        mfp = malloc(cap * sizeof(int));
    }
#endif
        ...
        if (mxg) {
            ...
            if (nb) {
                ...
                for (int j = 0; j < nb; j++) {
                    Slot *e = mslot[j];
                    mgp[j] = e->p13;
                    mup[j] = e->p13 + (int64_t)I * m->rb13;
                    mdp[j] = e->p2;
                    mgs[j] = e->s13;
                    mus[j] = e->s13 + I;
                    mds[j] = e->s2;
                }
                if (coli_metal_moe_block(nb, D, I, q4 ? 2 : 1, mgp, mup, mdp, mgs, mus, mds, mxg, mxoff, mnr, mrows,
                                         mrw, out, S)) {
                    m->t_expert += now_s() - te;
...
static float *step_mm(Model *m, const int *ids, int S, int pos0, int *tf_out, const uint8_t *dmel, int naud) {
    Cfg *c = &m->c;
    int D = c->hidden;
...
static void kv_alloc(Model *m, int max_t) {
    ...
    for (int i = 0; i < c->n_layers; i++) {
        ...
        for (int h = 0; h < kv && keep; h++) {
            memcpy(m->K[i] + (int64_t)h * max_t * hd, oldK[i] + (int64_t)h * old_max * hd,
                   (size_t)keep * hd * sizeof(float));
            memcpy(m->V[i] + (int64_t)h * max_t * hd, oldV[i] + (int64_t)h * old_max * hd,
                   (size_t)keep * hd * sizeof(float));
        }
    }
    if (oldK)
...
static void generate(Model *m, const int *prompt, int np, int n_new, int *out, const uint8_t *dmel, int naud) {
...
static void generate_stream(Model *m, Tok *T, const char *prompt, int n_new, const uint8_t *dmel, int naud) {
    Cfg *c = &m->c;
...
typedef struct {
    ...
    uint8_t *audio;
    int alen;
} SReq; /* raw DMel bytes after the payload */
#define SRV_QMAX 16
static SReq g_q[SRV_QMAX];
static int g_qn = 0;
...
static int serve_read_cmd(const char *cur_id) {
    ...
    if (!strcmp(cmd, "SUBMIT")) {
        ...
        if (fread(pl, 1, (size_t)plen, stdin) != (size_t)plen) {
            free(pl);
            return -1;
        }
        pl[plen] = 0;
        ...
        if (alen > 0) {
            ...
            if (fread(au, 1, (size_t)alen, stdin) != (size_t)alen) {
                free(pl);
                free(au);
                return -1;
            }
...
static void serve_one(Model *m, Tok *T, SReq *q) {
    ...
    if (q->alen % m->c.mel_bins != 0 || audio_tok_count(m, ids, np) != naud) {
        printf("ERROR %s audio frames (%d) do not match <|audio|> placeholders (%d)%s\n", q->id, naud,
               audio_tok_count(m, ids, np), m->audio_norm ? "" : " — snapshot has no audio tensors");
        fflush(stdout);
        free(ids);
        return;
    }
    ...
    if (getenv("INK_PREFIX_LOG")) {
        ...
        if (reuse) fprintf(stderr, "[PREFIX] reusing %d of %d prompt tokens (%.0f%%)\n", reuse, np, 100.0 * reuse / np);
            ...
            fprintf(stderr, "[PREFIX] no reuse: held=%d cap=%d prompt=%d%s%s\n", m->kvp.len, m->kvp.cap, np,
                    m->kvp.tainted ? " tainted" : "", (m->kvp.len > 0 && m->kvp.len < np) ? " (diverged)" : "");
        fflush(stderr);
    }
    ...
    for (int s = 0; s < q->max_tok && !cancelled; s++) {
        ...
        if (nhist < 128) hist[nhist++] = tk;
            ...
            memmove(hist, hist + 1, 127 * sizeof(int));
            hist[127] = tk;
        }
    ...
    printf("PROF %.3f %d %d %.3f %.3f %.3f %.3f %.3f %d\n", dt, np, gen, m->t_fill - f0, m->t_shared - s0,
           m->t_expert - e0, m->t_attn - a0, 0.0, gen + 1);
    fflush(stdout);
    free(ids);
}
...
static void serve_tiers_emap(Model *m) {
    ...
    char *hex = malloc((size_t)nsp * E * 2 + 1);
    int w = 0;
    for (int i = 0; i < c->n_layers; i++) {
        if (!c->sparse[i]) continue;
        LCache *lc = &m->cache[i];
        for (int e = 0; e < E; e++) {
            ...
            for (int z = 0; z < lc->n; z++)
                if (lc->slots[z].eid == e && lc->slots[z].filled) {
                    tier = 1;
                    break;
                }
...
static void serve_loop(Model *m, Tok *T) {
    ...
    for (;;) {
        while (!g_qn)
            if (serve_read_cmd(NULL) < 0) return; /* blocks on stdin */
        SReq q = g_q[0];
...
int main(int argc, char **argv) {
    ...
    if (g_topp > 0.f && g_topp < 1.f)
        fprintf(stderr,
                "[TOPP] %.2f: routed experts kept to cumulative weight — "
    ...
    if (chat && prompt && !dmel) {
        ...
        if (!chat_buf) {
            fprintf(stderr, "OOM chat template\n");
            return 1;
        }
        ...
        if (prompt[0]) w += sprintf(w, "<|message_user|><|content_text|>%s<|end_message|>", prompt);
    ...
    if (bits && (bits < 2 || bits > 8)) {
        fprintf(stderr, "quant_bits must be 0 (f32) or 2..8\n");
        return 1;
    }
    ...
    if (prompt || pfile) {
        ...
        printf("[cache] hit %.1f%% (%llu hit / %llu load)%s\n", tot ? 100.0 * m.hits / tot : 0.0,
               (unsigned long long)m.hits, (unsigned long long)m.miss, saved ? " | usage history saved" : "");
        ...
        if (g_topp > 0.f && m.ereq)
            printf("[topp] %.2f: %llu/%llu routed used (%.1f%% trimmed, "
                   ...
                   g_topp, (unsigned long long)m.euse, (unsigned long long)m.ereq,
    ...
    printf("== Inkling C engine (Stage A), cache = %d experts/layer, experts @ %s ==\n", cap,
           m.xq   ? "container (int4/int8 + .qs)"
           : bits ? "int (runtime quant)"
                  : "f32");
    printf("cfg: D=%d L=%d V=%d(%d) heads=%d/%d kv=%d/%d hd=%d win=%d d_rel=%d ext=%d E=%d+%d topk=%d\n", m.c.hidden,
           m.c.n_layers, m.c.vocab, m.c.unpad_vocab, m.c.n_heads, m.c.swa_heads, m.c.n_kv, m.c.swa_kv, m.c.head_dim,
           m.c.window, m.c.d_rel, m.c.rel_extent, m.c.n_experts, m.c.n_shared, m.c.topk);
    ...
    int rnaud = 0;
    if (dmint && ndm > 0) {
        if (ndm % m.c.mel_bins != 0) {
            fprintf(stderr, "dmel len %d not a multiple of mel_bins %d\n", ndm, m.c.mel_bins);
            return 1;
        }
    ...
    if (tfref && ntf == nfull) {
        ...
        free(lg);
        int ok = 0;
        for (int i = 0; i < nfull; i++) ok += (tf[i] == tfref[i]);
