...
# Safety rail (from upstream): never remove a tracked source file. The
# Makefile-derived globs above already target exact built basenames, so this
# never fires in normal operation, but it guards an on-demand/bench name that
# happens to collide with a source path.
