...
int main(void) {
    ...
    FILE *f = fopen("aud_tmp1", "wb");
    fputs("v1", f);
    fclose(f);
    if (rename("aud_tmp1", "aud_dst")) {
        printf("rename #1 FAIL\n");
        return 1;
    }
    f = fopen("aud_tmp2", "wb");
    fputs("v2", f);
    fclose(f);
    if (rename("aud_tmp2", "aud_dst")) {
        printf("rename #2 (dest exists) FAIL <- CRT EEXIST bug\n");
        return 1;
    }
    char buf[8] = {0};
    f = fopen("aud_dst", "rb");
    ...
    DWORD br;
    DeviceIoControl(h, FSCTL_SET_SPARSE, NULL, 0, NULL, 0, &br, NULL);
    off_t off = (off_t)5 * 1024 * 1024 * 1024; /* 5 GiB */
    const char magic[] = "COLIBRI64";
    OVERLAPPED ov = {0};
    ov.Offset = (DWORD)(off & 0xFFFFFFFFULL);
    ov.OffsetHigh = (DWORD)(off >> 32);
    if (!WriteFile(h, magic, sizeof(magic), &br, &ov)) {
        printf("write@5GB FAIL\n");
        close(fd);
        remove(p);
        return 1;
    }
