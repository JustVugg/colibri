...
def _monitor_state_bytes(monitor):
    """Canonical bytes for every response-derived mutable monitor field."""
    state = {
        "last": monitor._last,
        "deployment_identity": monitor._deployment_identity,
        "profile_versions": monitor._profile_versions,
        "profile_order": monitor._profile_order,
        "profile_counter": monitor._profile_counter,
        "profile_ambiguous": monitor._profile_ambiguous,
        "gpu_versions": monitor._gpu_versions,
        "gpu_observed": monitor._gpu_observed,
        "gpu_dirty_baseline": monitor._gpu_dirty_baseline,
    }
    return pickle.dumps(state, protocol=4)


def _identity_fixture(cuda=False):
    ...
    record.update(
        {
            ...
            "containment": {
                "version": 1,
                "mode": "cgroup-v2",
                "relative_path": "colibri/deployment-a/engine-0",
                "device": 41,
                "inode": 73,
            },
        }
    ...
    if not cuda:
        ...
        hardware = {"gpus": []}
    return manifest, report, plan, hardware


class _IdentityRace:
    def __init__(self, mutation_endpoint="health", cuda=False):
        manifest, report, plan, hardware = _identity_fixture(cuda=cuda)
        self.original = copy.deepcopy(manifest)
        self.persisted = copy.deepcopy(manifest)
        self.report = report
        self.plan = plan
        ...
        self.containment_live = True
        self.group_live = True
        self.restore_after_post_load = False
        ...
        self.profile = {
            "seq": 1,
            "turns": [{"completion_tokens": 10, "wall_s": 2.0}],
        }

    def load_manifest(self, required=True):
        self.assert_required(required)
        if self.raise_on_load:
            raise OSError("manifest replaced during telemetry sampling")
        snapshot = copy.deepcopy(self.persisted)
    ...
    def assert_required(required):
        if required is not True:
            raise AssertionError("runtime monitor must require the manifest")

    def process_matches(self, record):
    ...
    def process_group_members(self, pgid):
        member_pgid = pgid if self.group_live else pgid + 1
        return (
            [
                {
                    "pid": pgid,
    ...
    def verify_record(self, _record):
        if self.containment_live:
            return True, None
        return False, "managed cgroup identity was replaced"

    def urlopen(self, request, timeout):
        del timeout
        endpoint = request.full_url.rsplit("/", 1)[-1]
        payload = self.health if endpoint == "health" else self.profile
        response = _Response(payload)
        if endpoint == self.mutation_endpoint and self.mutator is not None:
            mutator = self.mutator
            self.mutator = None
            mutator(self)
            if self.restore_after_post_load:
...
class RuntimeIdentityRaceTest(unittest.TestCase):
    def test_contained_record_requires_injected_persisted_authorities(self):
        manifest, report, plan, hardware = _identity_fixture()
        race = _IdentityRace()
        monitor = RuntimeMonitor(
            urlopen=lambda *_args, **_kwargs: self.fail(
                "contained endpoint was probed without persisted authorities"
            ),
            subprocess_run=lambda *_args, **_kwargs: self.fail(
                "CPU-only monitor invoked nvidia-smi"
            ),
            wall_time=lambda: 1000.0,
            process_matches=lambda _record: (True, "running", {}),
            process_group_members=race.process_group_members,
            process_metrics=lambda _record: {"rss_bytes": MIB},
        )
        ...
        self.assertTrue(snapshot["service"]["stale"])
        self.assertIn(
            "containment identity cannot be revalidated",
            snapshot["service"]["error"],
        )

    def _assert_rejected_without_state_mutation(self, name, mutator):
        race = _IdentityRace(mutation_endpoint="health")
        monitor = race.monitor()
        warm = race.sample(monitor)
        self.assertEqual(warm["service"]["active"], 1)
        self.assertEqual(warm["latest_profile"]["completion_tokens"], 10)
        before = _monitor_state_bytes(monitor)

        race.health = {
            "status": "ok",
            "scheduler": {"active": 99, "queued": 98},
        }
        race.profile = {
            "seq": 99,
            "turns": [{"completion_tokens": 99, "wall_s": 1.0}],
        }
        race.mutator = mutator
        stale = race.sample(monitor)

        self.assertEqual(
            _monitor_state_bytes(monitor),
            before,
            "%s stale response mutated monitor state" % name,
        )
        self.assertTrue(stale["service"]["stale"])
        self.assertEqual(stale["service"]["active"], 1)
        self.assertEqual(stale["latest_profile"]["completion_tokens"], 10)

        race.persisted = copy.deepcopy(race.original)
        race.raise_on_load = False
        race.process_live = True
        race.containment_live = True
        race.group_live = True
        race.health = {
            "status": "ok",
            "scheduler": {"active": 7, "queued": 6},
        }
        race.profile = {
            "seq": 2,
            "turns": [{"completion_tokens": 20, "wall_s": 1.0}],
        }
        recovered = race.sample(monitor)

        self.assertFalse(recovered["service"]["stale"])
        self.assertEqual(recovered["service"]["active"], 7)
        self.assertFalse(recovered["profile_stale"])
        self.assertEqual(
            recovered["latest_profile"]["completion_tokens"],
            20,
        )
        self.assertNotEqual(_monitor_state_bytes(monitor), before)

    def test_health_response_binds_every_persisted_identity_field(self):
        record_fields = (
            ("pid", 5100),
            ("pgid", 5100),
            ("uid", 1001),
            ...
            ("port", 9123),
            ("node", 1),
            ("state_dir", "/run/colibri/replacement-state"),
            ("weights_dir", "/run/colibri/replacement-weights"),
        )
        containment_fields = (
            ("version", 2),
            ("mode", "replacement-mode"),
            ("relative_path", "colibri/replacement/engine-0"),
            ("device", 42),
            ("inode", 74),
        )
        cases = [
            (
                "deployment_id",
                lambda race: race.persisted.__setitem__(
            ...
            (
                "manifest_state",
                lambda race: race.persisted.__setitem__("state", "stopped"),
            ),
            (
                "process_removed",
                lambda race: race.persisted.__setitem__("processes", []),
            ),
        ]
        cases.extend(
            (
                "process_%s" % field,
                lambda race, field=field, value=value: race.persisted[
                    "processes"
        ...
        cases.extend(
            (
                "containment_%s" % field,
                lambda race, field=field, value=value: race.persisted[
                    "processes"
                ][0]["containment"].__setitem__(field, value),
            )
            for field, value in containment_fields
        )
        cases.extend(
            (
                (
                    "manifest_read_failure",
                    lambda race: setattr(race, "raise_on_load", True),
                ),
                (
                    "process_exit",
                    lambda race: setattr(race, "process_live", False),
                ),
                (
                    "live_process_group_replacement",
                    lambda race: setattr(race, "group_live", False),
                ),
                (
                    "live_containment_replacement",
                    lambda race: setattr(race, "containment_live", False),
                ),
            )
        )
        ...
        for name, mutator in cases:
            with self.subTest(identity=name):
                self._assert_rejected_without_state_mutation(name, mutator)

    def test_health_rejection_does_not_discard_independently_valid_profile(self):
        race = _IdentityRace(mutation_endpoint="health")
        monitor = race.monitor()
        race.sample(monitor)
        race.health = {
        ...
        race.restore_after_post_load = True
        race.mutator = lambda current: current.persisted["processes"][
            0
        ].__setitem__("nonce", "replacement-nonce")

        snapshot = race.sample(monitor)

        self.assertTrue(snapshot["service"]["stale"])
        self.assertEqual(snapshot["service"]["active"], 1)
        self.assertFalse(snapshot["profile_stale"])
        self.assertEqual(snapshot["latest_profile"]["completion_tokens"], 20)

    def test_profile_rejection_does_not_discard_independently_valid_health(self):
        race = _IdentityRace(mutation_endpoint="profile")
        monitor = race.monitor()
        race.sample(monitor)
        race.health = {
            "status": "ok",
        ...
        # Replace the live cgroup identity without changing the persisted
        # record. The injected verifier is the authority for that condition.
        race.mutator = lambda current: setattr(
            current, "containment_live", False
        )
        profile_state = (
            copy.deepcopy(monitor._last["profile"]),
            copy.deepcopy(monitor._profile_versions),
            copy.deepcopy(monitor._profile_order),
            monitor._profile_counter,
            monitor._profile_ambiguous,
        )

        snapshot = race.sample(monitor)

        self.assertFalse(snapshot["service"]["stale"])
        self.assertEqual(snapshot["service"]["active"], 7)
        self.assertTrue(snapshot["profile_stale"])
        self.assertEqual(snapshot["latest_profile"]["completion_tokens"], 10)
        self.assertEqual(
            (
                monitor._last["profile"],
                monitor._profile_versions,
                monitor._profile_order,
                monitor._profile_counter,
                monitor._profile_ambiguous,
            ),
            profile_state,
    ...
    def test_rejected_health_cannot_mutate_gpu_versions_or_dirty_baseline(self):
        race = _IdentityRace(mutation_endpoint="health", cuda=True)
        monitor = race.monitor()
        race.health["scheduler"] = {"active": 0, "queued": 0}
        warm = race.sample(monitor)
        self.assertFalse(warm["freshness"]["model"]["stale"])
        before = _monitor_state_bytes(monitor)

        race.health["scheduler"] = {"active": 1, "queued": 0}
        race.health["gpus_seq"] = 2
        race.health["gpus"][0]["model_bytes"] = 8 * MIB
        race.health["gpus"][0]["expert_bytes"] = 7 * MIB
        race.mutator = lambda current: current.persisted["processes"][
            0
        ].__setitem__("nonce", "replacement-nonce")
        ...
        self.assertTrue(stale["freshness"]["model"]["stale"])
        self.assertEqual(_monitor_state_bytes(monitor), before)

        race.persisted = copy.deepcopy(race.original)
        recovered = race.sample(monitor)
        self.assertEqual(monitor._gpu_versions, {8123: 2})
        self.assertIsNotNone(monitor._gpu_dirty_baseline)
        self.assertNotEqual(_monitor_state_bytes(monitor), before)
        self.assertTrue(recovered["freshness"]["model"]["stale"])
