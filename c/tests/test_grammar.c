...
static int feed(GrState *S, const char *s) {
    int n = 0;
    while (s[n]) {
        if (gr_accept(S, (unsigned char)s[n]) != 1) break;
        n++;
    }
    return n;
}
...
int main(void) {
    ...
    CHECK(gr_parse(&G, "# grammatica di prova\n"
                       "root ::= \"\\x41\"   # una A\n"
                       "         [\\x30-\\x32]\n") == 0);
    gr_state_init(&S, &G);
    CHECK(gr_forced(&S, buf, sizeof buf) == 1 && buf[0] == 'A');
