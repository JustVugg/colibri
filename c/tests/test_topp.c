...
static void ref_build(const float *lo, int V, double temp, double nuc, double *out, int *pidx, int *keep_out) {
    double mx = lo[0];
    for (int i = 1; i < V; i++)
        if (lo[i] > mx) mx = lo[i];
    double s = 0, invt = 1.0 / (temp > 1e-4 ? temp : 1e-4);
    for (int i = 0; i < V; i++) {
        out[i] = exp((lo[i] - mx) * invt);
        s += out[i];
    }
...
static void check_case(const char *label, int V, double nuc, const char *shape_name, const float *lo) {
    ...
    if (got_keep != ref_keep) FAIL("keep-count mismatch: got %d, ref %d", got_keep, ref_keep);
    ...
    if (has_ties) {
        ...
        int gm = 0;
        for (int i = 0; i < V; i++)
            if (g_pbuf[i] != 0.f) got[gm++] = (double)g_pbuf[i];
        if (gm != ref_keep) FAIL("tie-shape head size mismatch: got %d non-zero, ref %d", gm, ref_keep);
        for (int a = 1; a < gm; a++) {
            double k = got[a];
            int b = a - 1;
            while (b >= 0 && got[b] > k) {
        ...
        free(rsort);
        if (mm)
            FAIL("tie-shape multiset mismatch: %d/%d head values differ beyond 1e-6 rel (worst %.3g)", mm, ref_keep,
                 worst);
    } else {
        ...
        if (bad)
            FAIL("head not within 1e-6 rel of reference: %d entries differ (first id %d: got %.9g want %.9g, worst "
                 "%.3g)",
    ...
    double sum = 0;
    for (int i = 0; i < V; i++) sum += g_pbuf[i];
    if (fabs(sum - 1.0) > 1e-5) FAIL("head does not sum to 1.0: sum=%.12g (keep=%d)", sum, got_keep);

    free(ref);
    free(ridx);
    printf("  ok [V=%d nuc=%.3f shape=%s keep=%d%s sum=%.10f]\n", V, nuc, shape_name, got_keep,
           has_ties ? " (ties)" : "", sum);
}
#undef FAIL
...
static void fill_shape(float *lo, int V, int shape) {
    ...
    case 1: /* peaked: one dominant token, rest small and distinct (no ties).
        ...
        lo[0] = 3.f;
        lo[V / 3 < V ? V / 3 : V - 1] = 1.f;
        lo[V / 2 < V ? V / 2 : V - 1] = 0.5f;
        break;
    case 2: /* all-equal distinct decay (no ties): geometric, strictly decreasing */
        for (int i = 0; i < V; i++) lo[i] = (float)(-0.001 * (double)i);
        break;
    ...
    case 4: /* sharp-tail: a few hot, then a long flat floor (small tie at the floor).
        ...
        {
            int hot = V < 12 ? V : 12;
            for (int i = 0; i < hot; i++) lo[i] = (float)(2.0 - frand());
        }
        break;
    }
}
...
int main(void) {
    ...
    for (size_t si = 0; si < sizeof(sizes) / sizeof(sizes[0]); si++) {
        ...
        for (int shape = 0; shape < n_shapes; shape++) {
            ...
            for (size_t ni = 0; ni < sizeof(nucs) / sizeof(nucs[0]); ni++) {
                char label[32];
                snprintf(label, sizeof(label), "size[%zu]/shape[%d]", si, shape);
    ...
    /* guard-off path: g_nuc >= 1 must skip truncation entirely (full softmax kept) */
    {
        ...
        for (int i = 0; i < V; i++)
            if (g_pbuf[i] != 0.f) nz++;
        if (nz != V) {
            fprintf(stderr, "  FAIL [guard-off nuc=1.0]: %d/%d entries kept, expected all\n", nz, V);
            g_nfails++;
        ...
        for (int i = 0; i < V; i++)
            if (g_pbuf[i] != 0.f) nz++;
        if (nz != V) {
            fprintf(stderr, "  FAIL [guard-off nuc=0.0]: %d/%d entries kept, expected all\n", nz, V);
            g_nfails++;
