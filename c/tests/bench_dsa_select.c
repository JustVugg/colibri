...
static double bench_ns(void (*fn)(const float *, int, int, int *, int *), const float *isc, int nk, int keep) {
    static double ts[N_REPEAT];
    int *dst = malloc((size_t)nk * sizeof(int));
    int nd;
...
static void fill_realistic(float *isc, int nk) { /* few hot, long distinct tail */
    ...
    isc[0] = 3.f;
    isc[nk / 50 < nk ? nk / 50 : nk - 1] = 1.f;
    isc[nk / 200 < nk ? nk / 200 : nk - 1] = 0.5f;
}
...
int main(void) {
    ...
    int *dst = malloc((size_t)65536 * sizeof(int));
    int nd;
    ...
    printf("bench_dsa_select: DSA top-keep, old (qsort) vs new (partial-select)  keep=%d  (median of %d seeds x %d "
           "reps)\n",
    ...
    for (size_t sh = 0; sh < sizeof(shapes) / sizeof(shapes[0]); sh++) {
        for (size_t ni = 0; ni < sizeof(nks) / sizeof(nks[0]); ni++) {
            ...
            for (int sd = 0; sd < N_SEEDS; sd++) {
                ...
                for (int w = 0; w < 50; w++) {
                    keep_old(isc, nk, keep, dst, &nd);
                    keep_new(isc, nk, keep, dst, &nd);
                }
