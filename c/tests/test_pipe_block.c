...
static int build_fixture(Model *m, int fd) {
    ...
    m->S.n = NE * 6;
    m->S.cap = NE * 6;
    m->S.t = calloc(NE * 6, sizeof(st_tensor));
    ...
    for (int e = 0; e < NE; e++) {
        ...
        for (int k = 0; k < 3; k++) {
            ...
            m->S.t[e * 6 + k] = (st_tensor){strdup(name), fd, wo, WB, 3, WB};
            wo += WB;
            size_t n = strlen(name);
            memcpy(name + n, ".qs", 4);
            m->S.t[e * 6 + 3 + k] = (st_tensor){strdup(name), fd, so, sbytes[k], 2, sbytes[k] / 4};
            so += sbytes[k];
        }
    }
    return 0;
...
static int check_slot(ESlot *s, int e) {
    if (s->eid != e || s->g.fmt != 1 || s->u.fmt != 1 || s->d.fmt != 1) {
        fprintf(stderr, "  slot: eid=%d (want %d) fmt g/u/d=%d/%d/%d (want 1/1/1)\n", s->eid, e, s->g.fmt, s->u.fmt,
                s->d.fmt);
        return 1;
    }
    const unsigned char *g = (const unsigned char *)s->g.q8, *u = (const unsigned char *)s->u.q8,
...
static int test_implication_table(void) {
    struct {
        const char *pipe_env, *pw_env;
        int pipe_now, want;
    } T[] = {
        {NULL, "4", 0, 1},                                        /* pool sized, pipe off, PIPE unset → imply */
        {NULL, "16", 0, 1}, {NULL, "0", 0, 0},                    /* PIPE_WORKERS=0 must NOT enable a clamped pipe */
        {NULL, "", 0, 0},   {NULL, "-3", 0, 0}, {"0", "4", 0, 0}, /* explicit PIPE=0 always wins */
    ...
    for (size_t i = 0; i < sizeof(T) / sizeof(T[0]); i++)
        if (pipe_workers_imply_pipe(T[i].pipe_env, T[i].pw_env, T[i].pipe_now) != T[i].want) {
            fprintf(stderr, "FAIL: implication row %zu (PIPE=%s PIPE_WORKERS=%s pipe_now=%d)\n", i,
                    T[i].pipe_env ? T[i].pipe_env : "<unset>", T[i].pw_env ? T[i].pw_env : "<unset>", T[i].pipe_now);
            return 1;
        }
    return 0;
}
...
int main(void) {
    ...
    if (run_generations(&m, 0, 200) || run_generations(&m, 1, 200)) {
        close(fd);
        remove(TMPF);
        return 1;
    }
    ...
    for (int q = 0; q < NE; q++) {
        compat_aligned_free(m.ws[q].slab);
        free(m.ws[q].fslab);
    }
