...
#ifndef _WIN32
int main(void) {
    puts("compat direct tests: skipped (POSIX has native O_DIRECT)");
    return 0;
}
#else
...
static int fail(const char *m) {
    fprintf(stderr, "compat direct test failed: %s\n", m);
    return 1;
}
...
int main(void) {
    ...
    if (posix_fadvise(wfd, 0, (off_t)FSZ, POSIX_FADV_DONTNEED) != 0)
        return fail("DONTNEED should be a safe no-op (return 0)");
    if (posix_fadvise(-1, 0, (off_t)FSZ, POSIX_FADV_WILLNEED) != 0)
        return fail("WILLNEED on bad fd should no-op (return 0)");
