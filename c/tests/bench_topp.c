...
static double bench_ns(void (*fn)(const float *, int, double, double), const float *lo, int V, double temp,
                       double nuc) {
...
int main(void) {
    ...
    for (size_t sh = 0; sh < sizeof(shapes) / sizeof(shapes[0]); sh++) {
        ...
        for (size_t ni = 0; ni < sizeof(nucs) / sizeof(nucs[0]); ni++) {
            ...
            for (int w = 0; w < 50; w++) {
                dist_build_old(lo, V, temp, nuc);
                dist_build_new(lo, V, temp, nuc);
            }
