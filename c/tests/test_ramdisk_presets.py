...
class GpuDiscoveryTest(unittest.TestCase):
    def test_discovers_and_normalizes_gpu_pci_numa_locality(self):
        output = (
            "0, RTX 5090, 00000000:41:00.0, 32768, 30000\n"
            "1, RTX 5090, 00000000:C1:00.0, 32768, 29000\n"
        )
        ops = _GpuOps(
            {
        ...
        self.assertEqual(
            [
                (gpu["index"], gpu["pci_bus_id"], gpu["numa_node"])
    ...
    def test_maps_minus_one_only_on_a_single_effective_node(self):
        ...
        run = mock.Mock(
            return_value=argparse.Namespace(
                returncode=0,
                stdout="0, GPU, 0000:01:00.0, 1024, 512\n",
                stderr="",
            )
        )
