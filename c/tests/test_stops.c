...
static const char *TOKJSON = "{\"model\":{\"vocab\":{\"a\":0,\"b\":1,\"c\":2},\"merges\":[[\"a\",\"b\"]]},"
                             " \"added_tokens\":["
...
static void write_cfg(const char *dir, const char *fname, const char *eos_json) {
    char p[512];
    snprintf(p, sizeof(p), "%s/%s", dir, fname);
    FILE *f = fopen(p, "w");
    if (!f) {
        perror(p);
        exit(1);
...
static void write_tok(const char *dir) {
    char p[512];
    snprintf(p, sizeof(p), "%s/tokenizer.json", dir);
    FILE *f = fopen(p, "w");
    if (!f) {
        perror(p);
        exit(1);
...
static int expect(const char *what, int id, int want) {
    int got = is_stop(id);
    if (got != want) {
        fprintf(stderr, "  FAIL %s: token %d stop=%d, expected %d\n", what, id, got, want);
        return 1;
    }
    return 0;
}
...
int main(void) {
    ...
    if (!T.id_special[101]) {
        fprintf(stderr, "  FAIL <|user|> not marked special\n");
        fail = 1;
    }
    if (!T.id_special[104]) {
        fprintf(stderr, "  FAIL <sop> not marked special\n");
        fail = 1;
    }
    if (T.id_special[110]) {
        fprintf(stderr, "  FAIL <think> wrongly marked special\n");
        fail = 1;
    }
    if (T.id_special[111]) {
        fprintf(stderr, "  FAIL <tool_call> wrongly marked special\n");
        fail = 1;
    }
     ...
     *    the union must recover all three. This is @woolcoxm's case. */
    {
        ...
        load_cfg(&c, dir);
        if (c.n_stop != 3) {
            fprintf(stderr, "  FAIL union: expected 3 stops, got %d\n", c.n_stop);
            fail = 1;
    ...
    /* 3. no generation_config.json at all: must not crash, config alone wins */
    {
        ...
        load_cfg(&c, dir);
        if (c.n_stop != 3) {
            fprintf(stderr, "  FAIL no-genconfig: expected 3, got %d\n", c.n_stop);
            fail = 1;
     ...
     *    StopFilter. Only the batched gateway may reduce the engine stop set. */
    {
        ...
        c.stop_ids[0] = 100;
        c.n_stop = 1;
        test_set_env("SERVE", "1");
        clear_env("SERVE_BATCH");
        clear_env("COLI_SERVE_ALL_STOPS");
        stops_arm_tok(&c, 100, &T);
        fail |= expect("private chat keeps <|user|>", 101, 1);
        fail |= expect("private chat keeps <|observation|>", 102, 1);
        if (g_nstop != 5) {
            fprintf(stderr, "  FAIL private chat: expected 5 stops, got %d\n", g_nstop);
            fail = 1;
        }
        ...
        if (g_nstop != 1) {
            fprintf(stderr, "  FAIL batched gateway: expected 1 stop, got %d\n", g_nstop);
            fail = 1;
        }
        ...
        if (g_nstop != 5) {
            fprintf(stderr, "  FAIL gateway override: expected 5 stops, got %d\n", g_nstop);
            fail = 1;
        }
        clear_env("COLI_SERVE_ALL_STOPS");
        clear_env("SERVE_BATCH");
    ...
    rm_file(dir, "config.json");
    rm_file(dir, "generation_config.json");
    rm_file(dir, "tokenizer.json");
    rmdir(dir);
    if (fail) {
        printf("test_stops: FAIL\n");
        return 1;
    }
