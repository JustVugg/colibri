...
int main(void) {
    ...
    if (tc != NITEMS) {
        fprintf(stderr, "FAIL: total claimed %ld != %d\n", tc, NITEMS);
        fail = 1;
    }
    for (int i = 0; i < NITEMS && !fail; i++) {
        int s = atomic_load_explicit(&seen[i], memory_order_relaxed);
        if (s != 1) {
            fprintf(stderr, "FAIL: item %d claimed %d times (want exactly 1)\n", i, s);
            fail = 1;
        }
    }
    if (!fail)
        printf("test_pilot_ring: ok — %d items across %d workers, each claimed exactly once\n", NITEMS, NTHREADS);
    return fail;
}
