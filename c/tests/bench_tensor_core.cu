...
static double run(ColiCudaTensor *g,ColiCudaTensor *u,ColiCudaTensor *d,
                  const float *x,float *y,int rows,int iterations,int tensor_core){
    ColiCudaTensor *gs[1]={g},*us[1]={u},*ds[1]={d}; int rs[1]={rows};
    if(tensor_core){setenv("COLI_CUDA_TC_INT4","1",1);setenv("COLI_CUDA_TC_MIN_ROWS","1",1);}
...
int main(){
    ...
    std::vector<float> hs(I),ds(D),x((size_t)O*D),a((size_t)O*D),b((size_t)O*D);
    ...
    for(int rows: {1,2,4,8}){
        ...
        double tc=run(g,u,d,x.data(),b.data(),rows,3,1);
        double err=0,ref=0;for(int i=0;i<rows*D;i++){double e=b[i]-a[i];err+=e*e;ref+=(double)a[i]*a[i];}
        std::printf("rows=%d scalar_ms=%.3f tensor_ms=%.3f speedup=%.3fx rel_rms=%.5f\n",
                    rows,scalar,tc,scalar/tc,std::sqrt(err/(ref+1e-20)));
    }
