...
static int pbuf_has_nan(int V) {
    for (int i = 0; i < V; i++)
        if (!isfinite(g_pbuf[i])) return 1;
    return 0;
}
...
int main(void) {
    ...
    g_temp = 0.7f;
    g_nuc = 0.90f; /* il path serve di default */

    /* (a) logit sani: il campionato deve avere prob > 0 e nessun NaN nel buffer */
    for (int i = 0; i < V; i++) lo[i] = (float)((int)(xr() % 2000) - 1000) / 100.f;
    int known = 1337;
    lo[known] = 50.f; /* picco netto */
    dist_build(lo, V);
    if (pbuf_has_nan(V)) {
        printf("  FAIL: NaN in g_pbuf con logit sani\n");
        fail = 1;
    }
    if (g_pbuf[known] <= 0.f) {
        printf("  FAIL: il picco ha prob 0\n");
        fail = 1;
    }
    ...
    for (int b = 0; b < 3; b++) {
        for (int pos = 0; pos < 3; pos++) { /* lo[0], meta', ultimo */
            ...
            int amax = 777;
            lo[amax] = 9.0f; /* massimo FINITO atteso */
            ...
            dist_build(lo, V);
            if (pbuf_has_nan(V)) {
                printf("  FAIL: NaN sopravvive (%s @ %d)\n", bn[b], at);
                fail = 1;
                continue;
            }
    ...
    dist_build(lo, V);
    if (pbuf_has_nan(V)) {
        printf("  FAIL: tutti-NaN lascia NaN nel buffer\n");
        fail = 1;
