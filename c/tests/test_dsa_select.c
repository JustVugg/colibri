...
static void check_partition(const char *label, const float *isc, int nk, int keep, const char *shape_name) {
    float *tmp = malloc((size_t)nk * sizeof(float));
    memcpy(tmp, isc, (size_t)nk * sizeof(float));
    partial_select_desc(tmp, nk, keep);
    ...
    for (int i = 0; i < keep; i++)
        if (tmp[i] < top_min) top_min = tmp[i];
    for (int i = keep; i < nk; i++)
        if (tmp[i] > tail_max) tail_max = tmp[i];
    if (!(top_min >= tail_max)) FAIL("partition invariant violated: top_min=%.9g < tail_max=%.9g", top_min, tail_max);
    free(tmp);
}

/* ---- end-to-end: old vs new kept-set must be element-wise identical ---- */
static void check_case(const char *label, int nk, int keep, const char *shape_name, const float *isc) {
    ...
    if (first_diff >= 0)
        FAIL("kept-set differs at index %d: old dst[%d]=%d new dst[%d]=%d", first_diff, first_diff, da[first_diff],
             first_diff, db[first_diff]);
...
static void fill_shape(float *isc, int nk, int shape) {
    ...
    case 2: /* strictly decreasing geometric (no ties): sorted input -- worst case
        ...
        for (int i = 0; i < nk; i++) isc[i] = (float)(-0.001 * (double)i);
        break;
...
int main(void) {
    ...
    for (size_t ni = 0; ni < sizeof(nks) / sizeof(nks[0]); ni++) {
        ...
        for (int shape = 0; shape < n_shapes; shape++) {
            ...
            for (size_t ki = 0; ki < sizeof(keeps) / sizeof(keeps[0]); ki++) {
                ...
                if (keep <= 0) continue;
                char label[40];
                snprintf(label, sizeof(label), "nk[%zu]/keep[%zu]/shape[%d]", ni, ki, shape);
                const char *sn =
                    (const char *[]){"random", "peaked", "decreasing", "increasing", "plateau", "all-equal"}[shape];
    ...
    /* edge: keep == nk (nothing to partition; both paths keep everything) */
    {
        ...
        int *db = malloc(sizeof(int) * nk);
        int nb = keep_new(isc, nk, keep, db);
        if (nb != nk) {
            fprintf(stderr, "  FAIL [keep==nk]: kept %d expected %d\n", nb, nk);
            g_nfails++;
    ...
    /* edge: keep == 1 (threshold = the single max; quickselect must find it) */
    {
        ...
        for (int i = 0; i < nk; i++) isc[i] = (float)frand();
        int da[1], db[1];
        int na = keep_old(isc, nk, keep, da), nb = keep_new(isc, nk, keep, db);
     ...
     * the position scan must pick positions 0..keep-1 deterministically */
    {
        ...
        for (int i = 0; i < nb; i++)
            if (db[i] != i) bad = 1;
        if (nb != keep || bad) {
            fprintf(stderr, "  FAIL [all-equal keep=%d]: nb=%d bad=%d\n", keep, nb, bad);
            g_nfails++;
