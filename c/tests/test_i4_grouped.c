...
static void ref_grouped(double *y, double *mag, const float *x, const uint8_t *q4, const float *scale, int S, int I,
                        int O, int gs) {
...
static int check(const char *name, int S, int I, int O, int gs, int fill_edges) {
    ...
    for (int i = 0; i < S * O; i++) {
        ...
        if (rel > 1e-6) {
            if (bad < 3)
                fprintf(stderr, "%s: [%d] got %.9g want %.9g (|terms| %.3g, rel %.3g)\n", name, i, (double)y[i], yr[i],
                        ym[i], rel);
            bad++;
        }
    }
    ...
    free(ym);
    if (bad) {
        fprintf(stderr, "%s: FAIL (%d/%d mismatched, worst rel %.3g)\n", name, bad, S * O, worst);
        return 1;
    }
...
static int check_pair(const char *name, int S, int I, int O, int gs) {
    ...
    if (!qg || !qu || !sg || !su || !x || !yg || !yu || !rg || !ru || !dg || !du || !mg || !mu) {
        fprintf(stderr, "%s: OOM\n", name);
        return 1;
    }

    for (size_t i = 0; i < (size_t)O * rb; i++) {
        qg[i] = (uint8_t)(xr() & 0xFF);
        qu[i] = (uint8_t)(xr() & 0xFF);
    }
    ...
    for (int i = 0; i < S * O; i++) {
        ...
        if (eg > 1e-6 || eu > 1e-6) {
            if (bad < 3)
                fprintf(stderr, "%s: [%d] gate %.9g/%.9g up %.9g/%.9g (rel %.3g/%.3g)\n", name, i, (double)yg[i], dg[i],
                        (double)yu[i], du[i], eg, eu);
            bad++;
        }
    ...
    if (I % gs == 0 && !exact) {
        fprintf(stderr,
                "%s: FAIL fused != unfused bitwise on an ALIGNED shape "
    ...
    free(mu);
    if (bad) {
        fprintf(stderr, "%s: FAIL (%d mismatched)\n", name, bad);
        return 1;
    }
