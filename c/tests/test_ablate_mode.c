...
static int run_mode(Cfg *c, const char *manifest_path, const char *out_path){
    ...
    int rc=ablate_model_mode_run(&m,manifest_path);
    memset(&g_ablate_adapter_test,0,sizeof(g_ablate_adapter_test));
    return rc;
}
...
static void t_refusal(Cfg *c){
    ...
    static const struct { const char *body; const char *why; } bad[] = {
        ...
        { "0 3 2 0 0 1 2 64\n", "a token exactly at the vocab size -- one past the "
          "last valid id (999 above is not a boundary test: it is caught the same "
          "way whether the bound is >=V or the off-by-one >V)" },
        { "99999999999999999999999999 3 2 0 0 1 2 3\n",
          "an item id with enough digits to overflow 64-bit parsing (ERANGE) -- "
          "the item field has no upper-bound range check of its own, so an "
          "overflow silently clamped to INTMAX_MAX would otherwise be accepted "
          "as an enormous-but-'valid' item id" },
        { "0 3 2 0 0 1 2 3\n0 3 2 0 0 1 2 3\n", "a duplicate item id" },
    };
...
/* The vocab bound above (P3) proves the exact boundary (V) is refused; this
 * proves the check is not off by one in the OTHER direction either -- the
 * true last valid id (V-1) must still be accepted, or a fix for one off-by-one
 * could silently introduce the opposite one. */
static void t_token_boundary_accepts_the_last_valid_id(Cfg *c){
    printf("P3c the largest valid token id (vocab-1) is still accepted\n");
    manifest_write("0 3 2 0 0 1 2 63\n");
    int rc=run_mode(c,MANIFEST_PATH,EVIDENCE_PATH);
    char *text=slurp(EVIDENCE_PATH,NULL);
    CHECK(rc==0 && text!=NULL, "token id 63 (vocab-1, this fixture's vocab=64) runs to completion");
    free(text);
    remove(EVIDENCE_PATH);
}
...
static void t_dispatch_contract(Cfg *c){
     ...
     * not requested; the mode entry must refuse it and touch nothing. */
    int rc=ablate_model_mode_run(&m,NULL);
    char *text=slurp(EVIDENCE_PATH,NULL);
...
int main(int argc, char **argv){
    ...
    t_round_trip(&c);
    t_refusal(&c);
    t_token_boundary_accepts_the_last_valid_id(&c);
