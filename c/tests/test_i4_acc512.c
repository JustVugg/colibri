...
static double dot_i4f_double(const uint8_t *w, const float *x, int I) {
    ...
    for (int i = 0; i < I; i++) {
        uint8_t b = w[i >> 1];
        a += (double)x[i] * (double)(((b >> ((i & 1) * 4)) & 15) - 8);
    }
    return a;
}
...
static int trial(int I, int rows, double xscale, const char *label) {
    ...
    double e512 = 0, esca = 0;
    int bad = 0;
    for (int r = 0; r < rows; r++) {
        for (int i = 0; i < I; i += 2) {
            int q0 = (int)(rndu() * 16), q1 = (int)(rndu() * 16);
...
#else
int main(void) {
    puts("test_i4_acc512: skipped (no AVX-512 on this build)");
    return 0;
}
#endif
