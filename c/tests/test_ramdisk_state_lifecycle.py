...
class StateAndSafetyTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if os.path.__name__ != "posixpath":
            self.descriptor_seam.enter_context(portable_linux_manifest_paths())
    ...
    def test_atomic_json_stream_close_cannot_mask_primary_error(self):
        ...
        def capture_fdopen(descriptor, *args, **kwargs):
            del args, kwargs
    ...
    def test_status_propagates_control_flow_from_probe_seams(self):
        ...
        identity_interrupt = ramdisk._TerminationSignal(signal.SIGTERM)
        with mock.patch.object(
            ...
            with self.assertRaises(ramdisk._TerminationSignal):
                ramdisk.status(deep=False)

        liveness_interrupt = ramdisk._TerminationSignal(signal.SIGINT)
        with mock.patch.object(
            ramdisk, "_load_manifest", return_value=process_manifest
        ), mock.patch.object(
            ramdisk, "_mount_at", return_value=None
