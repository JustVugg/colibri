...
typedef struct {
    GrFrame f[GR_MAX_DEPTH];
    int16_t n;
} GrStack;
typedef struct {
    Grammar *G;
    GrStack st[GR_MAX_STACKS];
    int n;
    int alive;
} GrState;
...
static int gr__alt_new(Grammar *G, int ri) {
    ...
    if (R->n == R->cap) {
        ...
        GrAlt *na = (GrAlt *)realloc(R->a, (size_t)nc * sizeof(GrAlt));
        if (!na) return -1;
        R->a = na;
        R->cap = nc;
    }
...
static int gr__push(Grammar *G, int ri, int ai, const GrSym *sy) {
    ...
    if (A->n == A->cap) {
        ...
        GrSym *ns = (GrSym *)realloc(A->s, (size_t)nc * sizeof(GrSym));
        if (!ns) return -1;
        A->s = ns;
        A->cap = nc;
    }
...
static int gr__idlen(const char *p) {
    int n = 0;
    while (gr__idch(p[n])) n++;
    return n;
}
...
static int gr__esc(const char **pp) { /* dopo la barra: byte 0-255 o -1 */
    ...
    case '^':
        ...
        break;
    case 'x': {
        int h = gr__hex(p[1]), l = gr__hex(p[2]);
        if (h >= 0 && l >= 0) {
            c = h * 16 + l;
            p += 2;
        }
...
static int gr__lit(Grammar *G, int ri, int ai, const char **pp) {
    ...
    while (*p && *p != '"') {
        ...
        if (*p == '\\') {
            ...
            b = gr__esc(&p);
            if (b < 0) {
                snprintf(G->err, sizeof G->err, "invalid escape in literal");
                return -1;
            }
        ...
        memset(&s, 0, sizeof s);
        s.t = GR_CLS;
        s.c.bits[b >> 3] |= (uint8_t)(1u << (b & 7));
        if (gr__push(G, ri, ai, &s)) {
            snprintf(G->err, sizeof G->err, "out of memory");
            return -1;
        }
    }
    if (*p != '"') {
        snprintf(G->err, sizeof G->err, "unterminated literal");
        return -1;
    }
    *pp = p + 1;
    return 0;
...
static int gr__cls(Grammar *G, int ri, int ai, const char **pp) {
    ...
    while (*p && *p != ']') {
        ...
        if (*p == '\\') {
            ...
            lo = gr__esc(&p);
            if (lo < 0) {
                snprintf(G->err, sizeof G->err, "invalid escape in character class");
                return -1;
            }
    ...
    if (*p != ']') {
        snprintf(G->err, sizeof G->err, "unterminated character class");
        return -1;
    }
    if (neg)
        for (int i = 0; i < 32; i++) s.c.bits[i] = (uint8_t)~s.c.bits[i];
    *pp = p + 1;
    if (gr__push(G, ri, ai, &s)) {
        snprintf(G->err, sizeof G->err, "out of memory");
        return -1;
...
static int gr__postfix(Grammar *G, int ri, int ai, int n0, char op) {
    ...
    int ii = gr__anon(G);
    if (ii < 0) goto full;
    int ia = gr__alt_new(G, ii);
    if (ia < 0) goto full;
    for (int j = 0; j < k; j++)
        if (gr__push(G, ii, ia, &G->r[ri].a[ai].s[n0 + j])) goto full;
    G->r[ri].a[ai].n = n0;
    int rr = gr__anon(G);
    if (rr < 0) goto full;
    GrSym I;
    ...
    R.t = GR_REF;
    R.ref = (int16_t)rr;
    int a0 = gr__alt_new(G, rr);
    if (a0 < 0) goto full;
    if (gr__push(G, rr, a0, &I)) goto full;
    if (op == '*' || op == '+')
        if (gr__push(G, rr, a0, &R)) goto full;
    int a1 = gr__alt_new(G, rr);
    if (a1 < 0) goto full; /* "" per ? e *, I per + */
    if (op == '+')
        if (gr__push(G, rr, a1, &I)) goto full;
...
static int gr__alts(Grammar *G, int ri, const char **pp, int depth, int in_group) {
    if (depth > 32) {
        snprintf(G->err, sizeof G->err, "groups are nested too deeply");
        return -1;
    }
    const char *p = *pp;
    ...
    for (;;) {
        ...
        if (!*p) {
            if (in_group) {
                snprintf(G->err, sizeof G->err, "missing ')'");
                return -1;
            }
            break;
        }
        if (*p == ')') {
            if (!in_group) {
                snprintf(G->err, sizeof G->err, "unexpected ')'");
                return -1;
            }
            break;
        }
        ...
        if (*p == '"') {
            ...
            if (ref < 0) {
                snprintf(G->err, sizeof G->err, "too many rules");
                return -1;
            }
            p += nl;
            GrSym s;
            ...
            snprintf(G->err, sizeof G->err, "unexpected character '%c'", *p);
            return -1;
        }
        p = gr__ws(p);
        if (*p == '?' || *p == '*' || *p == '+') {
            if (gr__postfix(G, ri, ai, n0, *p)) return -1;
            p++;
        }
    }
    *pp = p;
...
static int gr_parse(Grammar *G, const char *src) {
    ...
    for (;;) {
        ...
        int nl = gr__idlen(p);
        if (nl <= 0) {
            snprintf(G->err, sizeof G->err, "expected a rule, found '%c'", *p);
            return -1;
        }
        const char *name = p;
        const char *q = gr__ws(p + nl);
        if (strncmp(q, "::=", 3)) {
            snprintf(G->err, sizeof G->err, "expected '::=' after '%.*s'", nl, name);
            return -1;
        }
        p = q + 3;
        ...
        if (G->r[ri].n > 0) {
            snprintf(G->err, sizeof G->err, "duplicate rule '%.*s'", nl, name);
            return -1;
        }
    ...
    for (int i = 0; i < G->n; i++) {
        ...
        if (G->r[i].n == 0) {
            snprintf(G->err, sizeof G->err, "rule '%s' is used but never defined", G->r[i].name);
            return -1;
        }
    }
    if (G->root < 0) {
        snprintf(G->err, sizeof G->err, "missing 'root' rule");
        return -1;
    }
    return 0;
}
...
static void gr_state_init(GrState *S, Grammar *G) {
    ...
    for (int a = 0; a < R->n; a++) {
        GrStack k;
        k.n = 1;
        k.f[0].r = (int16_t)G->root;
        k.f[0].a = (int16_t)a;
        k.f[0].s = 0;
        if (!gr__normalize(G, &k, S, 0)) {
            S->alive = 0;
            return;
        }
    }
