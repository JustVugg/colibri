...
def _parse_vm_stat(text, fallback_page_size):
    ...
    page_size = int(page_match.group(1)) if page_match else fallback_page_size
...
def _darwin_memory_available(*, run=None, fallback_page_size=None):
    ...
    try:
        ...
        if result.returncode == 0:
            available = _parse_vm_stat(result.stdout, fallback_page_size)
            if available:
                return available
...
def memory_available():
    ...
    if sys.platform == "win32":
        try:
            ...
            pass
    # macOS: use the stable absolute system-tool paths. Nix build sandboxes do
    # not expose these tools through PATH even on Darwin.
