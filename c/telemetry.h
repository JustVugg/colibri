...
static int64_t expert_bytes_probe(Model *m, int ebits) {
    ...
    if (m->has_mtp) {
        int64_t mtp = expert_bytes_layer(m, c->n_layers);
        if (mtp > eb) eb = mtp;
    }
...
static void hw_probe(char *cpu, size_t cn, int *cores, double *ram_total, double *ram_avail) {
...
#if defined(__x86_64__) || defined(__i386__)
    {
        unsigned int r[12] = {0};
        unsigned int *w = r;
        for (unsigned int f = 0x80000002u; f <= 0x80000004u; f++, w += 4) __get_cpuid(f, &w[0], &w[1], &w[2], &w[3]);
        char *b = (char *)r;
        b[47] = 0;
        while (*b == ' ') b++;
        snprintf(cpu, cn, "%s", b);
    ...
    *ram_total = *ram_avail = 0;
...
#elif defined(__APPLE__)
    {
        ...
        mach_msg_type_number_t nc = HOST_VM_INFO64_COUNT;
...
static void hwinfo_emit(Model *m) {
    ...
    for (int i = 0; i < g_cuda_ndev; i++) {
        size_t fr = 0, to = 0;
        coli_cuda_mem_info(g_cuda_devices[i], &fr, &to);
...
static void tiers_emit(Model *m) {
    ...
    int pinned = 0, lru = 0;
    for (int i = 0; i <= c->n_layers; i++) {
        pinned += m->npin ? m->npin[i] : 0;
        lru += m->ecn ? m->ecn[i] : 0;
    }
    int vram = 0;
    double vram_gb = 0;
#ifdef COLI_CUDA
    vram = m->gpu_expert_count;
    vram_gb = m->gpu_expert_bytes / 1e9;
#endif
    int ram = pinned - vram + lru;
    if (ram < 0) ram = 0;
    int disk = total - vram - ram;
    if (disk < 0) disk = 0;
    ...
    for (int i = 0; i <= c->n_layers; i++) {
        ...
        for (int e = 0; e < cols; e++) {
            ...
            if (!tier && m->ecache && m->ecache[i])
                for (int z = 0; z < m->ecn[i]; z++)
                    if (m->ecache[i][z].eid == e) {
                        tier = 1;
                        break;
                    }
