...
- **Router-lookahead prefetch** (`PILOT=1`, experimental) — the next layer's routing is 71.6% predictable from the current layer's post-attention state (measured); a dedicated I/O thread prefetches those experts while the current layer computes. `PILOT_N=n` extends the lookahead to layers L+1..L+n (predictability decays gracefully with depth — measured 63→42% from 1 to 4 layers ahead on a fixture — and the per-depth prefetch width `PILOT_K` decays with it); hints are deduplicated per token so adjacent layers don't re-enqueue the same experts. On bandwidth-saturated NVMe the measured sweet spot is small and precise: `PILOT=1 PILOT_K=2` gave **+41%** cold decode on a synthetic fixture (wide/deep prefetch is neutral-to-negative there; depth is for latency-bound stores).
...
The lookahead can also run **deeper than one layer**: `PILOT_N=n` applies the routers of layers L+1..L+n to L's post-attention state and issues all of it to the I/O thread, keeping n layers of reads in flight. Predictability decays gracefully with depth (measured on a synthetic fixture: 63% recall one layer ahead, 54% at two, 42% at four, still 25% at eight — always far above the 22% "same-as-last-token" baseline), so the prefetch width shrinks with depth: `PILOT_K` at depth 1, minus `PILOT_DECAY` (default 2) per extra layer. Hints are deduplicated per token pass, so consecutive layers re-predicting the same target don't spam the ring with duplicate `fadvise` calls. **Honest guidance from A/B measurement** (cold-cache 9 GB fixture from `tools/make_io_fixture.py`, Apple-Silicon NVMe, 128 tokens, 5 runs/config): the win lives in the *head* of the prediction ranking. Median decode throughput vs `PILOT=0` baseline (2.2 tok/s): `PILOT_K=8` neutral (2.2), `K=6` +15% (2.55), `K=4` +23% (2.7), **`K=2` +41% (3.1)**, `K=1` +27% (2.8). When demand reads already saturate the disk, every mispredicted speculative byte displaces a demand byte — so a small, precise prefetch wins and a wide or deep one loses (`PILOT_N=4` at `K=8` measured −14%; `PILOT_N=2 PILOT_K=4` matched `N=1 K=4`). Deep lookahead (`PILOT_N`>1) is for **latency-bound** stores (VHDX, network mounts, spinning disks) where the queue is shallow and issuing reads early buys parallelism the demand path can't get on its own. Defaults are unchanged (`PILOT_N=1`, `PILOT_K=8`); on a fast NVMe try `PILOT=1 PILOT_K=2`.
